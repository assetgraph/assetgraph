    ...
    _.values(seenIncludingAssets).forEach(function (includingAsset) {
        ...
        function flushBundle() {
            if (currentBundle.length > 0) {
                bundleAssets.push(makeBundle(assetGraph, queryObj, currentBundle));
                currentBundle = [];
            }
        }
        assetGraph.findRelations({from: includingAsset, type: [queryObj.incoming.type, 'HtmlConditionalComment']}).filter(function (outgoingRelation) {
            if (outgoingRelation.type === 'HtmlConditionalComment' && assetGraph.findRelations({from: outgoingRelation.to, type: queryObj.incoming.type}).length > 0) {
                flushBundle();
...
bundleStrategyByName.sharedBundles = function (assetGraph, queryObj) {
    ...
    _.values(seenIncludingAssets).forEach(function (includingAsset) {
        var outgoingRelations = assetGraph.findRelations({from: includingAsset, type: [queryObj.incoming.type, 'HtmlConditionalComment']}, true), // includeUnresolved
            previousBundle,
            canAppendToPreviousBundle = false,
            previousBundleIndex;

        outgoingRelations.forEach(function (outgoingRelation) {
            if (outgoingRelation.type === 'HtmlConditionalComment' && assetGraph.findRelations({from: outgoingRelation.to, type: queryObj.incoming.type}) > 0) {
                canAppendToPreviousBundle = false;
                return;
            }
