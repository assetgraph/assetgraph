...
function makeBundle(assetGraph, queryObj, assetsToBundle, cb) {
    if (assetsToBundle.length === 0) {
        return cb(new Error('makeBundle: Bundle must contain at least one asset'));
    } else if (assetsToBundle.length === 1) {
        // Shortcut
        return process.nextTick(function () {
            cb(null, [assetsToBundle[0]]);
        });
    }
    seq(assetsToBundle)
        ...
        .unflatten()
        .seq(function (parseTrees) {
            var bundleAsset = new assets[queryObj.type]({
                parseTree: assets[queryObj.type].mergeParseTrees(parseTrees)
            });
            bundleAsset.url = urlTools.resolveUrl(assetGraph.root, bundleAsset.id + bundleAsset.getExtension()); // FIXME
...
bundleStrategyByName.oneBundlePerIncludingAsset = function (assetGraph, queryObj, cb) {
    ...
    seq(_.values(seenIncludingAssets))
        .seqEach(function (includingAsset) {
            var callback = this;
            seq(assetGraph.findRelations({from: includingAsset, type: queryObj.incoming.type}).filter(function (relation) {
                return relation.to.id in assetsToBundleById;
            }))
                ...
                .unflatten()
                .seq(function (assetsToBundle) {
                    makeBundle(assetGraph, queryObj, assetsToBundle, this);
                })
                .seq(function (bundleAsset) {
                    bundleAssets.push(bundleAsset);
                    callback();
                })
                ['catch'](callback);
        })
        .seq(function () {
            cb(null, bundleAssets);
        })
        ['catch'](cb);
};
...
bundleStrategyByName.sharedBundles = function (assetGraph, queryObj, cb) {
    ...
    seq(bundles)
        .parMap(function (bundle) {
            makeBundle(assetGraph, queryObj, bundle, this);
        })
        .unflatten()
        .seq(function (bundleAssets) {
            cb(null, bundleAssets);
        })
        ['catch'](cb);
};
...
module.exports = function (queryObj, bundleStrategyName) {
    ...
    return function bundleAssets(assetGraph, cb) {
        bundleStrategyByName[bundleStrategyName](assetGraph, queryObj, passError(cb, function (bundleAssets) {
            ...
            bundleAssets.forEach(function (bundleAsset) {
                ...
                assetGraph.findRelations({from: bundleAsset}).forEach(function (outgoingRelation) {
                    assetGraph.refreshRelationUrl(outgoingRelation);
                });
            });
            cb();
        }));
