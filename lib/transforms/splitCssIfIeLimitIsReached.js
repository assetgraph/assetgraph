...
module.exports = function (queryObj, options) {
    ...
    function splitStyleSheet(cssAsset) {
        ...
        cssAsset.parseTree.nodes.forEach(function (rule, idx) {
            ...
            if (accumulatedRules + selectors <= rulesPerStylesheetLimit) {
                ...
                output.push({
                    type: cssAsset.type,
                    isPopulated: false,
                    parseTree: postcss.root().append(cssAsset.parseTree.nodes.slice(offset, idx).map(cloneWithRaws))
                });
        ...
        output.push({
            type: cssAsset.type,
            isPopulated: false,
            parseTree: postcss.root().append(cssAsset.parseTree.nodes.slice(offset).map(cloneWithRaws))
        });
