...
module.exports = options => {
    ...
    return function serializeSourceMaps(assetGraph) {
        ...
        for (const asset of assetGraph.findAssets({ type: {$in: [ 'Css', 'JavaScript' ]}, isLoaded: true })) {
            // For now, don't attempt to attach source maps to data-bind attributes etc.:
            if (asset.isInline && assetGraph.findRelations({ type: { $in: [ 'HtmlDataBindAttribute', 'HtmlKnockoutContainerless', 'HtmlParamsAttribute', 'HtmlStyleAttribute' ] }, to: asset }).length > 0) {
                continue;
            }
            ...
            for (const existingSourceMapRelation of assetGraph.findRelations({ from: asset, type: {$regex: /SourceMappingUrl$/ }})) {
            ...
            asset.addRelation({
                ...
                hrefType: asset && asset.isInline ? 'inline' : undefined,
                to: {
                    type: 'SourceMap',
