...
module.exports = (queryObj, options) => {
    ...
    return async function addPrecacheServiceWorker(assetGraph) {
        async function generateAndAttachPrecacheServiceWorkerAsync(entryPointHtmlAssets) {
            ...
            for (const htmlAsset of entryPointHtmlAssets) {
                assetGraph.eachAssetPostOrder(htmlAsset, {type: {$nin: ['HtmlAnchor', 'SvgAnchor', 'HtmlMetaRefresh', 'HtmlCacheManifest', 'HtmlConditionalComment', 'JavaScriptSourceUrl', 'JavaScriptSourceMappingUrl', 'JavaScriptSourceMap']}}, asset => {
            ...
            if (!baseUrl || /^[^:]*:\/+$/.test(baseUrl)) {
                throw new Error('addPrecacheServiceWorker: HTML assets reside on different domains or schemes, cannot create a shared service worker');
            }
            const serviceWorkerUrl = baseUrl + serviceWorkerFileNameFragments.join('-');
            if (assetGraph.findAssets({url: serviceWorkerUrl}).length > 0) {
                throw new Error('There is already a service worker at ' + serviceWorkerUrl + ' -- giving up');
            }
            let serviceWorkerStr = await Promise.fromNode(cb => swPrecache.generate({
                dynamicUrlToDependencies: textOrRawSrcByUrl,
                logger(message) {
                    assetGraph.info(serviceWorkerUrl.replace(/^.*\//, '') + ': ' + message);
                }
            }, cb));
        ...
        if (options.single) {
            ...
            if (batches.length > 1) {
                assetGraph.info(new Error('addPrecacheServiceWorker: HTML assets reside on different domains or schemes, creating a service worker per origin'));
            }
        } else {
            batches = entryPointHtmlAssets.map(asset => [asset]);
        }
        await Promise.map(batches, generateAndAttachPrecacheServiceWorkerAsync, { concurrency: 1 });
    };
};
