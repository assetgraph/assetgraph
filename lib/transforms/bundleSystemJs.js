...
function stringifyConditions(conditions) {
    var str = '';
    Object.keys(conditions).forEach(function (condition) {
        str += '-' + condition + '-' + conditions[condition];
    });
    return str;
}
...
function bundleStrategy(builder, entryPoints, options) {
    ...
    function generateBundleName(name, conditions) {
            ...
            .then(function(conditionalEnv) {
                ...
                Object.keys(conditionalEnv).forEach(function(condition) {
                    conditionalEnv[condition].forEach(function(conditionValue) {
                        ...
                        conditionsCombinations.push(conditions);
                    });
                });

                if (!conditionsCombinations.length)
                    return getModuleTrace(entryPoint.name, options.conditions || {})
                ...
                return Promise.all(conditionsCombinations.map(function(conditions, index) {
                    return getModuleTrace(entryPoint.name, { conditions: Object.assign(Object.assign({}, conditions), options.conditions || {}) })
                    .then(function(modules) {
    ...
    .then(function() {
         ...
         */
        if (commonModules && commonModules.length) {
            bundles[commonBundleName = generateBundleName('common-bundle')] = {
        ...
        entryPointModules.forEach(function(entryPoint) {
            if (entryPoint.modules) {
                ...
                if (entryPointBundleModules.length) {
                    var bundleName = generateBundleName('bundle-' + entryPoint.name);
            ...
            // entry point has conditionals
                ...
                if (commonVariationModules && commonVariationModules.length) {
                    bundles[commonVariationBundleName = 'common-' + generateBundleName(entryPoint.name)] = {
                ...
                entryPoint.conditionalVariations.forEach(function(variation) {
                    ...
                    // subtract common layer
                    if (commonModules && commonModules.length && containsModules(entryPointModules, commonModules)) {
                        entryPointBundleModules = subtractModules(entryPointBundleModules, commonModules);
                        entryPointBundleBundles.push(commonBundleName);
                    }

                    // subtract common variation layer
                    if (commonVariationModules && commonVariationModules.length && containsModules(entryPointModules, commonVariationModules)) {
                    ...
                    if (entryPointBundleModules.length) {
                        var bundleName = generateBundleName('bundle-' + entryPoint.name, variation.conditions);
                        entryPointBundleBundles.push(bundleName);
                        bundles[bundleName] = {
                            modules: entryPointBundleModules,
                            source: undefined,
...
module.exports = function (options) {
    ...
    return function bundleSystemJs(assetGraph) {
        ...
        return bundleStrategy(builder, entryPointNames.map(function (entryPointName) {
            ...
            // REMOVE ME: Emulate the proposed new bundleStrategy return value
            var newSyntaxEntryPointBundles = {};
            result.entryPointBundles.forEach(function (entryPointBundle) {
                entryPointBundle.bundles.forEach(function (bundle) {
                    (newSyntaxEntryPointBundles[entryPointBundle.name] = newSyntaxEntryPointBundles[entryPointBundle.name] || []).push({
                        conditions: {},
                        bundle: bundle,
                        deferredParent: entryPointBundle.deferredParent
                    });
                });
            });
            result.entryPointBundles = newSyntaxEntryPointBundles;
            ...
            Object.keys(entryPointInfosByPageId).forEach(function (pageId) {
                ...
                entryPointInfosByPageId[pageId].forEach(function (occurrence, i) {
                    var entryPoint = occurrence.argumentString;
                    result.entryPointBundles[entryPoint].forEach(function (bundleInfo) {
                        if (bundleInfo.deferredParent) {
                            return;
                        }
                        var bundleId = bundleInfo.bundle;
                        var conditions = bundleInfo.conditions || {};
                        var conditionsString = stringifyConditions(bundleInfo.conditions);
                        // Not sure about whether conditionsString really should be used as part of the key below.
                        // Maybe the bundle ids will be different and it'll all work out:
                        if (!isSeenByPageIdAndBundleId[pageId][bundleId + conditionsString]) {
                            var bundle = result.bundles[bundleId];
                            isSeenByPageIdAndBundleId[pageId][bundleId] = true;
                            var nextSuffixToTry = 0;
                            var bundleUrl;
                            do {
                                bundleUrl = assetGraph.root + bundleId + conditionsString + (nextSuffixToTry ? '-' + nextSuffixToTry : '') + '.js';
                                nextSuffixToTry += 1;
                            ...
                            if (occurrence.incomingRelation.type === 'JavaScriptWebWorker') {
                                ...
                                if (htmlScriptsToRemove.indexOf(occurrence.incomingRelation) === -1) {
                                    ...
                                    Object.keys(conditions).forEach(function (condition) {
                                        htmlScript.node.setAttribute('data-cond-' + condition, conditions[condition]);
                                    });
            ...
            var deferredBundles = [];
            Object.keys(result.entryPointBundles).forEach(function (entryPoint) {
                result.entryPointBundles[entryPoint].forEach(function (bundleInfo) {
                    if (bundleInfo.deferredParent) {
                        deferredBundles.push(bundleInfo);
                    }
                });
            });
            deferredBundles.forEach(function (entryPointBundle) {
                ...
                var bundleId = entryPointBundle.bundle;
                ...
                if (!isSeenByPageIdAndBundleId[parentOccurrence.page.id][bundleId]) {
                    ...
                    var conditionsString = stringifyConditions(entryPointBundle.conditions);
                    var bundleUrl;
                    var nextSuffixToTry = 0;
                    do {
                        bundleUrl = assetGraph.root + bundleId + conditionsString + (nextSuffixToTry ? '-' + nextSuffixToTry : '') + '.js';
                    ...
                    // FIXME: Seems like identical bundles get added in the "multi-page lazy import System.import" test
                    // Add a "bundle by name + conditionsString" hash? Depends on how the bundle names turn out.
                    var bundleAsset = new AssetGraph.JavaScript({
                        text: bundle.source,
                        url: bundleUrl,
                        sourceMap: preprocessSourceMap(bundle.sourceMap)
