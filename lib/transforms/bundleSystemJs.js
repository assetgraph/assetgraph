...
function bundleStrategy(builder, entryPoints, options) {
    ...
    .then(function () {
        ...
        return Promise.all(allEntryPoints.map(function (entryPoint) {
            return builder.traceConditionalEnv(entryPoint.name, options)
            .then(function (conditionalEnv) {
                ...
                if (!conditionsCombinations.length) {
                    return getModuleTrace(entryPoint.name, Object.assign({ traceResolvedConditionModules: false }, options))
                ...
                return Promise.all(conditionsCombinations.map(function (conditions, index) {
                    return getModuleTrace(entryPoint.name, Object.assign(Object.assign({ traceResolvedConditionModules: false }, options), { conditions: Object.assign(Object.assign({}, conditions), options.conditions || {}) }))
                    .then(function (modules) {
                            ...
                            if (!isSeenByPageIdAndBundleId[pageId][bundleId]) {
                                ...
                                Array.prototype.push.apply(attachAssetsPromises, (bundle.assetList || []).map(function (assetListEntry) {
                                    ...
                                    if (assetListEntry.type === 'css') {
                                        ...
                                        var matchUrl = assetConfig.url.match(/^([^?]*)(\?.*|\#.*|$)/);
                                        if (matchUrl && !/\.css/i.test(matchUrl[1])) {
                                            assetConfig.url = matchUrl[1] + '.css' + matchUrl[2];
                                        }
                                    }
                                    ...
                                    if (asset.type === 'Css') {
                                        ...
                                        var keys = Object.keys(conditions);
                                        if (keys.length > 0) {
                                            htmlStyle.node.setAttribute('data-systemjs-conditionals', keys.map(function (key) {
                                                return (/[^a-z]/.test(key) ? singleQuoteString(key) : key) + ': ' + singleQuoteString(conditions[key]);
                                            }).join(', '));
