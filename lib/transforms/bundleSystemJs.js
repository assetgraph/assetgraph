...
/*
 * Creates one bundle per entry point
 * Where an entryPoint has a deferredParent, it will build the bundle based on the parent bundle being assumed
 *
 */
function simpleBundleStrategy(builder, entryPoints, options) {
    // disable plugin asset inlining as asset graph will handle this itself
    if (!('inlinePlugins' in options)) {
    ...
    return Promise.all(entryPoints.map(function(entryPoint) {
        ...
        // entry point name is bundle name for this strategy
        outObj.entryPointBundles.push({
            name: entryPoint.name,
            bundles: [entryPoint.name],
            deferred: false
        });
        return builder.bundle(entryPoint.name, options).then(function(output) {
            outObj.bundles[entryPoint.name] = {
        ...
        return Promise.all(entryPoints.map(function(entryPoint) {
            ...
            return builder.bundle(entryPoint.name + ' - ' + entryPoint.deferredParent, options).then(function(output) {
                outObj.bundles[entryPoint.name] = {
                    source: output.source,
                    sourceMap: output.sourceMap,
                    modules: output.modules,
 ...
 */
 function bundleStrategy(builder, entryPoints, options) {
    ...
    return Promise.all(entryPoints.map(function(entryPoint, index) {
        ...
        return builder.trace(entryPoint.name);
        ...
        function addDeferredEntryPoints(trace, deferredParent) {
            return builder.getDeferredImports(trace).then(function(deferredImports) {
                ...
                return Promise.all(newEntryPoints.map(function(entryPoint, index) {
                    ...
                    return builder.trace(entryPoint.name).then(function(trace) {
                        traces.push(trace);
                    });
                }));
            });
...
module.exports = function (options) {
    options = options || {};
    return function bundleSystemJs(assetGraph, cb) {
        var doneFirstConfigCall = false;
        var orderingError;
        var configStatements = assetGraph.systemJsConfig.configStatements.sort(function (a, b) {
            ...
            orderingError = new Error('System.config calls come in conflicting order across pages');
        });
        if (orderingError) {
            return cb(orderingError);
        }
                ...
                Object.keys(configStatement.obj.manualBundles).forEach(function (bundleId) {
                    if (manualBundles[bundleId]) {
                        if (!_.isEqual([].concat(manualBundles[bundleId]).sort(), [].concat(configStatement.obj.manualBundles[bundleId]).sort())) {
                            manualBundleError = new Error('Conflicting definitions of the manual bundle ' + bundleId);
                        }
                    } else {
        ...
        if (manualBundleError) {
            return cb(manualBundleError);
        }
        ...
        configStatements.forEach(function (configStatement, i) {
            if (!isConfigIncludedOnEveryPage(configStatement) && configStatements.some(function (otherConfigStatement, j) {
                ...
                conflictErr = conflictErr || new Error('Configs conflict');
            }
        });
        ...
        var bundleStrategyOptions = { deferredImports: options.deferredImports, manualBundles: manualBundles };

        bundleStrategy(builder, entryPointNames.map(function (entryPointName) {
            ...
            return Promise.all(attachAssetsPromises).then(function () {
                cb();
            });
        }, cb);
    };
