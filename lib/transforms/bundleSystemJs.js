...
function simpleBundleStrategy(builder, entryPoints, options) {
  ...
  return Promise.all(entryPoints.map(function(entryPoint) {
    ...
    return builder.bundle(entryPoint.name, options)
  ...
  .then(function() {
    return Promise.all(entryPoints.map(function(entryPoint) {
      ...
      return builder.bundle(entryPoint.name + ' - ' + entryPoint.deferredParent, options)
      .then(function(output) {
        outObj.bundles[entryPoint.name] = {
          source: output.source,
          sourceMap: output.sourceMap,
 ...
 function bundleStrategy(builder, entryPoints, options) {
   ...
   .then(function(_traces) {
     ...
     function addDeferredEntryPoints(trace, deferredParent) {
       return builder.getDeferredImports(trace)
   ...
   .then(function() {
     ...
     .then(function() {
       ...
       return Promise.all(Object.keys(options.manualBundles).map(function(manualBundle) {
         ...
         return builder.bundle('[' + bundleModules.join('] + [') + ']', options)
         .then(function(output) {
     ...
     .then(function() {
       if (hasModules(commonTree))
         return builder.bundle(commonTree, options)
         .then(function(output) {
     ...
     .then(function() {
       return Promise.all(traces.map(function(trace, index) {
         ...
         if (hasModules(moduleTree))
           return builder.bundle(moduleTree, options)
           .then(function(output) {
...
module.exports = function (options) {
    ...
    return function bundleSystemJs(assetGraph, cb) {
        ...
        var strategyOptionsBySeenEntryPoint = {};
        var occurrences = [];
        ...
        }), { deferredImports: options.deferredImports }).then(function (result) {
