...
function extractSystemJsConfig(incomingRelations) { // HtmlScript, JavaScriptWebWorker, HtmlServiceWorkerRegistration and JavaScriptImportScripts
    var systemJsConfig = {
        configStatements: [],
        ...
        return Promise.all(allEntryPoints.map(function (entryPoint) {
            ...
            .then(function (conditionalEnv) {
                ...
                return Promise.all(conditionsCombinations.map(function (conditions, index) {
                    return getModuleTrace(entryPoint.name, Object.assign(Object.assign({ inlineConditions: inlineConditions }, options), { conditions: Object.assign(Object.assign({}, conditions), options.conditions || {}) }))
                    .then(function (modules) {
...
module.exports = function (options) {
    ...
    return function bundleSystemJs(assetGraph) {
        ...
        var systemJsConfig = extractSystemJsConfig(assetGraph.findRelations({type: ['HtmlScript', 'JavaScriptWebWorker', 'HtmlServiceWorkerRegistration', 'JavaScriptImportScripts'], to: {isLoaded: true}}));
        ...
        topLevelSystemImportCalls.forEach(function (topLevelSystemImportCall) {
            ...
            assetGraph.findRelations({ type: ['JavaScriptWebWorker', 'HtmlServiceWorkerRegistration', 'HtmlScript'], to: asset }).forEach(function (incomingRelation) {
                    ...
                    result.entryPointBundles.filter(function (entryPointBundlesEntry) {
                        ...
                        bundleIds.forEach(function (bundleId) {
                            if (!isSeenByPageIdAndBundleId[pageId][bundleId]) {
                                ...
                                if (occurrence.incomingRelation.type === 'JavaScriptWebWorker' || occurrence.incomingRelation.type === 'HtmlServiceWorkerRegistration') {
                                    ...
                                    if (htmlScriptsToRemove.indexOf(occurrence.incomingRelation) === -1) {
                                        ...
                                        if (bundleId !== 'common-bundle' && keys.length > 0) {
                                            htmlScript.node.setAttribute('data-systemjs-conditions', keys.map(function (key) {
                                ...
                                Array.prototype.push.apply(attachAssetsPromises, (bundle.assetList || []).map(function (assetListEntry) {
                                    ...
                                    if (asset.type === 'Css') {
                                        ...
                                        htmlStyle.attach(occurrence.incomingRelation.from, 'last');
                                        var keys = Object.keys(conditions);
                                        if (bundleId !== 'common-bundle' && keys.length > 0) {
                                            htmlStyle.node.setAttribute('data-systemjs-conditions', keys.map(function (key) {
                                                return (/[^a-z]/.test(key) ? singleQuoteString(key) : key) + ': ' + singleQuoteString(conditions[key]);
                                            }).join(', '));
