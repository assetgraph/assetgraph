...
async function bundleStrategy(builder, entryPoints, options) {
    ...
    if (options.deferredImports) {
        await Promise.map(entryPoints, async entryPoint => {
            ...
            allEntryPoints.push(...deferredImports.map(deferredImport => ({
...
module.exports = options => {
    ...
    return async function bundleSystemJs(assetGraph) {
        ...
        const systemJsConfig = extractSystemJsConfig(assetGraph.findRelations({type: {$in: ['HtmlScript', 'HtmlServiceWorkerRegistration', 'JavaScriptServiceWorkerRegistration', 'JavaScriptWebWorker', 'JavaScriptImportScripts']}, to: {isLoaded: true}}));
        ...
        const configStatements = systemJsConfig.configStatements.sort((a, b) => {
            ...
            const pageIds = _.uniq([...Object.keys(aRelationsByPageId), ...Object.keys(bRelationsByPageId)]);
        ...
        for (const htmlScript of assetGraph.findRelations({ type: 'HtmlScript' })) {
            ...
            if (htmlScript.node.hasAttribute('data-systemjs-polyfill')) {
                if (options.polyfill) {
                    const href = htmlScript.node.getAttribute('data-systemjs-polyfill') ||
            ...
            if (htmlScript.node.hasAttribute('data-systemjs-csp-production')) {
                ...
                htmlScript.to = assetGraph.addAsset({ url: assetGraph.resolveUrl(htmlScript.from.nonInlineAncestor.url, href) });
        ...
        for (const topLevelSystemImportCall of topLevelSystemImportCalls) {
            const incomingRelations = assetGraph.findRelations({
                type: {$in: ['HtmlScript', 'HtmlServiceWorkerRegistration', 'JavaScriptServiceWorkerRegistration', 'JavaScriptWebWorker']},
            ...
            if (configStatement.obj.manualBundles) {
                for (const bundleId of Object.keys(configStatement.obj.manualBundles)) {
                    if (manualBundles[bundleId]) {
                        if (!_.isEqual([...manualBundles[bundleId]].sort(), [...configStatement.obj.manualBundles[bundleId]].sort())) {
        ...
        try {
            ...
            function addBundle(bundleId, incomingRelation, conditions) {
                ...
                if (incomingRelation.type === 'HtmlServiceWorkerRegistration' || incomingRelation.type === 'JavaScriptServiceWorkerRegistration' || incomingRelation.type === 'JavaScriptWebWorker') {
                    const lastImportScript = assetGraph.findRelations({type: 'JavaScriptImportScripts', from: incomingRelation.to}).pop();

                    incomingRelation.to.addRelation({
                        type: 'JavaScriptImportScripts',
                        to: bundleAsset,
                        hrefType: 'rootRelative'
                    }, lastImportScript ? 'after' : 'first', lastImportScript);
                } else {
                    // assume 'HtmlScript'
                ...
                for (const { source, type, url, sourceMap } of (bundle.assetList || [])) {
                    ...
                    if (asset.type === 'Css') {
                        ...
                        assetGraph.warn(new Error('Cannot attach relation to ' + asset.urlOrDescription + ' when the <script> containing the entry point has the data-systemjs-remove property'));
                    }
                }
            }
            ...
            for (const bundleId of Object.keys(bundles)) {
                ...
                for (const { deferredParent } of bundle.entryPoints) {
                    if (deferredParent) {
                        ...
                        }, 'before', parentOccurrence.incomingRelation).to;
            ...
            for (const key of [...Object.keys(systemJsGlobals), ..._.difference(Object.keys(global), Object.keys(globalSnapshotBeforeBundling))]) {
                delete global[key];
            }
        }
    };
