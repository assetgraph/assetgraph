                            ...
                            if (!isSeenByPageIdAndBundleId[pageId][bundleId]) {
                                ...
                                if (occurrence.incomingRelation.type === 'HtmlServiceWorkerRegistration' || occurrence.incomingRelation.type === 'JavaScriptServiceWorkerRegistration' || occurrence.incomingRelation.type === 'JavaScriptWebWorker') {
                                    ...
                                    if (htmlScriptsToRemove.indexOf(occurrence.incomingRelation) === -1) {
                                        ...
                                        if (bundleId !== 'common-bundle' && keys.length > 0) {
                                            htmlScript.node.setAttribute('data-assetgraph-conditions', keys.map(function (key) {
                                ...
                                (bundle.assetList || []).forEach(function (assetListEntry) {
                                    ...
                                    if (asset.type === 'Css') {
                                        ...
                                        var keys = Object.keys(conditions);
                                        if (bundleId !== 'common-bundle' && keys.length > 0) {
                                            htmlStyle.node.setAttribute('data-assetgraph-conditions', keys.map(function (key) {
                                                return (/[^a-z]/.test(key) ? singleQuoteString(key) : key) + ': ' + singleQuoteString(conditions[key]);
                                            }).join(', '));
