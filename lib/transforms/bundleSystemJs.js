...
async function bundleStrategy(builder, entryPoints, options) {
  ...
  await Promise.all(
    allEntryPoints.map(async entryPoint => {
      conditionalEnv = await builder.traceConditionalEnv(entryPoint.name, {
        inlineConditions,
        ...options
      });
      ...
      function generateVariationsOverConditions(conditionList) {
        ...
        for (const nextVariation of nextVariations) {
          for (const curConditionValue of conditionalEnv[curCondition]) {
            const variationConditions = { ...nextVariation };
            variationConditions[curCondition] = curConditionValue;
            variations.push(variationConditions);
          }
        }
      ...
      if (conditionsCombinations.length === 0) {
        const modules = await getModuleTrace(entryPoint.name, {
          inlineConditions,
          ...options
        });
  ...
  await Promise.all(
    Object.keys(bundles).map(async bundleName => {
      ...
      const output = await builder.bundle(bundle.modules, {
        inlineConditions,
        ...options
      });
...
module.exports = ({ polyfill = false, conditions, deferredImports } = {}) => {
  return async function bundleSystemJs(assetGraph) {
    ...
    const globalSnapshotBeforeRequire = { ...global };
    ...
    // Mix SystemJS globals into global object
    for (const [key, value] of Object.entries(systemJsGlobals)) {
