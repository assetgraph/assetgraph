            ...
            enter: function (node) {
                if (node.type === 'CallExpression' &&
                    ...
                    if (node.callee.property.name === 'config') {
                        seenConfig = true;
                        systemJsConfig.configStatements.push({ asset: relation.to, node: node });
...
function bundleStrategy(builder, entryPoints, options) {
    ...
    // TODO:
    // support "conditions" as a new input option
    // Then shape entryPointBundles itself by conditions where entryPointBundles looks like:
    //   { name: [ conditions: { lang: 'en', config: 'dev' }, bundle: [....] ] }
    // We can then generate a common bundle for each conditional variation ensuring
    // bundles are properly shared and deduped against previously generated bundles
    // and nulled when they are empty.
    var outObj = {
        entryPointBundles: [],
        bundles: {}
    };
...
module.exports = function (options) {
    ...
    return function bundleSystemJs(assetGraph) {
        ...
        assetGraph.findRelations({ type: 'HtmlScript' }, true).forEach(function (htmlScript) {
            ...
            if (htmlScript.node.hasAttribute('data-systemjs-build-config')) {
                potentiallyOrphanedAssetsById[htmlScript.to.id] = htmlScript.to;
                htmlScript.node.removeAttribute('data-systemjs-build-config');
                htmlScript.detach();
                markDirty = true;
            }
            if (markDirty) {
