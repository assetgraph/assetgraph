...
function getSubsetsForFontUsage(assetGraph, htmlAssetTextsWithProps, formats) {
    ...
    htmlAssetTextsWithProps.forEach(function (item) {
        item.fontUsages.forEach(function (fontUsage) {
            ...
            var fontRelation = assetGraph.findRelations({ to: { url: fontUsage.props.src }}, true)[0];
    ...
    // console.log(htmlAssetTextsWithProps[0].fontUsages);
                ...
                htmlAssetTextsWithProps.forEach(function (item) {
                    item.fontUsages.forEach(function (fontUsage) {
                        ...
                        formats.forEach(function (format) {
                            ...
                            subsetPromiseMap[promiseId].then(function (subsetBuffer) {
                                if (!fontUsage.subsets) {
                                    fontUsage.subsets = {};
                                }

                                fontUsage.subsets[format] = subsetBuffer;
                            });
                        });
                    });
                });
...
var getFontFaceForFontUsage = memoizeSync(function (fontUsage, fontDisplay) {
    var subsets = fontOrder.map(function (format) {
    ...
    var hasInsertedFontDisplay = false;
    ...
    resultString.push.apply(resultString, Object.keys(fontUsage.props)
        ...
        .map(function (prop) {
            ...
            if (prop === 'font-family') {
                value = cssQuoteIfNecessary(value);
            }
            ...
            if (prop === 'font-display' && fontDisplay) {
                hasInsertedFontDisplay = true;
    ...
    if (fontDisplay && !hasInsertedFontDisplay) {
        resultString.push('  font-display: ' + fontDisplay);
    }
...
function getFontUsageStylesheet(fontUsages, fontDisplay) {
    var stylesheet = fontUsages
        .filter(function (fontUsage) { return fontUsage.subsets; })
        .map(function (fontUsage) {
            return getFontFaceForFontUsage(fontUsage, fontDisplay);
        })
        .join('\n\n');
...
module.exports = function (options) {
    ...
    return function subsetGoogleFonts(assetGraph, cb) {
        ...
        assetGraph
            ...
            .queue(function (assetGraph) {
                return getSubsetsForFontUsage(assetGraph, htmlAssetTextsWithProps, ['woff2', 'woff']);
            })
            .queue(function insertSubsets(assetGraph) {
                ...
                htmlAssetTextsWithProps
                    .forEach(function (htmlAssetTextWithProps) {
                        ...
                        var subsetCssText = getFontUsageStylesheet(subsetFontUsages, fontDisplay);
                        ...
                        var fallbackPreloadFontAssets = [];

                        if (!inlineSubsets) {
                            cssAsset.outgoingRelations.forEach(function (fontRelation) {
                                var fontAsset = fontRelation.to;
                                fallbackPreloadFontAssets.push(fontAsset);
                        ...
                        unsubsettedFontUsages.forEach(function (fontUsage) {
                            var fontAsset = assetGraph.findAssets({ url: fontUsage.props.src });
                            fallbackPreloadFontAssets.push(fontAsset);
