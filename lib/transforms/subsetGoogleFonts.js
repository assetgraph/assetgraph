        ...
        assetGraph
            ...
            .queue(function moveSubsetAssetsToLocal(assetGraph) {
                Object.keys(htmlAssetTextByPropsMap).forEach(function (assetUrl) {
                    ...
                    let fontStyleBundle = assetGraph.findAssets({ type: 'Css', isLoaded: true, url: /google-font-subsets\/fonts-/, originalText: source })[0];

                    if (!fontStyleBundle) {
                        fontStyleBundle = assetGraph.addAsset({
                            type: 'Css',
                            text: source,
                            originalText: source, // So that it's queryable even though it's minified and manipulated
                            fontStyleBundle: true
                        });
                    ...
                    var bundleRelation = htmlAsset.addRelation({
                        type: 'HtmlStyle',
                        hrefType: 'rootRelative',
                        to: fontStyleBundle
                    }, 'before', fontStyleSheetRelations[0]);
            ...
            .minifyCss({fontStyleBundle: true})
