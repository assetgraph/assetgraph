            ...
            for (var i = fragments.length ; i > 0 ; i -= 1) {
                ...
                if (prefix in this.paths) {
                    if (i === fragments.length) {
                        return urlTools.resolveUrl(baseUrl || this.baseUrl, this.paths[prefix] + '.js');
                    } else {
                        return urlTools.resolveUrl(baseUrl || this.baseUrl, urlTools.resolveUrl(this.paths[prefix].replace(/\/?$/, '/'), fragments.slice(i).join('/') + (lastFragmentExtension || '')));
                    }
                }
            }
            return urlTools.resolveUrl(baseUrl || this.baseUrl, url);
        };
                        ...
                        if (keyValueNode[0] === 'baseUrl' && keyValueNode[1][0] === 'string') {
                            ...
                            keyValueNode[1][1].forEach(function (pathNode) {
                                if (pathNode[1][0] === 'string') {
                                    requireJsConfig.paths[pathNode[0]] = pathNode[1][1];
                                } else {
