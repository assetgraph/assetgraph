            ...
            for (var i = fragments.length ; i > 0 ; i -= 1) {
                ...
                if (prefix in this.paths) {
                    if (i === fragments.length) {
                        url = this.paths[prefix] + '.js';
                    } else {
                        url = urlTools.resolveUrl(this.paths[prefix].replace(/\/?$/, '/'), fragments.slice(i).join('/') + (lastFragmentExtension || ''));
                    }
                    break;
                }
            }
            return assetGraph.resolveUrl(baseUrl, url);
        };
