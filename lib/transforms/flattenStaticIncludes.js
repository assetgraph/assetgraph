            ...
            if (!(relation.to.id in seenAssets)) {
                ...
                if (relation.to.type === 'CSS') {
                    ...
                    if (htmlStyleInsertionPoint) {
                        assetGraph.attachAndRegisterRelation(htmlStyle, 'after', htmlStyleInsertionPoint);
                    } else {
                        assetGraph.attachAndRegisterRelation(htmlStyle, 'first');
                    }
                    htmlStyleInsertionPoint = htmlStyle;
                } else {
                    assetGraph.attachAndRegisterRelation(new relations.HTMLScript({from: asset, to: relation.to}), 'before', htmlScriptRelation);
                }
            }
