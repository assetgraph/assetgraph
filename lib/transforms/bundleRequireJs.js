        ...
        assetGraph.findAssets(_.extend({type: 'Html'}, queryObj)).forEach(function (htmlAsset) {
            assetGraph.findRelations({from: htmlAsset, type: 'HtmlRequireJsMain'}).forEach(function (htmlRequireJsMain) {
                ...
                assetGraph.eachAssetPostOrder(htmlRequireJsMain, {type: ['JavaScriptAmdRequire', 'JavaScriptAmdDefine']}, function (asset, incomingRelation) {
                    ...
                    var moduleName = urlTools.buildRelativeUrl(incomingRelation.baseAsset.url, asset.url).replace(/\.js$/, "");
                    ...
                    if (asset.type === 'JavaScript') {
                        ...
                        if (/^kotpl!/.test(incomingRelation.node[1])) {
                            moduleName = moduleName.replace(/\.ko$/, "");
                            injectDefineStatementValueAst = ["assign",true,["sub",["dot",["dot",["name","ko"],"externalTemplateEngine"],"templates"],["string",moduleName.replace(/^.*\//, '')]],injectDefineStatementValueAst];
                            incomingRelation.node[1] = incomingRelation.node[1].replace(/^kotpl!/, '');
                        }
