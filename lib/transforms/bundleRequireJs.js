        ...
        assetGraph.findAssets(_.extend({type: 'Html'}, queryObj)).forEach(function (htmlAsset) {
            ...
            assetGraph.findRelations({from: htmlAsset, type: ['HtmlRequireJsMain', 'HtmlScript']}).forEach(function (startRelation) {
                ...
                function getModuleName(asset) {
                    ...
                    assetGraph.findRelations({to: asset, type: ['JavaScriptAmdRequire', 'JavaScriptAmdDefine', 'JavaScriptShimRequire']}).forEach(function (incomingRelation) {
                        var referencedModuleName = incomingRelation.href.replace(/\.js$/, '');
                        if (incomingRelation.isRelative) {
                ...
                assetGraph.eachAssetPostOrder(startRelation, {type: ['JavaScriptAmdRequire', 'JavaScriptAmdDefine', 'JavaScriptShimRequire'], to: {type: 'JavaScript'}}, function (asset) {
                    ...
                    var hasFoundPlausibleDefineStatementValueAst = false,
                        ...
                        moduleName = getModuleName(asset),
                        dependencyArrayAst,
                    ...
                    if (!hasFoundPlausibleDefineStatementValueAst) {
                        if (requireJsConfig.shim[moduleName] && requireJsConfig.shim[moduleName].exports) {
                            injectDefineStatementValueAst = uglifyAst.parseExpression(requireJsConfig.shim[moduleName].exports);
                        } else if (moduleName) {
                    ...
                    if (defineStatementAst && !injectDefineStatementAfter) {
                        bundleParseTree.body.push(defineStatementAst);
                    }
