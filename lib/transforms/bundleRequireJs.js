                        ...
                        if (/^JavaScript(?:ShimRequire|Amd(?:Define|Require))$/.test(outgoingRelation.type)) {
                            ...
                            if (injectDefineStatementValueAst) {
                                var callNode = new uglifyJs.AST_Call({
                                    ...
                                    args: [
                                        new uglifyJs.AST_String({value: getModuleName(outgoingRelation.to)}),
                                        injectDefineStatementValueAst
                                    ]
                                });
                            ...
                            if (outgoingRelation.type === 'JavaScriptInclude' && outgoingRelation.parentNode === outgoingRelation.from.parseTree.body) {
