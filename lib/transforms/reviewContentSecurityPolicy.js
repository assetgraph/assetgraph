    ...
    return function reviewContentSecurityPolicy(assetGraph) {
        ...
        assetGraph.findAssets(queryObj || { type: 'Html', isInline: false, isFragment: false, isLoaded: true }).forEach(function (htmlAsset) {
            ...
            htmlContentSecurityPolicies.forEach(function (htmlContentSecurityPolicy) {
                ...
                assetGraph.eachAssetPreOrder(htmlAsset, {type: assetGraph.query.not(['HtmlAnchor', 'HtmlMetaRefresh', 'JavaScriptSourceUrl', 'JavaScriptSourceMappingUrl', 'CssSourceUrl', 'CssSourceMappingUrl'])}, function (asset, incomingRelation) {
                ...
                directives.forEach(function (directive) {
                    ...
                    if (options.update) {
                        ...
                        origins = origins.filter(function (token) {
                            if (token === '\'nonce-developmentonly\'') {
                        ...
                        if (noncesWereRemoved && origins.indexOf("'unsafe-inline'") !== -1 && !origins.some(function (origin) {return /^'(?:nonce|sha\d+)-/.test(origin);})) {
