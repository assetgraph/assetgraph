...
module.exports = (
  queryObj,
  { includePath = false, update = false, level } = {}
) => {
  return function reviewContentSecurityPolicy(assetGraph) {
    ...
    for (const htmlAsset of assetGraph.findAssets(
      ...
      for (const htmlContentSecurityPolicy of htmlContentSecurityPolicies) {
        ...
        function supportsStrictDynamic(directive) {
          return contentSecurityPolicy.parseTree[directive]
            ? contentSecurityPolicy.parseTree[directive].includes(
                "'strict-dynamic'"
              )
            : defaultSrc &&
                ...
                defaultSrc.includes("'strict-dynamic'");
        }
          ...
          if (directive) {
            if (asset.isInline) {
              if (
                ...
                (!supportsUnsafeInline(directive) ||
                  hasNonceOrHash(directive) ||
                  supportsStrictDynamic(directive))
              ) {
