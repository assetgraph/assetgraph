        ...
        for (const htmlAsset of assetGraph.findAssets(queryObj || { type: 'Html', isInline: false, isFragment: false, isLoaded: true })) {
            ...
            for (const htmlContentSecurityPolicy of htmlContentSecurityPolicies) {
                ...
                function supportsUnsafeHashedAttributes(directive) {
                    return (
                        contentSecurityPolicy.parseTree[directive] ?
                            contentSecurityPolicy.parseTree[directive].includes("'unsafe-hashed-attributes'") :
                            defaultSrc && ContentSecurityPolicy.directiveFallsBackToDefaultSrc(directive) && defaultSrc.includes("'unsafe-hashed-attributes'")
                    );
                }
                ...
                function supportsHashSource(directive, hashSource) {
                    return (
                        contentSecurityPolicy.parseTree[directive] ?
                            ...
                            defaultSrc && ContentSecurityPolicy.directiveFallsBackToDefaultSrc(directive) && defaultSrc.includes(hashSource)
                    );
                }
                    ...
                    if (directive) {
                        if (asset.isInline) {
                            if ((directive === 'styleSrc' || directive === 'scriptSrc') && (!supportsUnsafeInline(directive) || hasNonceOrHash(directive))) {
                                ...
                                const isUnsafeHashedAttribute = incomingInlineRelation && (incomingInlineRelation.type === 'HtmlStyleAttribute' || incomingInlineRelation.type === 'HtmlInlineEventHandler');
                                ...
                                if (isUnsafeHashedAttribute) {
                                    ...
                                    encoding = incomingInlineRelation.from.nonInlineAncestor.encoding;
                                } else {
                                    rawSrc = asset.rawSrc;
                                }
                                const hashSource = `'sha256-${crypto.createHash('sha256').update(rawSrc, encoding).digest('base64')}'`;
                                ...
                                if (isUnsafeHashedAttribute && !supportsUnsafeHashedAttributes(directive)) {
                                    disallowedRelationsByDirectiveAndOrigin[directive] = disallowedRelationsByDirectiveAndOrigin[directive] || {};
                                    (disallowedRelationsByDirectiveAndOrigin[directive]["'unsafe-hashed-attributes'"] = disallowedRelationsByDirectiveAndOrigin[directive][hashSource] || []).push(incomingRelation);
                                }
                    ...
                    if (incomingRelation && incomingRelation.type === 'HttpRedirect') {
                        ...
                        const queue = [
                            ...incomingRelation.from.incomingRelations.filter(
                                relation => seenAssets.has(relation.from)
                            )
                        ];
                            ...
                            if (!seenRelations.has(relation)) {
                                ...
                                if (relation.type === 'HttpRedirect') {
                                    queue.push(...relation.from.incomingRelations.filter(relation => {
                                        return seenAssets.has(relation.from) && !seenRelations.has(relation);
                                    }));
                                } else {
                ...
                for (const directive of directives) {
                    let originsToAdd = Object.keys(disallowedRelationsByDirectiveAndOrigin[directive] || {});
                    let existingOrigins;
                    ...
                    if (options.update) {
                        ...
                        if (noncesWereRemoved && origins.includes("'unsafe-inline'") && !origins.some(isNonceOrHash)) {
                            ...
                            // because of relations that require 'unsafe-hashed-attributes'
                            if (nonceOrHashOrigins.length > 0 && !origins.includes("'unsafe-inline'")) {
                                if (!nonceOrHashOrigins.every(hashSource => {
                                    const relations = disallowedRelationsByDirectiveAndOrigin[directive] && disallowedRelationsByDirectiveAndOrigin[directive][hashSource];
                                    return relations && relations.every(relation => relation.type === 'HtmlInlineEventHandler' || relation.type === 'HtmlStyleAttribute');
                                })) {
                        ...
                        for (const nonWhitelistedOrigin of origins) {
                            ...
                            if (relations) {
                                ...
                                if (relations.length > 0) {
                                    if (nonWhitelistedOrigin === "'unsafe-hashed-attributes'") {
                                        if (options.level >= 3) {
                                            assetGraph.warn(
                                                new Error(
                                                    `${htmlAsset.urlOrDescription}: Missing ${fromCamelCase(directive)} 'unsafe-hashed-attributes':\n  ` +
                                            ...
                                            assetGraph.warn(
                                                new Error(
                                                    `${htmlAsset.urlOrDescription} contains one or more inline ${directive === 'styleSrc' ? 'style attributes' : 'event handlers'}, which cannot be whitelisted with CSP level 2.\n` +
