...
function directiveIsHighlySensitive(directive) {
    return directive === 'scriptSrc' || directive === 'styleSrc' || directive === 'frameSrc' || directive === 'objectSrc' || directive === 'manifestSrc' || directive === 'childSrc';
}
    ...
    return function reviewContentSecurityPolicy(assetGraph) {
        function originFromUrl(url, directive) {
            if (assetGraph.root && url.indexOf(assetGraph.root) === 0) {
                return '\'self\'';
            } else if (directiveIsHighlySensitive(directive)) {
                return url;
            } else {
        ...
        assetGraph.findAssets(queryObj || { type: 'Html', isInline: false, isFragment: false, isLoaded: true }).forEach(function (htmlAsset) {
            ...
            htmlContentSecurityPolicies.forEach(function (htmlContentSecurityPolicy) {
                ...
                assetGraph.eachAssetPostOrder(htmlAsset, {type: assetGraph.query.not(['HtmlAnchor', 'HtmlMetaRefresh', 'JavaScriptSourceUrl', 'JavaScriptSourceMappingUrl', 'CssSourceUrl', 'CssSourceMappingUrl'])}, function (asset, incomingRelation) {
                ...
                directives.forEach(function (directive) {
                    ...
                    if (options.update) {
                        ...
                        origins = origins.filter(function (token) {
                            return token !== '\'nonce-developmentonly\'';
                        });
                        if (origins.length > 1) {
                        ...
                        if (directive !== 'defaultSrc' && defaultSrc && _.difference(defaultSrc, origins).length === 0 && _.difference(origins, defaultSrc).length === 0) {
