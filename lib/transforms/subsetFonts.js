...
function fontFaceDeclarationForProps(fontProps, isSubset) {
  const display = fontProps['font-display'];
  const family = cssQuoteIfNecessary(
    fontProps['font-family'] + (isSubset ? '__subset' : '')
  );

  const cssText = `@font-face {
  ${(display && `font-display: ${display};`) || ''}
  font-family: ${family};
  font-weight: ${fontProps['font-weight']};
  font-style: ${fontProps['font-style']};
  font-stretch: ${fontProps['font-stretch']};
  src: ${fontProps['src']};
  unicode-range: ${fontProps['unicode-range']};
}`;
