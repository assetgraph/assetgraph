...
const fontPropsWithDefaults = [
  'font-style',
  'font-weight',
  'font-stretch',
  'font-display'
];
const initialValueByProp = _.pick(
  require('../util/fonts/initialValueByProp'),
  fontPropsWithDefaults
);
...
function getFontName(asset, fontProps) {
  const name = unquote(fontProps['font-family'])
    .replace(/__subset$/, '')
    .replace(/ /g, '_');

  const weight = normalizeFontPropertyValue(
    'font-weight',
    fontProps['font-weight']
  );

  const style = fontProps['font-style'] === 'italic' ? 'i' : '';

  const extension = `${asset.contentType.split('/').pop()}`;

  return `${name}-${weight}${style}-${asset.md5Hex.slice(0, 10)}.${extension}`;
}

function fontFaceDeclarationForProps(fontProps, isSubset) {
  const props = {};

  // Filter out initial values
  for (const [prop, value] of Object.entries(fontProps)) {
    if (prop === 'font-family') {
      props['font-family'] = cssQuoteIfNecessary(
        value + (isSubset ? '__subset' : '')
      );
    } else if (
      value !== normalizeFontPropertyValue(prop, initialValueByProp[prop])
    ) {
      props[prop] = value;
    }
  }
  ...
  return `@font-face {${propStrings.join(';')}}`;
}

const getFontFaceForFontUsage = memoizeSync(
  (assetGraph, fontUsage) => {
    const subsets = fontOrder
      .filter(format => fontUsage.subsets[format])
      .map(format => {
        const fontAsset = assetGraph.addAsset({
          contentType: fontContentTypeMap[format],
          rawSrc: fontUsage.subsets[format],
          subfont: true
        });

        fontAsset.fileName = getFontName(fontAsset, fontUsage.props);

        return {
          format,
          font: fontAsset,
          url: `data:${fontContentTypeMap[format]};base64,`
        };
      });

    const src = subsets
      .map(subset => `url(${subset.font.url}) format('${subset.format}')`)
      .join(', ');

    return fontFaceDeclarationForProps(
      {
        ...fontUsage.props,
  ...
  },
  {
    argumentsStringifier: args => {
      return [args[1].text, args[1].props]
...
function getFontUsageStylesheet(
  assetGraph,
  fontUsages,
  ...
  return fontUsages
    ...
    .map(fontUsage => getFontFaceForFontUsage(assetGraph, fontUsage))
    .concat(
      unusedFontFaceDeclarations.map(props => {
        let src = stripLocalTokens(props.src);
        if (props.relations.length > 0) {
          const fontAsset = props.relations[0].to;

          fontAsset.fileName = getFontName(fontAsset, props);
          fontAsset.subfont = true;

          src = `url(${fontAsset.url}) format(${fontAsset.contentType
            .split('/')
            .pop()})`;
        }

        const { relations, ...cssProps } = props;
...
These glyphs are used on your site, but they don't exist in the font you applied to them:`;
    ...
    for (const {
      ...
      if (unsubsettedFontUsages.length > 0) {
        ...
        const preloadData = unsubsettedFontUsages.map((fontUsage, idx) => {
          ...
          return `new FontFace(
            "${name}",
            "url('" + "__subfont__".toString('url') + "') format('${
      ...
      for (const fontRelation of cssAsset.outgoingRelations) {
        ...
        if (!fontAsset.isLoaded) {
          console.log(assetGraph.root);
          await assetGraph.drawGraph();
        }
      ...
      const cssUrl = subsetUrl + cssFileName;
      const [identicalAsset] = assetGraph.findAssets({ url: cssUrl });

      if (identicalAsset) {
        assetGraph.removeAsset(cssAsset);
        cssAsset = identicalAsset;
      }
              ...
              const urlValues = urlStrings.map((urlString, idx) =>
                urlString
                  .replace(
                    ...
                    `'" + "${fontRelations[idx].href}".toString("url") + "'`
                  )
                  .replace(/format\((.+?)\)/, `format('$1')`)
              );
              url = `"${urlValues.join(', ')}"`;
            } else if (prop in propNameMap) {
              props[propNameMap[prop]] = normalizeFontPropertyValue(
                prop,
                value
              );
            }
    ...
    // Move font subsets into correct directory
    for (const fontAsset of assetGraph.findAssets({
      isFont: true,
      subfont: true
    })) {
      fontAsset.path = subsetPath;
    }
    ...
    // FIXME: This shouldn't be needed. Mostly to clean up when running tests
    getFontFaceForFontUsage.purgeAll();
