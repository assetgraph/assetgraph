      ...
      assetGraph.warn(
        new Error(
          'The font ' +
            ...
            _.uniq(missingCharsByFontSrc[fontSrc])
              .map(
                ch =>
                  `U+${ch
                    .charCodeAt(0)
      ...
      for (const fontUsage of item.fontUsages) {
        ...
        for (const format of formats) {
          ...
          if (!fontCssUrlMap[mapId]) {
            fontCssUrlMap[mapId] =
              getGoogleFontSubsetCssUrl(fontUsage.props, fontUsage.text) +
              '&format=' +
              format;
          }
        }
  ...
  fontUsage => {
    const subsets = fontOrder
      ...
      .map(format => ({
        ...
        url:
          'data:' +
          fontContentTypeMap[format] +
          ';base64,' +
          fontUsage.subsets[format].toString('base64')
      }));
  ...
  return async function subsetFonts(assetGraph) {
          ...
          .addRelation(
            {
              ...
              to: {
                type: 'JavaScript',
                text:
                  'try {' + fontFaceContructorCalls.join('') + '} catch (e) {}'
              }
            },
            'before',
            cssRelation
    ...
    for (const googleFontStylesheet of googleFontStylesheets) {
      ...
      for (const googleFontStylesheetRelation of googleFontStylesheet.incomingRelations) {
        if (seenPages.has(googleFontStylesheetRelation.from)) {
          continue;
        }
        seenPages.add(googleFontStylesheetRelation.from);
        ...
        if (cssRule.type === 'decl' && cssRule.prop === 'font-family') {
          if (cssRule.parent.type === 'rule') {
            ...
            if (subsetFontFamily && !fontFamilies.includes(subsetFontFamily)) {
              cssRule.value =
                cssQuoteIfNecessary(subsetFontFamily) + ', ' + cssRule.value;
              changesMade = true;
            }
          }
        }
