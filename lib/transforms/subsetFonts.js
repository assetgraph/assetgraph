...
const getFontFaceForFontUsage = memoizeSync(
  (assetGraph, fontUsage) => {
    const subsets = fontOrder
      ...
      .map(format => {
        ...
        const nameProps = ['font-family', 'font-weight', 'font-style'].map(
          prop => fontUsage.props[prop]
        );
        ...
        fontAsset.baseName = `${fileNamePreFix}-${fontAsset.md5Hex.slice(
          0,
          10
        )}`;
        fontAsset.extension = `.${fontAsset.contentType.split('/').pop()}`;
    ...
    const src = subsets
      .map(subset => `url(${subset.font.fileName}) format(${subset.format})`)
      .join(', ');
  ...
  font-family: ${cssQuoteIfNecessary(`${family}__subset`)};
  font-weight: ${weight};
  font-style: ${style};
  font-stretch: ${stretch};
