...
const getFontFaceForFontUsage = memoizeSync(
  fontUsage => {
    ...
    resultString.push(`  unicode-range: ${unicodeRange(fontUsage.codepoints.used)};`);
    ...
    for (const htmlAssetTextWithProps of htmlAssetTextsWithProps) {
      for (const fontUsage of htmlAssetTextWithProps.fontUsages) {
        ...
        try {
          ...
          // FIXME: Should this emit a warning or just swallow?
          // A warning is already emitted in the subsetting code
          continue;
        }
