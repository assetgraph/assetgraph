...
//   OR the size of the image is less than or equal to 'sizeThreshold' AND the image doesn't have any other incoming relations
//
            ...
            assetGraph.findRelations({type: 'HtmlStyle', from: htmlAsset}).forEach(function (htmlStyle) {
                ...
                cssImages.forEach(function (cssImage) {
                    if (cssImage.cssRule.style.getPropertyValue('-one-image-inline') ||
                        (sizeThreshold >= 0 && cssImage.to.rawSrc.length <= sizeThreshold) && assetGraph.findRelations({to: cssImage.to}).length === 1) {
