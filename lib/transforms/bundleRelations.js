...
const combineJavaScriptAsts = require('../combineJavaScriptAsts');
const createConditionalCommentHousekeeper = require('../createConditionalCommentHousekeeper');
...
module.exports = (
  ...
  return function bundleRelations(assetGraph) {
    ...
    bundleStrategyByName.oneBundlePerIncludingAsset = () => {
      const getOpenConditionalComments = createConditionalCommentHousekeeper();
      const bundleAssets = [];
      ...
      for (const includingAssetId of Object.keys(relationsByIncludingAsset)) {
        ...
        for (const relationType of _.uniq(relationsToBundle.map(r => r.type))) {
          let currentBundle = [];
