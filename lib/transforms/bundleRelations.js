...
module.exports = function (queryObj, options) {
    ...
    return function bundleRelations(assetGraph) {
        function getDiscriminatorForRelation(relation) {
            ...
            if (relation.type === 'HtmlScript') {
                discriminatorFragments.push(relation.isLoaded);
            }
        ...
        function makeBundle(assetsToBundle) {
            ...
            if (type === 'JavaScript') {
                ...
                assetsToBundle.forEach(function (asset, i) {
                    ...
                    if (asset.strict) {
                        var warning = new Error('Global "use strict"-directive may cause leakage in bundling.');
                        warning.asset = asset;
                        assetGraph.emit('warn', warning);

                        /* Possible future alternative:
                        // Wrap the whole thing in an IIFE
                        // asset text needs to be rewritten so all decelarations are instead assignments to properties on root
                        parseTree = uglifyJs.parse('(function (root) {' + asset.text + '}(this));');
                        */
                    }

                    // Append asset to new bundle
                    Array.prototype.push.apply(constructorOptions.parseTree.body, parseTree.body);
