    ...
    return function bundleRelations(assetGraph) {
        function getDiscriminatorForRelation(relation) {
            ...
            if (relation.type === 'HtmlScript') {
                ...
                // Check relation.node.ownerDocument.documentElement,
                // otherwise node.matchesSelector throws an error for document fragments
                if (relation.node.ownerDocument.documentElement && relation.node.matchesSelector('head script')) {
