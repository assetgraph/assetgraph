...
function makeBundle(assetGraph, assetsToBundle) {
    ...
    if (type === 'JavaScript') {
        ...
        parseTrees.forEach(function (parseTree) {
            parseTree.parentStyleSheet = constructorOptions.parseTree;
            Array.prototype.push.apply(constructorOptions.parseTree.cssRules, parseTree.cssRules);
        });
    }
...
bundleStrategyByName.oneBundlePerIncludingAsset = function (assetGraph, queryObj) {
    ...
    Object.keys(relationsByIncludingAsset).forEach(function (includingAssetId) {
        ...
        _.unique(_.pluck(relationsToBundle, 'type')).forEach(function (relationType) {
            var currentBundle = [],
                bundleDiscriminator, // Only used for HtmlStyle (the media attribute)
