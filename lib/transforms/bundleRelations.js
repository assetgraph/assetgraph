...
function getParsedDataAssetGraphConditionalsFromHtmlElement(node) {
                ...
                if (!seenReferringAssets[incomingRelation.from.id]) {
                    ...
                    if (incomingRelation.from.type === 'Html') {
                        ...
                        incomingRelations.forEach(function (relation) {
                            ...
                            var conditions = getParsedDataAssetGraphConditionalsFromHtmlElement(relation.node);
                            if (conditions) {
                        ...
                        if (conditionNames.length > 0) {
                            ...
                            bundleRelation.node.setAttribute('data-assetgraph-conditions', Object.keys(combinedAssetGraphConditions).map(function (key) {
                                return (/[^a-z]/.test(key) ? singleQuoteString(key) : key) + ': ' + (Array.isArray(combinedAssetGraphConditions[key]) ? '[' + combinedAssetGraphConditions[key].map(singleQuoteString).join(', ') + ']' : singleQuoteString(combinedAssetGraphConditions[key]));
                            }).join(', '));
                        }
                    }
