...
module.exports = (queryObj, options) => {
    ...
    return function bundleRelations(assetGraph) {
        ...
        function makeBundle(assetsToBundle, incomingType) {
            ...
            const outgoingRelations = assetGraph.findRelations({from: assetsToBundle});
            ...
            const bundleAsset = assetGraph.add({
                type,
                url: urlTools.resolveUrl(assetGraph.root, 'bundle-' + bundleAsset.id + bundleAsset.extension),
                parseTree,
                outgoingRelations,
