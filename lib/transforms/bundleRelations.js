    ...
    return function bundleRelations(assetGraph) {
        ...
        function makeBundle(assetsToBundle, incomingType) {
            ...
            const bundleAsset = assetGraph.addAsset({
                ...
                parseTree,
                baseName: 'bundle',
                outgoingRelations: assetGraph.findRelations({from: assetsToBundle}),
            ...
            for (const asset of assetsToBundle) {
                if (assetGraph.findRelations({to: asset, type: assetGraph.query.not('SourceMapFile')}).length === 0) {
        ...
        bundleStrategyByName.oneBundlePerIncludingAsset = () => {
            ...
            for (const includingAssetId of Object.keys(relationsByIncludingAsset)) {
                ...
                for (const relationType of _.uniq(_.map(relationsToBundle, 'type'))) {
                    ...
                    for (const outgoingRelation of assetGraph.findRelations(assetGraph.constructor.query.or({type: relationType}, {type: 'HtmlConditionalComment'}))) {
                        if (outgoingRelation.type === 'HtmlConditionalComment') {
                            ...
                            if (assetGraph.findRelations({to: outgoingRelation.to, type: assetGraph.query.not('SourceMapFile')}).length > 1) {
        ...
        bundleStrategyByName.sharedBundles = () => {
            ...
            for (const includingAsset of _.values(seenIncludingAssets)) {
                ...
                for (const relationType of _.uniq(_.map(relationsToBundle, 'type'))) {
                    const outgoingRelations = assetGraph.findRelations({from: includingAsset, type: [relationType, 'HtmlConditionalComment']});
                    let previousBundle;
                    let bundleDiscriminator;
