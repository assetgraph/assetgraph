        ...
        function makeBundle(assetsToBundle, incomingType) {
            ...
            if (type === 'JavaScript') {
                ...
                for (const asset of assetsToBundle)  {
                    for (const relation of assetGraph.findRelations({from: asset, type: 'JavaScriptSourceMappingUrl'})) {
            ...
            const bundleAsset = assetGraph.add({
                type,
                parseTree,
                baseName: 'bundle',
            ...
            for (const incomingRelation of (incomingType === 'HtmlScript' ? incomingRelations.slice().reverse() : incomingRelations)) {
                if (!seenReferringAssets.has(incomingRelation.from)) {
                    ...
                    // FIXME: Could be directly supported by the HtmlScript relation?
                    if (incomingType === 'HtmlScript') {
