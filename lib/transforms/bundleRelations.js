    ...
    return function bundleRelations(assetGraph) {
        ...
        function makeBundle(assetsToBundle, incomingType) {
            ...
            const bundleAsset = assetGraph.addAsset({
                ...
                parseTree,
                baseName: 'bundle',
                outgoingRelations: assetGraph.findRelations({from: {$in: assetsToBundle}}),
            ...
            const incomingRelations = assetGraph.findRelations({type: incomingType, to: {$in: assetsToBundle}});
            ...
            const bundleRelationHrefType = incomingRelations.some(
                incomingRelation => !['relative', 'inline'].includes(incomingRelation.hrefType)
            ...
            for (const asset of assetsToBundle) {
                if (assetGraph.findRelations({to: asset, type: {$not: 'SourceMapFile'}}).length === 0) {
                    for (const sourceMapFileRelation of assetGraph.findRelations({to: asset, type: 'SourceMapFile'})) {
        ...
        bundleStrategyByName.oneBundlePerIncludingAsset = () => {
            ...
            for (const includingAssetId of Object.keys(relationsByIncludingAsset)) {
                ...
                for (const relationType of _.uniq(_.map(relationsToBundle, 'type'))) {
                    ...
                    for (const outgoingRelation of assetGraph.findRelations({type: {$in: [relationType, 'HtmlConditionalComment']}})) {
                        if (outgoingRelation.type === 'HtmlConditionalComment') {
                            ...
                            if (assetGraph.findRelations({to: outgoingRelation.to, type: {$not: 'SourceMapFile'}}).length > 1) {
        ...
        bundleStrategyByName.sharedBundles = () => {
            ...
            for (const includingAsset of _.values(seenIncludingAssets)) {
                ...
                for (const relationType of _.uniq(_.map(relationsToBundle, 'type'))) {
                    const outgoingRelations = assetGraph.findRelations({from: includingAsset, type: {$in: [relationType, 'HtmlConditionalComment']}});
                    let previousBundle;
                    let bundleDiscriminator;
