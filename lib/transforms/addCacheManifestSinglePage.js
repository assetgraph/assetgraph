...
module.exports = function (queryObj) {
    return function addCacheManifestSinglePage(err, assetGraph, cb) {
        if (err) {
            throw err;
        }
        assetGraph.findAssets(queryObj).forEach(function (htmlAsset) {
            ...
            if (existingManifestRelations.length === 1) {
                ...
                if (htmlAsset.url) {
                    manifest.url = htmlAsset.url + '.manifest';
                } else {
                    manifest.url = assetGraph.resolver.root + manifest.id + '.' + manifest.defaultExtension;
                }
