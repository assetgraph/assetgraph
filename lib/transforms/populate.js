        ...
        function processAsset(asset) {
            return asset.load().then(function () {
                return Promise.map(asset.externalRelations, function (originalRelation) {
                    var targetAsset;
                    try {
                        targetAsset = assetGraph.createAsset(originalRelation.to);
                    } catch (err) {
                        assetGraph.emit('warn', err);
                    }
                    if (targetAsset && followRelationsMatcher(targetAsset, assetGraph)) {
                        if (!targetAsset.assetGraph) {
                            assetGraph.addAsset(targetAsset);
                        }
                        if (!stopAssetsMatcher(targetAsset)) {
                            assetQueue.push(targetAsset);
                        }
                    }
                });
            }, function (err) {
