            ...
            step(
                ...
                error.passToFunction(cb, function (resolvedAssetConfigArrays) {
                    ...
                    function initializeAndAddRelation(relation) {
                        relation.to = new assets[relation.assetConfig.type](relation.assetConfig);
                        assetGraph.addAsset(relation.to);
                        if (initializedRelations.length) {
                            assetGraph.addRelation(relation, 'after', initializedRelations[initializedRelations.length - 1]);
                        } else {
...
console.log({originalRelation: originalRelation});
console.log({assetConfig: resolvedAssetConfigs});
