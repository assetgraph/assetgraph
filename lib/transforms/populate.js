                ...
                async.each(asset.externalRelations, function (originalRelation, cb) {
                    originalRelation.resolve(function (err, resolvedAssetConfig) {
                        if (err) {
                            assetGraph.emit('warn', err);
                            cb();
                        } else if (resolvedAssetConfig.to.isResolved && followRelationsMatcher(resolvedAssetConfig, assetGraph)) {
                            assetGraph.ensureAssetConfigHasType(resolvedAssetConfig.to, function (err) {
                                if (err) {
