                    ...
                    asset.load(function (err) {
                        ...
                        (function gatherExternalRelations(asset) {
                            ...
                            asset.outgoingRelations.forEach(function (outgoingRelation) {
                                ...
                                } else if (outgoingRelation.type === 'HttpRedirect' && outgoingRelation.to.isRedirect) {
                                    // Special case: 302/301 redirect to the exact same URL should actually try loading it again
                                    var previousRedirects = assetGraph.findAssets({
                                        url: outgoingRelation.to.url,
                                        isRedirect: true
                                    });

                                    if (previousRedirects.length < 2) {
                                        outgoingRelation.to = { url: outgoingRelation.to.url };
                                        externalRelations.push(outgoingRelation);
                                    } else {
                                        var error = new Error('Infinite redirect loop detected.');
                                        error.asset = outgoingRelation.from;
