...
module.exports = options => {
  ...
  return async function populate(assetGraph) {
    ...
    while (assetQueue.length > 0) {
      await Promise.all(
        assetQueue.splice(0, maxWaitingCallbacks).map(processAsset)
      );
    }
  };
};
