  ...
  return async function bundleWebpack(assetGraph) {
    ...
    try {
      ...
      if (options.configPath) {
        throw new Error(
          'Webpack config path given (' +
            options.configPath +
            '), but webpack itself could not be found. Please install it in your project: ' +
            e.stack
        );
      }
    }
    ...
    try {
      ...
      if (webpack) {
        assetGraph.info(
          new Error(
            'Webpack is installed, but could not load the webpack config (tried ' +
              triedConfigPaths.join(' ') +
              '): ' +
              err.message
          )
        );
    ...
    for (const relation of assetGraph.findRelations({
      ...
      from: {
        url: {
          $in: assetInfos.map(function(url) {
            return url.url;
          })
        }
      }
