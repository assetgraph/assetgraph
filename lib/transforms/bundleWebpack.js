...
module.exports = function (queryObj, options) {
    ...
    return function bundleWebpack(assetGraph) {
        ...
        var config;
        try {
            config = require.main.require(configPath);
        } catch (err) {
            ...
            var readPkgUpResult = readPkgUp.sync();
            ...
            if (typeof options.configPath === 'undefined') {
                assetGraph.emit('info', new Error('Could not load webpack config ' + configPath + ', but webpack is installed: ' + err.message));
                return;
            } else {
        ...
        return Promise.fromNode(function (cb) {
            ...
            return assetGraph.loadAssets(urls.map(function (url) {
                ...
                var bundleAsset = assetGraph.findAssets({url: urlTools.fsFilePathToFileUrl(pathModule.resolve(basePath, config.output.filename))})[0];
