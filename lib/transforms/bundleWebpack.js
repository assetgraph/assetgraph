    ...
    return async function bundleWebpack(assetGraph) {
        ...
        try {
            ...
            if (options.configPath) {
                throw new Error('Webpack config path given (' + options.configPath + '), but webpack itself could not be found. Please install it in your project: ' + e.stack);
            }
        }
        ...
        try {
            ...
            if (webpack) {
                assetGraph.info(new Error('Webpack is installed, but could not load the webpack config (tried ' + triedConfigPaths.join(' ') + '): ' + err.message));
            }
            return;
        }
        ...
        config.output.devtoolModuleFilenameTemplate = config.output.devtoolModuleFilenameTemplate || 'file://[absolute-resource-path]';
        ...
        for (const plugin of plugins) {
            if (plugin.constructor.name === 'UglifyJsPlugin' && (plugin.options.mangle || plugin.options.compress)) {
                ...
                assetGraph.info(new Error('UglifyJsPlugin detected, turning off mangling and compression so outgoing relations can be detected. This is fine for assetgraph-builder, which will run UglifyJs later anyway'));
            }
        }
        ...
        if (!hasHtmlWebpackPlugin) {
            ...
            const potentialBundlePaths = assetGraph.findRelations({
                ...
                return urlTools.fileUrlToFsPath(assetGraph.resolveUrl(relation.baseUrl, relation.to.url));
            });
            ...
            if (config.entry && typeof config.entry === 'object' && !Array.isArray(config.entry)) {
                config.entry = Object.keys(config.entry).reduce((entryPointsToBeBuilt, entryPointName) => {
                    if (potentialBundlePaths.includes(createAbsolutePath(entryPointName))) {
        ...
        if (existingAssetsWereReplaced) {
            assetGraph.info(new Error('Please remove ' + config.output.path + ' before building with assetgraph to speed up the process'));
        }

        await assetGraph.loadAssets(assetInfos.map(({ url, output }) => ({
            url,
        ...
        for (const relation of assetGraph.findRelations({
            ...
            from: { url: { $in: assetInfos.map(function (url) { return url.url; }) } }
        })) {
        ...
        for (const asset of assetGraph.findAssets({ type: 'JavaScript', url: {$in: assetInfos.map(assetInfo => assetInfo.url)}})) {
