var _ = require('underscore'),
    ...
    // JavaScriptRequireJsCommonJsCompatibilityRequire relations are assumed to be doubled by JavaScriptAmdDefine relations
    // (run convertJavaScriptRequireJsCommonJsCompatibilityRequireToJavaScriptAmdDefine first)
    requireJsRelationTypes = ['JavaScriptAmdRequire', 'JavaScriptAmdDefine', 'JavaScriptShimRequire'];
    ...
    return function flattenRequireJs(assetGraph) {
        ...
        assetGraph.findAssets(_.extend({type: 'Html'}, queryObj)).forEach(function (htmlAsset) {
            ...
            assetGraph.findRelations({from: htmlAsset, type: ['HtmlRequireJsMain', 'HtmlScript']}).forEach(function (startRelation) {
                ...
                assetGraph.eachAssetPostOrder(startRelation, {type: requireJsRelationTypes, to: {type: 'JavaScript'}}, function (asset) {
                    if (!asset.isLoaded) {
                        return;
                    }
