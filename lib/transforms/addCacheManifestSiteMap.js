...
exports.addCacheManifestSiteMap = function (queryObj) {
    return function addCacheManifestSiteMap(err, assetGraph, cb) {
        var allManifests = [],
            sharedManifest;
        assetGraph.findAssets(queryObj).forEach(function (htmlAsset) {
            ...
            if (existingManifestRelations.length > 1) {
                ...
                if (!sharedManifest) {
                    ...
                    sharedManifest.url = assetGraph.resolver.root + sharedManifest.id + '.' + sharedManifest.defaultExtension;
        ...
        assetGraph.findAssets({url: query.defined}).forEach(function (asset) {
