...
try{
        var allManifests = [],
            sharedManifest;
        assetGraph.findAssets('isInitial', true).filter(function (asset) {return asset.type === 'HTML';}).forEach(function (htmlAsset) {
            var existingManifestRelations = assetGraph.findRelations('from', htmlAsset).filter(function (relation) {
                return relation.type === 'HTMLCacheManifest';
            });
        ...
        assetGraph.assets.forEach(function (asset) {
            if (asset.url) {
                allManifests.forEach(function (manifest) {
                    if (asset !== manifest && !assetGraph.findRelations('to', asset).some(function (relation) {return relation.from === manifest;})) {
