...
module.exports = options => {
    ...
    return function inlineCriticalCss(assetGraph) {
        ...
        const pages = assetGraph
            ...
            .filter(page => {
                const relations = assetGraph
                    .findRelations({ type: 'HtmlStyle', from: page, to: { isInline: false }});
        ...
        for (const page of pages) {
            ...
            const cssRelations = assetGraph.findRelations({ type: 'HtmlStyle', from: page, to: { isInline: false }});
            ...
            if (criticalParseTree.nodes.length > 0) {
                if (page.parseTree.querySelector('html')) {
                    // FIXME: Implement support for 'firstInHead'
                    const criticalRelation = page.addRelation({
                        type: 'HtmlStyle',
                        to: { type: 'Css', parseTree: criticalParseTree }
                    }, 'first');
                    criticalRelation.attachToHead(page, 'last');
                } else {
                    assetGraph.warn(new Error(`Cannot inline the critical CSS in ${page.urlOrDescription}, the page does not have an <html> element`));
                }
            }
        }
    };
