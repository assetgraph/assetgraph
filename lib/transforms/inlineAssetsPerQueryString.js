...
const defaultQueryObj = {isInline: false, query: {inline: value => value !== undefined}};
...
module.exports = (queryObj, {minimumIeVersion = Infinity} = {}) => {
    ...
    return function inlineAssetsPerQueryString(assetGraph) {
        ...
        function getMinimumIeVersionUsage(asset, stack = []) {
            ...
            stack.push(asset);
            const minimumIeVersion = Math.min(...asset.incomingRelations.filter(
                incomingRelation => !stack.includes(incomingRelation.from)
            ).map(
                incomingRelation => {
                    let matchCondition;
                    if (incomingRelation.type === 'HtmlConditionalComment') {
                        matchCondition = incomingRelation.condition.match(/^(gte?|lte?)\s+IE\s+(\d+)$/i);
                    } else if (incomingRelation.conditionalComments && incomingRelation.conditionalComments.length > 0) {
                        matchCondition = incomingRelation.conditionalComments[0].nodeValue.match(/^\[if\s+(gte?|lte?)\s+IE\s+(\d+)\s*\]\s*>\s*<\s*!\s*$/i);
                    }
                    if (matchCondition) {
                        if (matchCondition[1].substr(0, 2) === 'lt') {
                            return 1;
                        } else {
                            return parseInt(matchCondition[2], 10) + (matchCondition[1].toLowerCase() === 'gt' ? 1 : 0);
                        }
                    } else {
        ...
        for (const asset of assetGraph.findAssets(queryObj)) {
            ...
            if (!inlineParamValue || /^(?:true|on|yes|1)$/i.test(inlineParamValue)) {
                ...
                const minimumIeVersionUsage = minimumIeVersion !== Infinity && getMinimumIeVersionUsage(asset);
                for (const incomingRelation of asset.incomingRelations) {
                    ...
                    if (minimumIeVersion !== undefined) {
                        if (minimumIeVersion <= 7 && minimumIeVersionUsage <= 7) {
                            assetGraph.warn(new Error(`Inlining ${assetUrl} causes the asset not to be parsed by IE7 and below`));
                        } else if (incomingRelation.href.length > 32768 && minimumIeVersion <= 8 && minimumIeVersionUsage <= 8) {
                            assetGraph.warn(new Error(`Inlining ${assetUrl} causes the asset not to be parsed by IE8 and below because the data url is > 32 KB`));
                        }
                    }
                }
            } else {
                try {
                    ...
                    if (/already exists in the graph, cannot update url/.test(err.message)) {
