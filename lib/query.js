...
query.queryObjToMatcherFunction = function (queryObj) {
    if (typeof queryObj === 'function') {
        ...
        if (queryObj.length === 0) {
            ...
            return query.queryObjToMatcherFunction(queryObj[0]);
        } else {
            return query.or.apply(query.or, queryObj);
        }
    }
...
query.queryAssetGraph = function (assetGraph, objType, queryObj, candidateArrayOrSet) {
    queryObj = queryObj || {};
    var filters = [];
    if (typeof queryObj === 'function') {
    ...
    candidateArrayOrSet = candidateArrayOrSet || assetGraph['_' + objType + 's'];

    if (filters.length === 0) {
        if (Array.isArray(candidateArrayOrSet)) {
            return candidateArrayOrSet;
        } else {
            return Array.from(candidateArrayOrSet);
        }
    }
    const result = [];
    for (const item of candidateArrayOrSet) {
...
query.or = function () { // ...
    ...
    var valueMatchers = Array.from(arguments).map(query.createValueMatcher);
