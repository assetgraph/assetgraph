...
query.queryAssetGraph = function (assetGraph, objType, queryObj) {
        ...
        if (objType === 'asset' && (fieldName === 'incoming' || fieldName === 'outgoing')) {
            ...
            if (query.indices[objType].indexOf(fieldName) !== -1) {
                if (_.isArray(fieldQueryObj) && fieldQueryObj.every(function (item) {
                    // Don't attempt index lookups if the array contains a matcher function or something even worse:
                    return item.isAsset || item.isRelation || !/^(?:string|boolean|number)$/.test(typeof item);
                })) {
                    indexLookup = [];
    ...
    if (shortestIndexLookup && numFields === 1) {
        result = shortestIndexLookup;
    } else {
        result = shortestIndexLookup || assetGraph['_' + objType + 's'];
