...
class AssetGraph extends EventEmitter {
     ...
     */
    add(asset, fromUrl, incomingRelation) {
        if (Array.isArray(asset) || (typeof asset === 'string' && !/^[a-zA-Z-\+]+:/.test(asset) && asset.includes('*'))) {
            throw new Error('AssetGraph#add does not accept an array or glob patterns, try AssetGraph#addAll');
        }
        if (!asset.isAsset) {
            asset = this.createAsset(asset, fromUrl, incomingRelation);
        }
    ...
    /**
     * assetGraph.addAll(assets)
     * =========================
     *
     * Add an asset to the graph.
     *
     * @param {Asset|String|Object|Array} The asset (or spec) to add
     * @return {Asset[]} The assets instances that were added
     */
    addAll(asset, fromUrl, incomingRelation) {
        if (Array.isArray(asset)) {
            return _.flatten(asset.map(asset => this.add(asset, fromUrl, incomingRelation)));
            ...
            return _.flatten(glob
                ...
                .map(path => this.add(encodeURI(`file://${path}`), fromUrl, incomingRelation)));
        } else {
            return [ this.add(asset, fromUrl, incomingRelation) ];
        }
    }
    ...
    createAsset(assetConfig, fromUrl, incomingRelation) {
        if (typeof assetConfig === 'string') {
            if (/^[a-zA-Z-\+]+:/.test(assetConfig)) {
                assetConfig = { url: assetConfig };
            } else {
                assetConfig = { url: this.resolveUrl(fromUrl || this.root, encodeURI(assetConfig)) };
            }
        }
        if (typeof assetConfig.url === 'string') {
            if (!/^[a-zA-Z-\+]+:/.test(assetConfig.url)) {
                assetConfig.url = this.resolveUrl(fromUrl || this.root, assetConfig.url);
            }
        ...
        if (typeof assetConfig.url === 'string') {
            if (/^data:/.test(assetConfig.url)) {
                ...
                if (existingAssets.length > 0) {
                    ...
                    const existingAsset = existingAssets[existingAssets.length - 1];
                    existingAsset.init(assetConfig);
