...
function AssetGraph(config) {
    ...
    } else if (!/^[^:]+:/.test(this.root)) { // No protocol, assume local file system path
...
AssetGraph.traversal = require('./traversal');
...
_.extend(AssetGraph.prototype, {
    resolveAssetConfig: function (obj, fromUrl, cb) {
        ...
        if (obj.isAsset) {
            ...
            if (resolver) {
                resolver(obj, fromUrl, error.passToFunction(cb, function (resolvedAssetConfig) {
    ...
    findRelations: function (queryObj) {
        return AssetGraph.query.queryAssetGraph(this, 'relation', queryObj);
    },
    ...
    _refreshInlineAssets: function (asset, cb) {
        var that = this;
        seq.ap(that.findRelations({from: asset, to: {url: AssetGraph.query.isUndefined}}))
    ...
    addRelation: function (relation, position, adjacentRelation) { // position and adjacentRelation are optional
        ...
        if (!relation.to || !relation.to.isAsset) {
            throw new Error("AssetGraph.addRelation: 'to' property of relation is not an asset: " + relation.to);
        }
        position = position || 'last';
        if (position === 'last') {
    ...
    updateRelationTarget: function (relation, newTarget) {
        ...
        var i = this._indices.relation.to[relation.to.id].indexOf(relation);
        if (i === -1) {
            throw new Error("assetGraph.updateRelationTarget: Relation not in graph: " + relation);
        }
        this._indices.relation.to[relation.to.id].splice(i, 1);
        relation.to = newTarget;
        (this._indices.relation.to[relation.to.id] = this._indices.relation.to[relation.to.id] || []).push(relation),
        this.refreshRelationUrl(relation);
    },
    ...
    runTransform: function (transform, cb) {
        var that = this,
            startTime = new Date(),
            done = error.passToFunction(cb, function () {
