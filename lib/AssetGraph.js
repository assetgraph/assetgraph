...
AssetGraph.prototype = {
     ...
     */
    findBaseAsset: function (relation) {
        ...
        while (queue.length) {
            ...
            if (baseAssetMatcher(asset)) {
                return asset;
            }
            seenAssets[asset.id] = true;

            // Add all yet-unseen assets reachable via incoming relations to the end of the queue:
            Array.prototype.push.apply(queue, _.pluck(this.findRelations({to: asset, from: function (asset) {
                return !(asset.id in seenAssets);
            }}), 'from'));
        }
        throw new Error("AssetGraph.findBaseAsset: Cannot base asset of " + relation + " matching " + util.inspect(relation.baseAssetQuery));
    },
    ...
    setAssetUrl: function (asset, url) { // Implicitly un-inlines asset
        ...
        this.findRelations({to: asset}).forEach(function (incomingRelation) {
            incomingRelation._setRawUrlString(fileUtils.buildRelativeUrl(this.findBaseAsset(incomingRelation).url, url));
    ...
    attachAndAddRelation: function (relation, position, adjacentRelation) {
        ...
        if (relation.to.url) {
            relation._setRawUrlString(fileUtils.buildRelativeUrl(this.findBaseAsset(relation).url, relation.to.url));
        }
        this.markAssetDirty(relation.from);
    },
