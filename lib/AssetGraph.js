...
function AssetGraph(config) {
    ...
    this.resolver = new AssetConfigResolver();
    this.assets = [];
    this.relations = [];
    this.indices = {};
    this.assetsById = {};
...
AssetGraph.prototype = {
    ...
    _refreshInlineRelations: function (asset, cb) {
        // FIXME: Be consistent about setting the asset.isDirty flag so that a lot of work can be avoided:
        var inlineRelations = this.findRelations({from: asset, to: {url: query.undefined}});
        if (!inlineRelations.length) {
            return cb();
        }
        var that = this;
        step(
            function () {
                ...
                inlineRelations.forEach(function (relation) {
                    var callback = group();
