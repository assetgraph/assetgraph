...
class AssetGraph extends EventEmitter {
    ...
    add(asset, fromUrl) {
        ...
        if (!asset.isAsset) {
            asset = this.createAsset(asset, fromUrl);
        }
        if (!this.idIndex[asset.id]) {
            ...
            asset.populate(fromUrl);
        }
        return asset;
    }

    /**
     * assetGraph.addAll(asset(s))
     * ===========================
     *
     * Add an asset to the graph.
     *
     ...
     * @return {Asset[]} The assets instances that were added
     */
    addAll(asset, fromUrl) {
        if (Array.isArray(asset)) {
            return _.flatten(asset.map(asset => this.add(asset, fromUrl)));
            ...
            return _.flatten(glob
                ...
                .map(path => this.add(encodeURI(`file://${path}`), fromUrl)));
        } else {
            return [ this.add(asset, fromUrl) ];
        }
    }
