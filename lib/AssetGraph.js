...
AssetGraph.prototype = {
    ...
    _createMatcherFunction: function (queryObj) {
        if (typeof queryObj === 'function') {
            return queryObj;
        }
        var matchers = _.map(queryObj, function (query, fieldName) {
            if (typeof query === 'function') {
                return function (obj) {
                    return query(obj[fieldName]);
                };
            } else if (_.isArray(query)) {
                return function (obj) {
                    return query.indexOf(obj[fieldName]) !== -1;
                };
            } else if (typeof query === 'object' && !query.isAsset && !query.isRelation) {
                var keys = _.keys(query);
                return function (obj) {
                    for (var i = 0 ; i < keys.length ; i += 1) {
                        var key = keys[i];
                        if (obj[fieldName][key] !== query[key]) {
                            return false;
                        }
                    }
                    return true;
                ...
                return function (obj) {
                    return obj[fieldName] === query;
                };
            }
        });
        if (matchers.length === 0) {
            throw new Error("AssetGraph.getMatcherFunction: No keys in query object");
    ...
    _query: function (type, queryObj) {
        ...
        if (shortestIndexLookup && numFields === 1) {
            ...
            return (shortestIndexLookup || that[type + 's']).filter(this._createMatcherFunction(queryObj));
        }
    },
