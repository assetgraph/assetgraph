...
_.extend(AssetGraph.prototype, {
    ...
    resolveAssetConfig: function (obj, fromUrl, cb) {
        ...
        if (obj.isAsset) {
            ...
            if (obj.url.indexOf(':') !== -1) {
                ...
                if (/^\/(?:[^\/]|$)/.test(obj.url) && /^file:/.test(fromUrl) && /^file:/.test(that.root)) {
                    obj.url = urlTools.resolveUrl(that.root, obj.url.substr(1));
                } else {
                    obj.url = urlTools.resolveUrl(fromUrl, obj.url);
                }
                that.resolveAssetConfig(obj, fromUrl, cb);
            }
        } else {
            cb(new Error("AssetGraph.resolveAssetConfig: Cannot resolve asset config (no url)."));
        }
    },
    ...
    inlineRelation: function (relation) {
        var incomingRelations = this.findRelations({to: relation.to});
        ...
        incomingRelations[0]._inline();
    },

    _refreshInlineRelations: function (asset) {
        this.findRelations({from: asset, to: {url: AssetGraph.query.isUndefined}}).forEach(function (relation) {
            this._refreshInlineRelations(relation.to);
    ...
    getAssetText: function (asset) {
        this._refreshInlineRelations(asset);
        return asset.text;
    },

    getAssetRawSrc: function (asset) {
        this._refreshInlineRelations(asset);
        return asset.rawSrc;
    },
    ...
    populateRelationsToExistingAssets: function (clonedAsset, originalAsset) {
        var cloneRelations = clonedAsset.getRelations(),
            originalRelations = this.findRelations({from: originalAsset});
        if (cloneRelations.length !== originalRelations.length) {
        ...
        cloneRelations.forEach(function (cloneRelation, i) {
            var originalRelation = originalRelations[i];
            if (originalRelation.type !== cloneRelation.type) {
                throw new Error("AssetGraph.cloneAsset assertion error: Relation types don't match!");
            }
            cloneRelation.isResolved = true;
            if (originalRelation.to.url) {
                cloneRelation.to = originalRelation.to;
                this.addRelation(cloneRelation);
            } else {
                var clonedInlineAsset = this.cloneAsset(originalRelation.to);
                cloneRelation.to = clonedInlineAsset;
                this.addRelation(cloneRelation);
                this.populateRelationsToExistingAssets(clonedInlineAsset, originalRelation.to);
            }
        }, this);
    }
});
