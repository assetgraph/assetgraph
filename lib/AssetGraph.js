...
class AssetGraph extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (typeof options.canonicalRoot !== 'undefined') {
      ...
      if (
        !/^(?:https?:)?\/\/(:?[a-z0-9](?:[-a-z0-9]*[a-z0-9])?\.)+[a-z][-a-z]*[a-z]/i.test(
  ...
  removeAsset(asset) {
    ...
    if (asset._outgoingRelations) {
      ...
      for (const outgoingRelation of outgoingRelations) {
        if (outgoingRelation.to.isAsset && outgoingRelation.to.isInline && outgoingRelation.to !== outgoingRelation.from) {
          // Remove inline asset
    ...
    for (const incomingRelation of asset.incomingRelations) {
      if (incomingRelation.from !== incomingRelation.to) {
        this.warn(
          new Error(
