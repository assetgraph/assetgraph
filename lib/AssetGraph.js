...
class AssetGraph extends EventEmitter {
     ...
     * @return {Asset[]} The assets instances that were added
     */
    addAll(asset, incomingRelation) {
        if (Array.isArray(asset)) {
            return _.flatten(asset.map(asset => this.add(asset, incomingRelation)));
            ...
            return _.flatten(glob
                ...
                .map(path => this.add(encodeURI(`file://${path}`), incomingRelation)));
        } else {
            return [ this.add(asset, incomingRelation) ];
        }
    }
