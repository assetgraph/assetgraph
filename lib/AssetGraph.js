 ...
 *           path. Defaults to `file://<process.cwd()>/`.
 *           The purpose of the root option is to allow resolution of
 *           root-relative urls (eg. `<a href="/foo.html">`) from `file:`
 *           locations.
 *
 * Examples:
 *
 ...
 *         // => "file:///current/working/dir/relative/path/"
 *
...
_.extend(AssetGraph.prototype, {
    ...
    /**
     * attachAndAddRelation(relation, position[, adjacentRelation])
     * ============================================================
     *
    ...
    /**
     * detachAndRemoveRelation(relation)
     * =================================
     *
     ...
     * asset.
     *
     * @param {Relation} relation The relation that should have its
     * `href` refreshed.
     * @api public
     */
    ...
    /**
     * updateRelationTarget(relation, newTargetAsset)
     * ==============================================
     *
    ...
    /**
     * replaceAsset(oldAsset, newAsset)
     * ================================
     *
    ...
    /**
     * cloneAsset(asset[, incomingRelations])
     * ======================================
     *
     ...
     * @param {Array} incomingRelations (optional) Some incoming
    ...
    /**
     * populateRelationsToExistingAssets(asset)
     * ========================================
     *
    ...
    /**
     * getBaseAssetForRelation(relation)
     * =================================
     *
     ...
     *
     * @param {Relation} relation The relation to find the base asset
     * path for.
    ...
    /**
     * _addToIndices(obj, position[, adjacentObj])
     * ===========================================
     *
    ...
    /**
     * _lookupIndex(indexType, indexName, value)
     * =========================================
     *
     * Look up a value in a relation or asset index.
     *
     * @param indexType The index type, either "asset" or "relation"
     * @param indexName The index name, supported values: "type",
     * "from" (relations only), "to" (relations only), "isInitial"
     * (assets only).
     * @api private
     */
    ...
    /**
     * _markContainingAssetDirtyIfInline(asset)
     * ========================================
     *
     * Walk through the chain of containing assets and mark them
     * dirty. Does nothing if the asset isn't inline.
     *
     * @param {Asset} asset The asset.
     * @api private
     */
    _markContainingAssetDirtyIfInline: function (asset) {
        if (!asset || !asset.isAsset) {
            throw new Error("AssetGraph._markContainingAssetDirtyIfInline: Not an asset: ", asset);
        }
        if (asset.isInline) {
            var incomingRelations = this.findRelations({to: asset});
    ...
    /**
     * _findBaseAssetForRelation(relation)
     * ===================================
     *
     * Helper function for the getBaseAssetForRelation method.
     *
     * @param {Relation} relation The relation for which to find the
     * base asset path. Must be in the graph.
     * @return {Array} The complete path to the base asset:
     * [baseAsset, intermediateRelation, asset, ...]. For normal
     * non-inline assets that will just be [baseAsset].
     * @api private
     */
    ...
    /**
     * _updateUrlIndex(asset, newUrl, oldUrl)
     * ======================================
     *
     * Update the graph's url index for an asset.
     *
     * @param {Asset} asset The asset that had its url changed.
     * @param {String} newUrl (optional) The new url of the asset. A
     * falsy value means that the asset is now inline.
     * @param {String} oldUrl (optional) The old (previous) url of the
     * asset. A falsy value means that the asset was previously
     * inline.
     * @api private
     */
    ...
    /**
     * _registerBaseAssetPath(relation)
     * ================================
     *
    ...
    /**
     * _unregisterBaseAssetPath(relation)
     * ==================================
     *
