...
class AssetGraph extends EventEmitter {
     ...
     */
    add(assetConfig, incomingRelation) {
            ...
            if (!asset) {
                ...
                if (assetConfig instanceof this.Asset) {
                    ...
                    if (!incomingRelation && !asset.url) {
                        ...
                        let baseName = asset.baseName || asset.id;
                        let extension = asset.extension || asset.defaultExtension;
                        if (asset.fileName) {
                            extension = asset.extension;
                        }
                        do {
                            asset.url = this.root + baseName + (nextSuffixToTry ? '-' + nextSuffixToTry : '') + extension;
                            nextSuffixToTry += 1;
                        } while (this.findAssets({ url: asset.url }).length > 0);
                    }
                }
            }
        }
    ...
    addAll(asset) {
        if (Array.isArray(asset)) {
            return _.flatten(asset.map(asset => this.add(asset)));
            ...
            return _.flatten(glob
                ...
                .map(path => this.add(encodeURI(`file://${path}`))));
        } else {
