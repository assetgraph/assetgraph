...
AssetGraph.prototype = {
    ...
    _setRawUrlStringOfRelation: function (relation, url) {
        if (relation._getRawUrlString() !== url) {
            relation._setRawUrlString(url);
    ...
    inlineAsset: function (asset, cb) {
        ...
        affectedRelations.forEach(function (affectedRelation) {
            ...
            if (affectedRelation.to.url) {
                this._setRawUrlStringOfRelation(affectedRelation,
                                                fileUtils.buildRelativeUrl(this.getBaseAssetForRelation(affectedRelation).url, affectedRelation.to.url));
            }
        }, this);
        this.getAssetText(asset, error.passToFunction(cb, function (src) {
                ...
                that.findRelations({from: asset, to: {url: query.undefined}}).forEach(function (relation) {
                    ...
                    that._refreshInlineAssets(relation.to, error.passToFunction(callback, function () {
                        if (relation.to.isDirty) {
                            // Maybe pass the asset to _inline so it can decide if it wants it as text or Buffer (inline images etc.)
                            that.getAssetText(relation.to, error.passToFunction(callback, function (src) {
    ...
    setAssetUrl: function (asset, url) {
        ...
        [].concat(this._objInBaseAssetPaths[asset.id]).forEach(function (affectedRelation) {
            ...
            if (this.getBaseAssetForRelation(affectedRelation) === asset) {
                if (affectedRelation.to.url) {
                    this._setRawUrlStringOfRelation(affectedRelation,
                                                    fileUtils.buildRelativeUrl(asset.url, affectedRelation.to.url));
                }
            }
        }, this);
        this.findRelations({to: asset}).forEach(function (incomingRelation) {
            this._setRawUrlStringOfRelation(incomingRelation,
                                            fileUtils.buildRelativeUrl(this.getBaseAssetForRelation(incomingRelation).url, url));
        }, this);
    },
    ...
    attachAndAddRelation: function (relation, position, adjacentRelation) {
        ...
        if (relation.to.url) {
            this._setRawUrlStringOfRelation(relation, fileUtils.buildRelativeUrl(this.getBaseAssetForRelation(relation).url, relation.to.url));
        }
        this.markAssetDirty(relation.from);
    },
