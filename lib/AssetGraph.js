...
class AssetGraph extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    if (this.mountRoot) {
      // Make sure that this.mountRoot starts with a slash but does not end with one:
      if (!this.mountRoot.startsWith('/')) {
        this.mountRoot = `/${this.mountRoot}`;
      }
      this.mountRoot = this.mountRoot.replace(/\/?$/, '');
      if (
        this.canonicalRoot &&
        new URL(this.canonicalRoot).pathname !== `${this.mountRoot}/`
      ) {
        throw new Error(
          'When passing both canonicalRoot and mountRoot, the canonicalRoot must have the mountRoot its path component'
        );
      }
    } else {
      this.mountRoot = '';
  ...
  resolveUrl(fromUrl, url) {
    if (
      ...
      if (this.mountRoot && url.startsWith(this.mountRoot)) {
        url = url.substr(this.mountRoot.length);
      }
