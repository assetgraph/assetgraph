...
// Register relations for the assets that support pluggable relations
for (const type of ['Html', 'Xml', 'Svg', 'Atom', 'Rss', 'JavaScript']) {
  const relationsDir = pathModule.resolve(__dirname, 'relations', type);
  for (const fileName of fs.readdirSync(relationsDir)) {
    const Relation = require(pathModule.resolve(relationsDir, fileName));
    AssetGraph.registerRelation(Relation);
    AssetGraph[type].registerRelation(Relation);
  }
}

// Register the non-pluggable relations
for (const fileName of fs.readdirSync(
