...
_.extend(AssetGraph.prototype, {
    ...
    transform: function () { // ...
        var that = this,
            transforms = _.toArray(arguments).filter(function (transform) {
        ...
        function proceed(err) {
            ...
            if (currentTransform) {
                that.emit('transform', currentTransform, new Date().getTime() - startTime);
            }
            startTime = new Date();
            if (i < transforms.length) {
                currentTransform = transforms[i];
                i += 1;
                currentTransform(err || null, that, proceed);
            }
        }
        proceed();
    }
