...
const javaScriptRelationsDir = pathModule.resolve(
  __dirname,
  'relations',
  'JavaScript'
);
for (const fileName of fs.readdirSync(javaScriptRelationsDir)) {
  const Relation = require(pathModule.resolve(
    javaScriptRelationsDir,
    fileName
  ));
  AssetGraph.registerRelation(Relation);
  AssetGraph.JavaScript.registerRelation(Relation);
}

const htmlRelationsDir = pathModule.resolve(__dirname, 'relations', 'Html');
for (const fileName of fs.readdirSync(htmlRelationsDir)) {
  const Relation = require(pathModule.resolve(htmlRelationsDir, fileName));
  AssetGraph.registerRelation(Relation);
  AssetGraph.Html.registerRelation(Relation);
}

const svgRelationsDir = pathModule.resolve(__dirname, 'relations', 'Svg');
for (const fileName of fs.readdirSync(svgRelationsDir)) {
  const Relation = require(pathModule.resolve(svgRelationsDir, fileName));
  AssetGraph.registerRelation(Relation);
  AssetGraph.Svg.registerRelation(Relation);
}
