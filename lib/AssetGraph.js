...
AssetGraph.prototype = {
    ...
    // Only a sketch, this is too cumbersome to work with:
    cloneAssetAndOutgoingRelations: function (asset, cb) {
        var that = this;
        asset._clone(error.passToFunction(cb, function (assetClone) {
            assetClone.getOriginalRelations(error.passToFunction(cb, function (cloneRelations) {
                var existingRelations = that.findRelations({from: asset});
                if (cloneRelations.length !== existingRelations.length) {
                    cb(new Error("Not implemented yet"));
                } else {
                    for (var i = 0 ; i < existingRelations.length ; i += 1) {
                        cloneRelations[i].to = existingRelations[i].to;
                    }
                    cb(null, assetClone, cloneRelations);
                }
            }));
        }));
    },
