...
_.extend(AssetGraph.prototype, {
    ...
    populateRelationsToExistingAssets: function (clonedAsset) {
        clonedAsset.outgoingRelations.forEach(function (outgoingRelation) {
            if (outgoingRelation.href) {
                var targetUrl = urlTools.resolveUrl(this.getBaseAssetForRelation(outgoingRelation).url, outgoingRelation.href);
                ...
                // Inline asset
                var inlineAsset = AssetGraph.assets.create(outgoingRelation.to);
                this.addAsset(inlineAsset);
                outgoingRelation.to = inlineAsset;
                this.addRelation(outgoingRelation);
                this.populateRelationsToExistingAssets(inlineAsset);
            }
        }, this);
    }
});
