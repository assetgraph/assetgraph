...
class AssetGraph extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    // options.root might be undefined, in which case urlTools.urlOrFsPathToUrl will use process.cwd()
    /**
     ...
     * @type {string}
     */
    this.root = normalizeUrl(urlTools.urlOrFsPathToUrl(options.root, true)); // ensureTrailingSlash
   ...
   * @param {Relation} [incomingRelation] Supplying an incomingRelation will cause the assets baseUrl to be resolved from the relation rather than [AssetGraph.root]{@link AssetGraph#root}
  ...
  addAsset(assetConfig, incomingRelation) {
    ...
    /** @type {Asset} */
    let asset;
    if (/** @type {Asset} */ (assetConfig).isAsset) {
      // An already instantiated asset
      asset = /** @type {Asset} */ (assetConfig);
      asset.assetGraph = this;
    } else {
      /** @type {AssetConfig} */
      const assetConfigObject = assetConfig;
      if (typeof assetConfigObject.url === 'string') {
        if (!/^[a-zA-Z-+]+:/.test(assetConfigObject.url)) {
          assetConfigObject.url = this.resolveUrl(
            baseUrl,
            assetConfigObject.url
          );
        }
        assetConfigObject.url = assetConfigObject.url.replace(/#.*$/, '');

        if (!/^data:/i.test(assetConfigObject.url)) {
        ...
        if (this.canonicalRoot) {
          if (assetConfigObject.url.startsWith(this.canonicalRoot)) {
            assetConfigObject.url = assetConfigObject.url.replace(
              this.canonicalRoot,
              this.root
            );
          ...
          if (assetConfigObject.url === this.canonicalRoot.slice(0, -1)) {
            assetConfigObject.url = this.root;
          }
        }
        if (/^data:/.test(assetConfigObject.url)) {
          const parsedDataUrl = resolveDataUrl(assetConfigObject.url);
          if (parsedDataUrl) {
            Object.assign(assetConfigObject, parsedDataUrl);
            assetConfigObject.url = undefined;
            assetConfigObject.type =
              assetConfigObject.type ||
              this.lookupContentType(assetConfigObject.contentType);
          } else {
            this.warn(
              new errors.ParseError(
                `Cannot parse data url: ${assetConfigObject.url}`
              )
            );
          }
        } else if (/^javascript:/i.test(assetConfigObject.url)) {
          assetConfigObject.text = assetConfigObject.url.replace(
            /^javascript:/i,
            ''
          );
          assetConfigObject.url = undefined;
          assetConfigObject.type = 'JavaScript';
        } else {
          // Check if an asset with that url already exists in the graph,
          // and if it does, update it with the information contained
          // in assetConfigObject:
          asset = this._urlIndex[assetConfigObject.url];
          if (asset) {
            // New information about an existing asset has arrived
            asset.init(assetConfigObject);
          }
        }
      }
      if (!asset) {
        if (typeof assetConfigObject.url === 'undefined' && incomingRelation) {
          assetConfigObject.incomingInlineRelation = incomingRelation;
        }
        if (assetConfig instanceof this.Asset) {
          asset = /** @type {Asset} */ (assetConfigObject);
        } else {
          if (assetConfig.type) {
            asset = new AssetGraph[assetConfigObject.type](assetConfig, this);
          } else {
            asset = new this.Asset(assetConfigObject, this);
          }
  ...
  addAssets(...assetConfigs) {
    ...
    for (const assetConfig of _.flattenDeep(assetConfigs)) {
      if (
        ...
        /** @type {string[]} */
        const assetFilePaths = glob.sync(
          pathModule.resolve(
        ...
        assets.push(
          ...assetFilePaths.map(path =>
            this.addAsset(encodeURI(`file://${path}`))
          )
        );
      } else {
   ...
   * @return {string}
   */
  buildRootRelativeUrl(url, baseUrl) {
    return (
      (this._canonicalRootPath || '') +
      urlTools.buildRootRelativeUrl(baseUrl || this.root, url, this.root)
    );
  }
  ...
  /**
   * @callback GraphTraversalLambda
   * @param {Asset} asset
   * @param {Relation} incomingRelation
   * @return {Void}
   */

  /**
   * @param {Asset | Relation} startAssetOrRelation
   * @param {object} relationQueryObj
   * @param {GraphTraversalLambda} lambda
   */
   ...
   * @param {Asset | Relation} startAssetOrRelation
   * @param {object} relationQueryObj
   * @param {GraphTraversalLambda} preOrderLambda
   * @param {GraphTraversalLambda} [postOrderLambda]
   * @protected
   * @ignore
   */
  _traverse(
    ...
    /** @type {Relation} */
    let startRelation;
    if (/** @type {Relation} */ (startAssetOrRelation).isRelation) {
      startRelation = /** @type {Relation} */ (startAssetOrRelation);
      startAsset = startRelation.to;
    } else {
      // incomingRelation will be undefined when (pre|post)OrderLambda(startAsset) is called
      startAsset = /** @type {Asset} */ (startAssetOrRelation);
    }
    ...
    if (transform.length < 2) {
      setImmediate(() => {
        ...
        if (returnValue && typeof returnValue.then === 'function') {
          /** @type {Promise<any>} */ (returnValue).then(
...
/**
 * Information event. Some actions in the graph might need to inform the user about some conditions that happened during graph transformation.
 * The information level event should be used for non-critical information that do not require immediate attention
 * @event AssetGraph#info
 * @type {string | Error}
 */

/**
 * Warning event. Some operations on the graph can encounter errors that are very likely to require the users attention.
...
AssetGraph.registerTransform = (fileNameOrFunction, name) => {
  if (typeof fileNameOrFunction === 'function') {
    ...
    AssetGraph.transforms.__defineGetter__(name, () =>
      require(/** @type {string} */ (fileNameOrFunction))
    );
  }
 ...
 *
 * @static
 * @param  {Relation | String} fileNameOrConstructor A Relation constructor or a file name that exports one
...
AssetGraph.registerRelation = (fileNameOrConstructor, type) => {
  if (typeof fileNameOrConstructor === 'function') {
    const Constructor = /** @type {Relation} */ (fileNameOrConstructor);
    type = type || Constructor.name;
    Constructor.prototype.type = type;
    AssetGraph[type] = AssetGraph.prototype[type] = Constructor;
  } else {
    const fileName = /** @type {string} */ (fileNameOrConstructor);
    const fileNameRegex =
      os.platform() === 'win32' ? /\\([^\\]+)\.js$/ : /\/([^/]+)\.js$/;
    // Assume file name
    type = type || fileName.match(fileNameRegex)[1];
    const getter = () => {
