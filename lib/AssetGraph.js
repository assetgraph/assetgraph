...
AssetGraph.prototype = {
    ...
    findAssets: function (queryObj) {
        ...
        var results = this._query('asset', queryObj);
        if (incoming) {
    ...
    addAsset: function (asset) {
        ...
        this.objInBaseAssetPaths[asset.id] = [];
    },
    ...
    inlineAsset: function (asset, cb) {
        var affectedRelations = [].concat(this.objInBaseAssetPaths[asset.id]);
    ...
    setAssetUrl: function (asset, url) {
        ...
        this.objInBaseAssetPaths[asset.id].forEach(function (relation) {
            if (!oldUrl) {
    ...
    _registerBaseAssetPath: function (relation) {
        ...
        this.objInBaseAssetPaths[relation.id] = [];
        for (var i = 0 ; i < baseAssetPath.length ; i += 1) {
            this.objInBaseAssetPaths[baseAssetPath[i].id].push(relation);
        }
    ...
    _unregisterBaseAssetPath: function (relation) {
        ...
        for (var i = 0 ; i < baseAssetPath.length ; i += 1) {
            this.objInBaseAssetPaths[baseAssetPath[i].id].splice(this.objInBaseAssetPaths[baseAssetPath[i].id].indexOf(relation), 1);
        }
    ...
    removeRelation: function (relation) {
        if (this.objInBaseAssetPaths[relation.id].length) {
