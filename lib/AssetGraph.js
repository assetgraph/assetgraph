...
AssetGraph.prototype = {
    ...
    cloneAsset: function (asset, incomingRelations, cb) {
        ...
        asset._clone(error.passToFunction(cb, function (assetClone) {
            if (asset.url) {
                assetClone.url = fileUtils.resolveUrl(asset.url, assetClone.id + '.' + assetClone.defaultExtension);
            }
            if (asset.isInitial) {
            ...
            incomingRelations.forEach(function (relation) {
                // Nasty hack to preserve the position of the incoming relations in their respective indices.
                // FIXME: Maybe implement a refreshIndices(relationOrAsset, newPropertiesHash) method to do that.
                var i = that.indices.relation.to[relation.to.id].indexOf(relation);
                if (i === -1) {
                    throw new Error("assetGraph.cloneAsset: Incoming relation not in graph: " + relation);
                }
                that.indices.relation.to[relation.to.id].splice(i, 1);
                relation.to = assetClone;
                (that.indices.relation.to[relation.to.id] = that.indices.relation.to[relation.to.id] || []).push(relation),
