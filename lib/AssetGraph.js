...
_.extend(AssetGraph.prototype, {
    ...
    _refreshInlineRelations: function (asset, cb) {
        ...
        this.eachAssetPostOrder(asset, {to: {url: AssetGraph.query.isUndefined}}, function (asset, incomingRelation) {
            if (incomingRelation && asset.isDirty && incomingRelation._inline) {
                // Some read-only "assets" don't support _inline
                asset.isDirty = false;
                relationsToInline.push(incomingRelation);
            }
        });
        seq(relationsToInline)
            .seqEach(function (relation) {
                relation._inline(this);
            })
            .seq(function () {
                cb();
            })
