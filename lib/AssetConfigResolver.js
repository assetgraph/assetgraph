...
function AssetConfigResolver(config) {
    ...
    if (!/^[^:]+:/.test(this.root)) { // No protocol?
        this.root = urlTools.fsDirToFileUrl(this.root || process.cwd());
    }
...
AssetConfigResolver.prototype.resolve = function (assetConfig, fromUrl, cb) {
    ...
    if (_.isArray(assetConfig)) {
        ...
        return seq.ap(assetConfig)
            .parMap(function (_assetConfig) {
                that.resolve(_assetConfig, fromUrl, this);
            })
            .seq(function () {
    ...
    if (assetConfig.url) {
        ...
        if (protocol === 'file') {
                    ...
                    if (response.statusCode >= 400) {
                        ...
                        if ('content-type' in response.headers) {
                            ...
                            if (matchContentType) {
                                ...
                                console.log("AssetConfigResolver: Couldn't parse Content-Type " + response.headers['content-type'] + " while fetching " + assetConfig.url);
                            }
                        }
            ...
            if (dataUrlMatch) {
                ...
                if (dataUrlMatch[2]) {
                    assetConfig.originalEncoding = dataUrlMatch[2];
                }
                if (dataUrlMatch[3]) {
            ...
            return fsTools.findParentDirCached(urlTools.fileUrlToFsPath(fromUrl), protocol, error.passToFunction(cb, function (parentPath) {
