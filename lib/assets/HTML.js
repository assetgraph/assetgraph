        ...
        this.getParseTree(error.passToFunction(cb, function (parseTree) {
            ...
            while (queue.length) {
                ...
                if (node.nodeType === 1) { // Element
                    ...
                    } else if (nodeName === 'source' && node.parentNode && /^(?:video|audio)$/i.test(node.parentNode.nodeName)) {
                        originalRelations.push(new relations[node.parentNode.nodeName.toLowerCase() === 'video' ? 'HTMLVideo' : 'HTMLAudio']({
                            from: that,
                            to: node.getAttribute('src'),
                            node: node
                        }));
                    } else if (nodeName === 'param' && node.getAttribute('name') === 'src' && node.parentNode && node.parentNode.nodeName.toLowerCase() === 'object') {
                        originalRelations.push(new relations.HTMLObjectSrcParam({
                            from: that,
