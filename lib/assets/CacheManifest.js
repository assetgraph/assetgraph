            ...
            text.split(/\r?\n|\n?\r/).forEach(function (line, i) {
                ...
                if (matchNewSection) {
                    ...
                    if (tokens.length === (currentSectionName === 'FALLBACK' ? 2 : 1)) {
                        ...
                        console.warn("CacheManifest.getParseTree: Skipped illegal line in " + currentSectionName + " section: " + line);
                    }
                }
            });
            cb(null, parseTree);
    ...
    getText: function (cb) {
        ...
        process.nextTick(function () {
            if ('parseTree' in that) {
                ...
                if (that.parseTree.CACHE) {
                    src += getSectionText(that.parseTree.CACHE);
                }
                _.each(that.parseTree, function (nodes, sectionName) {
