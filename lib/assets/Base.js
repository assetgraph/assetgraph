...
Base.prototype = {
    ...
    getEncoding: function (cb) {
        var encoding = this.encoding || this.defaultEncoding;
        ...
        that.getEncoding(error.passToFunction(cb, function (encoding) {
            if (that.isDirty) {
                ...
                that.getText(error.passToFunction(cb, function (text) {
                    var targetEncoding = that.targetEncoding || encoding || that.defaultEncoding;
                ...
                that.getRawSrc(error.passToFunction(cb, function (rawSrc) {
                    if (!that.targetEncoding || that.targetEncoding === encoding) {
                        ...
                        cb(null, new iconv.Iconv(encoding, that.targetEncoding).convert(rawSrc));
                    }
                }));
            }
        }));
