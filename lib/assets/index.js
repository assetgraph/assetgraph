    ...
    return assetConfig.rawSrcProxy(passError(cb, function (rawSrc, metadata) {
        ...
        function foundType(type) {
            if (type) {
                ...
                cb(new Error("assets.ensureAssetConfigHasType: Couldn't determine asset type from asset config: ", assetConfig));
            }
        }
...
assets.resolveConfig = function (assetConfig, fromUrl, assetGraph, cb) {
    ...
    if (assetConfig.isAsset || assetConfig.isResolved) {
        ...
        process.nextTick(function () {
            cb(new Error("assets.resolveConfig: Cannot resolve asset config (no url): " + JSON.stringify(assetConfig)));
        });
    }
};
