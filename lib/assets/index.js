...
assets.ensureAssetConfigHasType = function (assetConfig, cb) {
    ...
    return assetConfig.rawSrcProxy(function (err, rawSrc, metadata) {
        if (err) {
            assetGraph.emit('error', new Error("assets.ensureAssetConfigHasType: Couldn't load " + (assetConfig.url || util.inspect(assetConfig)) + ", assuming assets.Base"));
            assetConfig.type = 'Base';
            return cb();
        }
        ...
        function foundType(type) {
            if (!type) {
                assetGraph.emit('error', new Error("assets.ensureAssetConfigHasType: Couldn't determine asset type from asset config: " + util.inspect(assetConfig)) + ", assuming assets.Base");
            }
            assetConfig.type = type || 'Base';
            cb();
        }
