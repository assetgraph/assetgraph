...
extendWithGettersAndSetters(Css.prototype, {
    ...
    _updateText: function (forceUpdateSourceMap) {
        ...
        var result;
        if (this.isPretty) {
            result = perfectionist.process(this.parseTree, { map: { inline: false, annotation: false } });
            ...
            }).process(postcss.parse(this.parseTree), { map: { inline: false, annotation: false } });
        } else {
            result = this.parseTree.toResult({ map: { inline: false, annotation: false } });
        }
        this._text = result.root.toString(); // cssnano case?
        var map = result.map;
        map._mappings._array.forEach(function (mapping) {
            mapping.source = mapping.source.replace(/^http:\/+/, 'http://').replace(/^file:\/*/, 'file:///');
        });

        this._sourceMap = map.toJSON();
