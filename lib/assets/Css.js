            ...
            if (node.type === 'comment') {
                ...
                for (const childNode of node.nodes) {
                    ...
                    if (propertyWithImageUrlRegExp.test(propertyName)) {
                        for (const [tokenNumber, href] of AssetGraph.CssImage.prototype.findUrlsInPropertyValue(propertyValue).entries()) {
                            if (!/^#/.test(href)) { // Don't model eg. filter: url(#foo);
                                outgoingRelations.push({
                                    type: 'CssImage',
                                    href,
                                    tokenNumber,
                    ...
                    if (propertyName === 'filter' || propertyName === '-ms-filter') {
                        for (const [tokenNumber, href] of AssetGraph.CssAlphaImageLoader.prototype.findUrlsInPropertyValue(propertyValue).entries()) {
                            outgoingRelations.push({
