...
const propertyWithImageUrlRegExp =
  /^(?:content|_?cursor|_?background(?:-image)?|(?:-[a-z]+-)?(?:border-image(?:-source)?|mask|mask-image|mask-image-source|mask-box-image|mask-box-image-source)|(?:\+|-webkit-)?filter)$/i;
      ...
      if (node.type === 'comment') {
        ...
        for (const childNode of node.nodes) {
          ...
          if (propertyWithImageUrlRegExp.test(propertyName)) {
            for (const [tokenNumber, href] of AssetGraph.CssImage.prototype
