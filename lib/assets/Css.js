...
extendWithGettersAndSetters(Css.prototype, {
    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        // Fast path: Don't bother building a parse tree if we can tell that there're no relations by doing a
        // regular expression match:
        if (this._text && !this._parseTree && !/@import|url\s*\(|AlphaImageLoader/i.test(this._text)) {
        ...
        this.eachRuleInParseTree(function (cssRule, parentRuleOrStylesheet) {
            if (cssRule.type === cssom.CSSRule.IMPORT_RULE) {
                ...
                if (src) {
                    AssetGraph.CssFontFaceSrc.prototype.findUrlsInPropertyValue(src).forEach(function (url, tokenNumber) {
                ...
                ['filter', '_filter', '-ms-filter'].forEach(function (propertyName) {
                    if (propertyName in style) {
                        ...
                        AssetGraph.CssAlphaImageLoader.prototype.findUrlsInPropertyValue(value).forEach(function (url, tokenNumber) {
