...
function extractEncodingFromDecodedSrc(text) {
...
_.extend(Css.prototype, {
    ...
    getOriginalEncoding: function (cb) {
        ...
        if (that.originalEncoding) {
            ...
            process.nextTick(function () {
                cb(null, extractEncodingFromDecodedSrc(that.text) || that.defaultEncoding);
            });
    ...
    getParseTree: memoizeAsyncAccessor('parseTree', function (cb) {
        this.getText(passError(cb, function (text) {
            ...
            text = text.replace(/@charset\s*([\'\"])\s*[\w\-]+\s*\1;/, "");
            cb(null, cssom.parse(text));
        }));
    }),
    ...
    getText: function (cb) {
        ...
        process.nextTick(function () {
            if ('parseTree' in that) {
                var cssString = that.parseTree.toString();
                if (!that.isPretty) {
