...
const propertyWithImageUrlRegExp = /^(?:content|_?cursor|_?background(?:-image)?|(?:-[a-z]+-)?(?:border-image(?:-source)?|mask|mask-image|mask-image-source|mask-box-image|mask-box-image-source)|(?:\+|-webkit-)?filter)$/;

function extractEncodingFromText(text) {
    const matchCharset = text.match(/@charset\s*([\'\"])\s*([\w\-]+)\s*\1/mi);
...
class Css extends Text {
    ...
    _updateText(forceUpdateSourceMap) {
        const sourceMapsEnabled = !this.assetGraph || this.assetGraph.sourceMaps !== false;
        ...
        if (this._parseTree) {
            let result;
            if (this.isPretty) {
                result = postcss(perfectionist).process(this.parseTree, { map: sourceMapsEnabled && { inline: false, annotation: false, sourcesContent: true } });
                ...
                })).process(Css.cloneWithRaws(this.parseTree), { map: sourceMapsEnabled && { inline: false, annotation: false, sourcesContent: true } });
            } else {
                result = this.parseTree.toResult({ map: sourceMapsEnabled && { inline: false, annotation: false, sourcesContent: true } });
            }
    ...
    get parseTree() {
        if (!this._parseTree) {
            ...
            const sourceUrl = this.sourceUrl || (nonInlineAncestor && nonInlineAncestor.url) || this.url || 'standalone-' + this.id + '.css';
            try {
    ...
    set sourceMap(sourceMap) {
        ...
        if (typeof this._parseTree !== 'undefined') {
            Object.assign(this._parseTree.source.input, postcss.parse('', { map: { prev: sourceMap } }).source.input);
        }
    }
    ...
    findOutgoingRelationsInParseTree() {
        ...
        this.eachRuleInParseTree((node, parentRuleOrStylesheet) => {
            if (node.type === 'comment') {
                const matchSourceUrlOrSourceMappingUrl = node.text.match(/[@#]\s*source(Mapping)?URL=([^\s\n]*)/);
                ...
                for (const childNode of node.nodes) {
                    if (childNode.type === 'decl' && childNode.prop === 'src') {
                        for (const [tokenNumber, url] of AssetGraph.CssFontFaceSrc.prototype.findUrlsInPropertyValue(childNode.value).entries()) {
                ...
                for (const childNode of node.nodes) {
                    const propertyName = childNode.prop;
                    const propertyValue = childNode.value;
                    if (propertyWithImageUrlRegExp.test(propertyName)) {
                        for (const [tokenNumber, url] of AssetGraph.CssImage.prototype.findUrlsInPropertyValue(propertyValue).entries()) {
                        ...
                        const matchUrl = propertyValue.match(/\burl\((\'|\"|)([^#\'\"][^\'\"]*?)\1\)/);
                        if (matchUrl) {
                    ...
                    if (propertyName === 'filter' || propertyName === '-ms-filter') {
                        for (const [tokenNumber, url] of AssetGraph.CssAlphaImageLoader.prototype.findUrlsInPropertyValue(propertyValue).entries()) {
...
// Waiting for https://github.com/postcss/postcss/issues/364
Css.cloneWithRaws = function (original) {
    ...
    (function reattachRaws(clone, original) {
        ...
        if (clone.nodes && original.nodes) {
            for (let i = 0 ; i < original.nodes.length && i < clone.nodes.length ; i += 1) {
...
const originalAddMapping = postcssSourceMap.SourceMapGenerator.prototype.addMapping;
...
const originalApplySourceMap = postcssSourceMap.SourceMapGenerator.prototype.applySourceMap;
