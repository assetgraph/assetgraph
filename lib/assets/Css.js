...
extendWithGettersAndSetters(Css.prototype, {
    ...
    get text() {
        if (!('_text' in this)) {
            if (this._parseTree) {
                var text = this._parseTree.toString();
                if (!this.isPretty) {
                    try {
                        text = csso.justDoIt(text);
                    } catch (e) {
                        var line = parseInt(e.toString().match(/\d+$/)[0], 10);
                        this.assetGraph.emit('warn', new errors.ParseError({
                            message: 'CSS minification: Parse error in block starting on: ' + this.urlOrDescription +
                                '#' + line + '\n' +
                                'Falling back to using unminified content',
                            line: e.line,
                            asset: this
                        }));
                    }
