    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        if (this._parseTree[0] instanceof uglifyJs.AST_String) {
            ...
            while (cursor instanceof uglifyJs.AST_PropAccess) {
                this.wildCardValueAsts.push(cursor.expression || new uglifyJs.AST_String(cursor.property));
    ...
    toAst: function () {
        ...
        if (nestingLevel > 0) {
            this.wildCardValueAsts.forEach(function (wildCardValueAst) {
                if (wildCardValueAst instanceof uglifyJs.AST_String) {
                    expressionAst = new uglifyJs.AST_Dot({expression: expressionAst, property: wildCardValueAst.value});
                } else {
