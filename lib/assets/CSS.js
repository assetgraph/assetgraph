...
_.extend(CSS.prototype, {
    ...
    getEncoding: function (cb) {
        ...
        that.getRawSrc(error.passToFunction(cb, function (rawSrc) {
            var matchCharset = rawSrc.toString("binary", 0, Math.min(1024, rawSrc.length)).match(/@charset\s*([\'\"])\s*([\w\-]+)\s*\1/mi);
            if (matchCharset) {
                cb(null, matchCharset[2]);
            } else {
                        ...
                        if ((CSS.vendorPrefix + '-sprite-selector-for-group') in style) {
                            originalRelations.push(new relations.CSSSpritePlaceholder({
                                ...
                                to: {
                                    ...
                                    rawSrc: CSS.extractInfoFromRule(cssRule, CSS.vendorPrefix + '-sprite-')
                                },
                                parentRule: parentRuleOrStylesheet,
                                cssRule: cssRule
                            }));
