...
extendWithGettersAndSetters(JavaScript.prototype, {
    ...
    get text() {
        if (!('_text' in this)) {
            ...
            if (parseTree) {
                ...
                text = text.replace(/;*$/, ';'); // Always end with a semicolon like the UglifyJS binary
    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        var tryFoldConstantToString = function (node, parentNode) {
            ...
            [node.start, node.end].forEach(function (token) {
                if (token && token.comments_before) {
                    token.comments_before.forEach(function (comment) {
                        var matchSourceUrl = comment.value.match(/@\s*sourceURL=([^\s\n]*)/);
                        if (matchSourceUrl && seenComments.indexOf(comment) === -1) {
                            outgoingRelations.push(new AssetGraph.JavaScriptSourceUrl({
                                from: this,
                                node: comment,
                                to: {
                                    url: matchSourceUrl[1]
                                }
                            }));
