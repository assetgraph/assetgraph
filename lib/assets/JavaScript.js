        ...
        walker.with_walkers({
            call: function () {
                ...
                if (node[1][0] === 'dot' &&
                    ...
                    var diveIntoAngularMethodCall = function (methodName, callNode, argumentNodes) {
                        ...
                        angularWalker.with_walkers({
                            object: function () {
                                ...
                                node[1].forEach(function (keyValue) {
                                    if (keyValue[0] === 'templateUrl' && keyValue[1][0] === 'string') {
                                        outgoingRelations.push(new relations.JavaScriptAngularJsTemplate({
                                            from: this,
                                            to: {
                                                type: 'AngularJsTemplate',
                                                url: keyValue[1][1]
                                            },
                                            node: keyValue,
                                            parentNode: node
                            ...
                            call: function () {
                                var stack = angularWalker.stack(),
                                    node = stack[stack.length - 1];
                                if (node[1][0] === 'dot' && node[1][1][0] === 'name' && node[1][1][1] === '$templateCache' &&
                                    node[1][2] === 'put' && node[2].length === 2 && node[2][0][0] === 'string' && node[2][1][0] === 'string') {
                                    outgoingRelations.push(new relations.JavaScriptAngularJsTemplateCacheAssignment({
                                        from: this,
                                        to: new (require('./AngularJsTemplate'))({
                    ...
                    while (stack[stackPosition - 1][0] === 'dot' && stack[stackPosition - 2][0] === 'call') {
                        ...
                        diveIntoAngularMethodCall(methodName, callNode, argumentNodes);
                        stackPosition -= 2;
                    }
                }
                ...
                if (node[1][0] === 'name' && node[1][1] === 'INCLUDE') {
                    ...
                    if (node[2][0][0] === 'call' && node[2][0][1][0] === 'name' && node[2][0][1][1] === 'GETTEXT' && node[2][0][2].length === 1) {
                        ...
                        if (node[2][0][2][0][0] === 'string') {
                            outgoingRelation = new relations.JavaScriptTrHtml({
                                from: this,
                                node: node,
                                to: {
                                    url: node[2][0][2][0][1]
