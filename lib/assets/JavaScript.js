      ...
      enter(node, parentNode) {
        ...
        if (node.type === 'CallExpression') {
          ...
          if (
            ...
            ['paintWorklet', 'layoutWorklet', 'animationWorklet', 'audioWorklet'].includes(node.callee.object.property.name) &&
            ...
            if (node.callee.object.object.name === 'CSS' || node.callee.object.property.name === 'audioWorklet') {
              ...
              if (getHrefType(href) === hrefTypes.RELATIVE) {
                const err = new Error(`Using a relative URL when adding a worklet can cause problems. The href is resolved relative to the page's url, not the javascript module that include it`);
                err.assset = this;
