...
extendWithGettersAndSetters(JavaScript.prototype, {
    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        var traverse = function (node) {
            ...
            } else if (node[0] === 'call' && node[1][0] === 'name' && (node[1][1] === 'require' || node[1][1] === 'define') &&
                       node[2].length === 2 && node[2][0][0] === 'array' && node[2][1][0] === 'function') {
                node[2][0][1].forEach(function (arrayItemAst) {
                    if (arrayItemAst[0] === 'string') {
                        outgoingRelations.push(new relations.JavaScriptAmdRequire({
                            from: this,
                            to: {
                                url: relations.JavaScriptAmdRequire.extractHrefFromRequireItem(arrayItemAst[1])
                            },
