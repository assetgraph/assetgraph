    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        var traverse = function (node) {
            if (node[0] === 'call' && node[1][0] === 'dot' && node[1][1][0] === 'name' && node[1][1][1] === 'one') {
                ...
                if (node[2].length !== 1 || node[2][0][0] !== 'string') {
                    console.warn('Skipping unsupported JavaScriptCommonJsRequire: There must be a single string argument: ' + JSON.stringify(node));
                } else if (!/^file:/.test(baseUrl)) {
                    console.warn('Skipping JavaScriptCommonJsRequire (only supported from file: urls): ' + JSON.stringify(node));
                } else {
