...
/**
 * Used to detect whether the JavaScript source code has pretty formatting
 *
 * Source code is deemed 'pretty' if it matches any of these rules:
 * - Has a newline followed by a tab or space
 * - Has a semicolon followed by a newline which is not terminating the string
 * - Has a semicolon terminating the string
 */
const isPrettyPattern = /\n[ \t]|;\r?\n[^$]|;$/;
...
class JavaScript extends Text {
  ...
  _applySourceMapToParseTree() {
    ...
    try {
      ...
      if (
        !/^Cannot use 'in' operator to search for|Line must be greater than/.test(
          e.message
        )
      ) {
        throw e;
  ...
  _parse(text, sourceType = 'module') {
    ...
    this.isPretty = isPrettyPattern.test(text);
    ...
    estraverse.traverse(parseTree, {
      enter(node, parentNode) {
        ...
        if (
          ...
          // Javasscript paint worklet: https://developer.mozilla.org/en-US/docs/Web/API/PaintWorklet#Load_a_PaintWorklet
          ...
          if (
            ...
            node.callee.property.type === 'Identifier' &&
            node.callee.property.name === 'addModule' &&
            node.arguments[0] &&
            node.arguments[0].type === 'Literal'
          ) {
            if (
