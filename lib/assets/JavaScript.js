    ...
    get text() {
        if (!('_text' in this)) {
            ...
            if (parseTree) {
                ...
                delete outputOptions.side_effects; // uglifyJs.OutputStream will throw if passed unknown options. Find a better solution.
                var outputStream = uglifyJs.OutputStream(outputOptions);
                ...
                if (node.expression instanceof uglifyJs.AST_Symbol && node.expression.name === 'INCLUDE') {
                    ...
                    if (outgoingRelation) {
                        ...
                        syntaxErrors.push(new errors.SyntaxError({
                           ...
                           ((node.args.length === 2 && (node.args[1] instanceof uglifyJs.AST_Function || node.args[1] instanceof uglifyJs.AST_SymbolRef)) || node.args.length === 1) &&
