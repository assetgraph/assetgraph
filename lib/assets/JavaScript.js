...
extendWithGettersAndSetters(JavaScript.prototype, {
    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        if (assetGraph && assetGraph.requireJsConfig && this.incomingRelations.some(function (incomingRelation) {return /^JavaScript(?:ShimRequire|Amd(?:Define|Require))$/.test(incomingRelation.type);})) {
            var moduleName = assetGraph.requireJsConfig.getModuleName(this, new AssetGraph.JavaScriptShimRequire({from: this, href: 'bogus'}).baseAsset.nonInlineAncestor.url), // Argh!
        ...
        var requireJsCommonJsCompatibilityNestingLevel = 0;

        function descendIfRequireJsCommonJsCompatibility(node, descend) {
            ...
            if (lastArgument && lastArgument instanceof uglifyJs.AST_Function) {
                if (lastArgument.argnames.length === 3 && lastArgument.argnames[0].name === 'require' && lastArgument.argnames[1].name === 'exports' && lastArgument.argnames[2].name === 'module') {
                    requireJsCommonJsCompatibilityNestingLevel += 1;
                    descend();
                    requireJsCommonJsCompatibilityNestingLevel -= 1;
                }
            }
                ...
                if (node.expression instanceof uglifyJs.AST_Symbol && node.expression.name === 'INCLUDE') {
                    ...
                    if (node.args.length === 1) {
                        ...
                        if (!(parentNode instanceof uglifyJs.AST_Call) || !(parentNode.expression instanceof uglifyJs.AST_Symbol) || parentNode.expression.name !== 'TRHTML') {
                            node.args[0] = uglifyAst.foldConstant(node.args[0]);
                    ...
                    if (node.args[0] instanceof uglifyJs.AST_Call && node.args[0].expression instanceof uglifyJs.AST_Symbol && node.args[0].expression.name === 'GETTEXT' && node.args[0].args.length === 1) {
                        ...
                        node.args[0].args[0] = uglifyAst.foldConstant(node.args[0].args[0]);
                    ...
                    arrayNode.elements.forEach(function (arrayItemAst, i) {
                        arrayItemAst = arrayNode.elements[i] = uglifyAst.foldConstant(arrayItemAst);
                    ...
                    descendIfRequireJsCommonJsCompatibility(node, descend);
                    ...
                    if (arrayNode) {
                        arrayNode.elements.forEach(function (arrayItemAst, i) {
                            arrayNode.elements[i] = arrayItemAst = uglifyAst.foldConstant(arrayItemAst);
                            if (arrayItemAst instanceof uglifyJs.AST_String) {
                                if (!isSeenNamedAmdDefinesByModuleName[arrayItemAst.value] && arrayItemAst.value !== 'exports') {
                    ...
                    if (requireJsCommonJsCompatibilityNestingLevel > 0) {
