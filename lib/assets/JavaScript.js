...
function serializeAst(node) {
    return node.print(uglifyJs.OutputStream()).toString();
}
        ...
        if (!('_text' in this)) {
            if (this._parseTree) {
                this._text = this._parseTree.print(uglifyJs.OutputStream({
                    ...
                    source_map: null
                })).toString().replace(/;*$/, ';'); // Always end with a semicolon like the UglifyJS binary
            } else {
        ...
        var parseTree = this.parseTree,
            walker = new uglifyJs.TreeWalker(function (node) {
                ...
                if (node instanceof uglifyJs.AST_Call) {
                    ...
                    if (node.expression instanceof uglifyJs.Dot && node.expression.property === 'module' &&
                    ...
                    if (node.expression instanceof uglifyJs.AST_Symbol && node.expression.name === 'INCLUDE') {
                        if (node.args.length === 1 && node.args[0] instanceof uglifyJs.AST_String) {
                            ...
                            syntaxErrors.push(new errors.SyntaxError({
                                message: "Invalid INCLUDE syntax: Must take a single string argument:" + serializeAst(node),
                                asset: this
                            }));
                        }
                        ...
                        if (node.args.length === 1) {
                            ...
                            if (!(parentNode instanceof uglifyJs.AST_Call) || !(parentNode.expression instanceof uglifyJs.AST_Symbol) || parentNode.expression.name !== 'TRHTML') {
                                ...
                                if (node.args[0] instanceof uglifyJs.AST_String) {
                                    ...
                                    syntaxErrors.push(new errors.SyntaxError({
                                        message: "Invalid GETTEXT syntax: " + serializeAst(node),
                                        asset: this
                                    }));
                                }
                            }
                        ...
                        if (node.args[0] instanceof uglifyJs.AST_Call && node.args[0].expression instanceof uglifyJs.AST_Symbol && node.args[0].expression.name === 'GETTEXT' && node.args[0].args.length === 1) {
                            ...
                            constantFoldNode(node.args[0].args[0]);
                        ...
                        if (outgoingRelation) {
                            ...
                            syntaxErrors.push(new errors.SyntaxError({
                                message: "Invalid TRHTML syntax: " + serializeAst(node),
                                asset: this
                            }));
                        }
                        ...
                        arrayNode.elements.forEach(function (arrayItemAst) {
                            ...
                            if (arrayItemAst instanceof uglifyJs.AST_String) {
                                ...
                                warnings.push(new errors.SyntaxError('Skipping non-string JavaScriptAmdRequire item: ' + serializeAst(node)));
                            }
                        }, this);
                        ...
                        if (arrayNode) {
                            arrayNode.elements.forEach(function (arrayItemAst) {
                                ...
                                if (arrayItemAst instanceof uglifyJs.AST_String) {
                                    ...
                                    warnings.push(new errors.SyntaxError('Skipping non-string JavaScriptAmdDefine item: ' + serializeAst(node)));
                                }
                            }, this);
                        }
                    } else if (node.expression instanceof uglifyJs.AST_Symbol && node.expression.name === 'require' &&
                        ...
                        if (/^file:/.test(baseUrl)) {
                            ...
                            try {
                                ...
                                warnings.push(new errors.SyntaxError({message: "Couldn't resolve " + serializeAst(node) + ", skipping", relationType: 'JavaScriptCommonJsRequire'}));
                            }
                            ...
                            warnings.push(new errors.SyntaxError({message: 'Skipping JavaScriptCommonJsRequire (only supported from file: urls): ' + serializeAst(node), relationType: 'JavaScriptCommonJsRequire'}));
                        }
                    }
                }
            });
