...
extendWithGettersAndSetters(JavaScript.prototype, {
    ...
    get text() {
        if (!('_text' in this)) {
            if (this._parseTree) {
                ...
                text += uglifyJs.uglify.gen_code(this._parseTree, {beautify: this.isPretty});
                this._text = text;
            } else {
    ...
    get parseTree() {
        if (!this._parseTree) {
            ...
            try {
                ...
                var err = new Error('Parse error in ' + (this.url || 'inline JavaScript' + (this.nonInlineAncestor ? ' in ' + this.nonInlineAncestor.url : '')) + '\n' +
    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        if (errors.length) {
            var err = new Error(_.pluck(errors, 'message').join("\n"));
            if (this.assetGraph) {
                this.assetGraph.emit('error', err);
            } else {
                throw err;
