...
extendWithGettersAndSetters(JavaScript.prototype, {
    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        var resolveAmdRequireUrl = function (url) {
            var fragments = url.indexOf('/') !== -1 && url.split('/');
            if (fragments && fragments.length > 1 && this.assetGraph && this.assetGraph.requireJsPaths && fragments[0] in this.assetGraph.requireJsPaths) {
                return urlTools.resolveUrl(this.assetGraph.requireJsPaths[fragments[0]], fragments.slice(1).join('/'));
            } else {
                return url;
            }
        }.bind(this);
                ...
                if (node[1][0] === 'dot' && node[1][1][0] === 'name' && node[1][1][1] === 'one') {
                    ...
                    arrayNode[1].forEach(function (arrayItemAst) {
                        if (arrayItemAst[0] === 'string') {
                            ...
                            outgoingRelation.to = {url: resolveAmdRequireUrl(outgoingRelation.href)};
