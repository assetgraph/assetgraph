...
class JavaScript extends Text {
  static registerRelation(Class) {
    if (Class.selectors) {
      for (const selector of Object.keys(Class.selectors)) {
        JavaScript.selectors.push([
          selector, // esquery.parse(selector), // Would be nice if esdispatch accepted a parsed selector
  ...
  findOutgoingRelationsInParseTree() {
    ...
    const esdispatcher = new ESDispatcher();
    for (const [selector, fn, Class] of JavaScript.selectors) {
      esdispatcher.addListener(selector, (node, stack) => {
        let relations = fn(node, stack);
        if (relations) {
          if (!Array.isArray(relations)) {
            relations = [relations];
          }
          for (const relation of relations) {
            relation.type = relation.type || Class.name;
          }
          outgoingRelations.push(...relations);
        }
      });
    }
    esdispatcher.observe(parseTree);

    // FIXME: Shouldn't be a side effect of findOutgoingRelationsInParseTree
    this.strict = this.parseTree.body.some(
      node =>
...
JavaScript.registerRelation(require('../relations/JavaScriptStaticUrl'));
JavaScript.registerRelation(
  require('../relations/JavaScriptServiceWorkerRegistration')
);
JavaScript.registerRelation(require('../relations/JavaScriptWorklet'));
JavaScript.registerRelation(require('../relations/JavaScriptDynamicImport'));
JavaScript.registerRelation(require('../relations/JavaScriptImportScripts'));
JavaScript.registerRelation(require('../relations/JavaScriptImport'));
JavaScript.registerRelation(require('../relations/JavaScriptFetch'));
JavaScript.registerRelation(require('../relations/JavaScriptWebWorker'));
JavaScript.registerRelation(require('../relations/JavaScriptExport'));
JavaScript.registerRelation(require('../relations/JavaScriptSourceUrl'));
JavaScript.registerRelation(require('../relations/JavaScriptSourceMappingUrl'));
JavaScript.registerRelation(require('../relations/SystemJsBundle'));
