...
extendWithGettersAndSetters(JavaScript.prototype, {
    ...
    get parseTree() {
        if (!this._parseTree) {
            var text = this.text;
            try {
                this._parseTree = esprima.parse(text, { loc: true, attachComment: true, source: this.urlOrDescription });
            } catch (e) {
                                        ...
                                        if (node.type === 'ObjectExpression') {
                                            node.properties.forEach(function (property) {
                                                if (property.kind === 'init' && property.key.type === 'Identifier') {
                                                    if (property.key.name === 'templateUrl' && property.value.type === 'Literal' && typeof property.value.value === 'string') {
                                                    ...
                                                    } else if (property.key.name === 'template' && property.value.type === 'Literal' && typeof property.value.value === 'string') {
                                                        outgoingRelations.push(new AssetGraph.JavaScriptAngularJsTemplate({
                                                            from: that,
