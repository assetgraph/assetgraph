        ...
        this.getParseTree(error.passToFunction(cb, function (parseTree) {
            ...
            function extractStringNodes(arrayOrStringAst, errorMessage) {
                ...
                if (arrayOrStringAst[0] === 'string') {
                    ...
                    firstErrorOrNull = firstErrorOrNull || new Error(errorMessage + ': first argument must be string or array of strings: ' + JSON.stringify(arrayOrStringAst));
                }
                return stringNodes;
            }
                ...
                if (node[0] === 'stat' && Array.isArray(node[1]) &&
                    ...
                    if (methodName === 'setPath') {
                        ...
                        if (args.length === 0) {
                            ...
                            if (methodName !== 'syncRequire' || args[0][0] !== 'name') { // The Ext bootstrapper itself contains some Ext.syncRequire(h) statements
            ...
            traverse(parseTree);
            if (firstErrorOrNull) {
                firstErrorOrNull.message = that.toString() + ': ' + firstErrorOrNull.message;
                cb(firstErrorOrNull);
            } else {
