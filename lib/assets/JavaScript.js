...
extendWithGettersAndSetters(JavaScript.prototype, {
    ...
    get text() {
        if (!('_text' in this)) {
            if (this._parseTree) {
                var text = '';
                (this.initialComments || []).forEach(function (comment) {
    ...
    get parseTree() {
        if (!this._parseTree) {
            ...
            if (firstToken) {
                this.initialComments = firstToken.comments_before;
            }
            try {
                this._parseTree = uglifyJs.parser.parse(text);
            } catch (e) {
                var err = new errors.ParseError({
                    message: 'Parse error in ' + (this.url || 'inline JavaScript' + (this.nonInlineAncestor ? ' in ' + this.nonInlineAncestor.url : '')) + '\n' + e.message + ' (line ' + e.line + ', column ' + e.col + ')',
                    line: e.line,
                    column: e.col,
                    asset: this
                });
                ...
                if (node[1][0] === 'dot' && node[1][1][0] === 'name' && node[1][1][1] === 'one') {
                    ...
                    if (node[1][2] === 'include') {
                        if (node[2].length === 1 && node[2][0][0] === 'string') {
                            ...
                            syntaxErrors.push(new errors.SyntaxError({
                                message: "Invalid one.include syntax: Must take a single string argument:" + uglifyJs.uglify.gen_code(node),
                                asset: this
                            }));
                        }
                        ...
                        if (node[2].length === 1 && node[2][0][0] === 'string') {
                            ...
                            syntaxErrors.push(new errors.SyntaxError({
                                message: "Invalid one.getText syntax: " + uglifyJs.uglify.gen_code(node),
                                asset: this
                            }));
                        }
