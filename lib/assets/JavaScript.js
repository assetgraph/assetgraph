...
class JavaScript extends Text {
    ...
    _getEscodegenOptions() {
        const outputOptions = _.defaults({}, this.serializationOptions, this.assetGraph && this.assetGraph.javaScriptSerializationOptions);
        return {
            format: {
                compact: outputOptions.compact, // Takes precedence over space, newline, and indent
                space: typeof outputOptions.space === 'undefined' ? (this.isPretty || !this.toBeMinified ? ' ' : '') : outputOptions.space,
                newline: typeof outputOptions.newline === 'undefined' ? (this.isPretty || !this.toBeMinified ? '\n' : '') : outputOptions.newline,
                semicolons: typeof outputOptions.semicolons === 'undefined' ? this.isPretty || !this.toBeMinified : outputOptions.semicolons,
                parentheses: typeof outputOptions.parentheses === 'undefined' ? this.isPretty || !this.toBeMinified : outputOptions.parentheses,
                ...
                indent: {
                    style: typeof outputOptions.indent_level === 'number' ?
                        ...
                        (this.isPretty || !this.toBeMinified ? '    ' : '')
                }
            },
            comment: true
        };
    ...
    _patchUpSerializedText(text, parseTree) {
        if (text.length > 0) {
            if (this.isInline) {
                ...
                const lastStatement = parseTree.body.length && parseTree.body[parseTree.body.length - 1];
    ...
    get textAndSourceMap() {
        if (typeof this._text === 'string') {
            ...
            const parseTree = this._parseTree;
            const result = escodegen.generate(parseTree, _.defaults({ sourceMap: true, sourceMapWithCode: true }, this._getEscodegenOptions()));
    ...
    get parseTree() {
        if (!this._parseTree) {
            ...
            const sourceUrl = this.sourceUrl || (nonInlineAncestor && nonInlineAncestor.url) || this.url || 'standalone-' + this.id + '.js';
            ...
            try {
                ...
                if (this.assetGraph && parsedAsScript) {
                    this.assetGraph.info(new errors.ParseError('Could not parse ' + this.urlOrDescription + ' as a module, fall back to script mode\n' + errorParsingAsModule.message));
                }
            }
        }
    ...
    minify() {
        ...
        function filterCommentAttribute(node, attributeName) {
            if (node[attributeName]) {
                const leftoverComments = node[attributeName].filter(JavaScript.shouldCommentNodeBePreservedInNonPrettyPrintedOutput);
                ...
                for (const propertyName of ['leadingComments', 'trailingComments']) {
                    for (const comment of (node[propertyName] || [])) {
                        if (!processedComments.includes(comment)) {
                            ...
                            const matchSourceUrlOrSourceMappingUrl = comment.value.match(/[@#]\s*source(Mapping)?URL=([^\s\n]*)/);
                            if (matchSourceUrlOrSourceMappingUrl) {
                                ...
                                const matchSystemJsBundle = comment.value.match(/#\s*SystemJsBundle=([^\s\n]*)/);
