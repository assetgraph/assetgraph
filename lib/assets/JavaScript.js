...
const espurify = require('espurify-papandreou');
...
class JavaScript extends Text {
  ...
  get parseTree() {
    if (!this._parseTree) {
      ...
      try {
        this._parseTree = espree.parse(text, espreeOptions);
        escodegen.attachComments(
          this._parseTree,
          this._parseTree.comments,
          this._parseTree.tokens
        );
        if (this._sourceMap) {
          this._applySourceMapToParseTree();
        }
      } catch (errorParsingAsModule) {
        espreeOptions.sourceType = 'script';
        ...
        try {
          ...
          this._parseTree = espree.parse('', { loc: true, source: sourceUrl });
        }
