...
_.extend(JavaScript.prototype, {
    ...
    getParseTree: memoizeAsyncAccessor('parseTree', function (cb) {
        ...
        this.getText(passError(cb, function (text) {
            var parseTree;
            try {
                parseTree = uglify.parser.parse(text);
            } catch (e) {
    ...
    getText: function (cb) {
        ...
        process.nextTick(function () {
            if ('parseTree' in that) {
                cb(null, uglify.uglify.gen_code(that.parseTree, {beautify: that.isPretty}));
            } else {
                            ...
                            args[1][1].forEach(function (keyValue) {
                                if (/^(?:mixins|requires|extends|uses)$/.test(keyValue[0])) {
                                    extractStringNodes(keyValue[1], 'Invalid Ext.define.' + keyValue[0] + ' syntax').forEach(function (stringNode) {
                                        originalRelations.push(new relations.JavaScriptExtJsRequire({
                                            from: that,
