...
for (const urlPropertyName of ['username', 'password', 'host', 'hostname', 'pathname', 'port', 'protocol']) {
    Object.defineProperty(Asset.prototype, urlPropertyName, {
        get() {
            const url = this.url;
            if (url) {
                if (urlPropertyName === 'pathname' && url.startsWith(this.assetGraph.root)) {
                    return url.substr(this.assetGraph.root.length - 1);
                }
        ...
        set(value) {
            ...
            if (url) {
                if (urlPropertyName === 'pathname' && url.startsWith(this.assetGraph.root)) {
                    this.url = this.assetGraph.root + value.replace(/^\//, '');
                } else {
