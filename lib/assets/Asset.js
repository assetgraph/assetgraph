...
/** @typedef {import('../relations/Relation').RelationConfig} RelationConfig */
 ...
 *
 *
 * @typedef {Object<string, any>} AssetConfig
 *
 ...
 * @property {string} [text] The raw source of the asset
 *
 ...
 *
 * @property {string} [id]
 * @property {Number} [lastKnownByteLength]
 * @property {string} [encoding]
 * @property {object} [parseTree]
...
class Asset extends EventEmitter {
    ...
    try {
      if (!this.isLoaded) {
        ...
        if (protocol === 'file') {
          ...
          /** @typedef {import('http').IncomingMessage} HttpResponse */
          const response = /** @type {HttpResponse} */ (await this.assetGraph.teepee.request(
            {
              ...requestOptions,
              headers: {
                ...headers,
  ...
  /**
   * @param {Relation | RelationConfig} relationConfig
   * @return {Relation}
   */
  _vivifyRelation(relationConfig) {
    relationConfig.from = this;
    /** @type {Relation} */
    let vivifiedRelation;
    if ('isRelation' in relationConfig) {
      vivifiedRelation = relationConfig;
    } else {
      const originalHref = relationConfig.href;
      if (typeof originalHref === 'string') {
        relationConfig.to = { url: originalHref };
        delete relationConfig.href;
      }
      const type = relationConfig.type;
      delete relationConfig.type;

      vivifiedRelation = new AssetGraph[type](relationConfig);
      // Make sure that we preserve the href of relations that maintain it as a direct property
      // rather than a getter/setter:
      if (typeof originalHref === 'string' && !('href' in vivifiedRelation)) {
        vivifiedRelation.href = originalHref;
      }
      if (vivifiedRelation.to && this.assetGraph) {
        if (/^#/.test(vivifiedRelation.to.url)) {
          // Self-reference, only fragment
          // Might need refinement for eg. url(#foo) in SvgStyle in inline Svg
          vivifiedRelation._to = this;
        } else {
          // Implicitly create the target asset:
          vivifiedRelation._to = this.assetGraph.addAsset(
            vivifiedRelation.to,
            vivifiedRelation
          );
        }
   ...
   * @param {'first' | 'last' | 'before' | 'after'} [position='last']
   ...
   *
   * @return {RelationConfig[]} The Assets outgoing Relations
   */
  ...
  set query(query) {
    ...
    if (typeof query === 'string') {
      queryObj = qs.parse(query.replace(/^\?/, ''));
    } else {
      queryObj = query;
    }
    this._updateQueryString(queryObj);
  }
...
Asset.prototype.isResolved = true;

Asset.prototype.isRedirect = false;
 ...
 * @default true
 */
Asset.prototype.isExternalizable = true;
 ...
 */
Asset.prototype.contentType = 'application/octet-stream';
 ...
 * @type {string[]}
 */
Asset.prototype.supportedExtensions = [];
