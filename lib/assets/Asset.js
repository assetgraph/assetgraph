...
class Asset extends EventEmitter {
    ...
    get type() {
        if (this._type === 'Asset') {
            ...
            if (typeFromExtension && (!commonTargetType || AssetGraph[commonTargetType].prototype['is' + commonTargetType])) {
                return typeFromExtension;
            } else if (commonTargetType) {
                return commonTargetType;
            }
        }
        return this._type;
    }
            ...
            if (!Class || Class === AssetGraph.Image || Class === AssetGraph.Text || Class === AssetGraph.Font) {
                ...
                if (commonTargetType) {
                    if (Class && !AssetGraph[commonTargetType].prototype['is' + Class.prototype.type]) {
                        this.assetGraph.warn(`${this.urlOrDescription} used as both ${Class.prototype.type} and ${commonTargetType}`);
                    } else {
