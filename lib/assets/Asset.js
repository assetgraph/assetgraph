...
class Asset extends EventEmitter {
    ...
    init(config = {}) {
        ...
        extendDefined(this, _.omit(config, ['rawSrc', 'parseTree', 'text', 'encoding', 'sourceMap', 'lastKnownByteLength', 'url', 'fileName', 'extension', 'outgoingRelations']));
        if (!this.id) {
            this.id = '' + _.uniqueId();
        }
    }
    ...
    async load() {
        if (!this.isLoaded) {
            ...
            if (protocol === 'javascript') {
                this.url = undefined;
                this._text = url.replace(/^javascript:/i, '');
        ...
        if (!Class && (this._rawSrc || typeof this._text === 'string') && !this.hasOwnProperty('contentType')) {
    ...
    addRelation(relation, position, adjacentRelation) {
        ...
        if (relation.to) {
            ...
            const nonInlineAncestor = this.nonInlineAncestor;
            relation.to = this.assetGraph.add(relation.to, nonInlineAncestor && nonInlineAncestor.url);
        } else {
            // Shouldn't necessarily be prohibited if there's an href?
            throw new Error('addRelation: \'to\' property of relation is missing');
        }
        if (!relation.isRelation) {
            relation = new this.assetGraph.constructor[relation.type](relation);
        }

        position = position || 'last';
        if (typeof relation.node === 'undefined' && relation.attach) {
            relation.attach(this, position, adjacentRelation);
        } else {
            relation.addToOutgoingRelations(this, position, adjacentRelation);
            relation.refreshHref();
        }
            ...
            for (const outgoingRelation of this.outgoingRelations) {
                ...
                if (typeof relativeUrl === 'string') {
                    ...
                    if (/^data:/.test(relativeUrl)) {
                        let assetConfig = resolveDataUrl(relativeUrl);
                        if (assetConfig) {
                            ...
                            // data: url could not be parsed, leave an Asset instance in the graph
                            assetConfig = { url: relativeUrl };
                        }
                        outgoingRelation.to = this.assetGraph.createAsset(assetConfig, fromUrl);
                        if (!outgoingRelation.to.assetGraph) {
                            this.assetGraph.addAsset(outgoingRelation.to);
                        }
                    } else {
                        const baseUrl = fromUrl || outgoingRelation.baseUrl;
                        if (baseUrl) {
                            const targetUrl = this.assetGraph.resolveUrl(baseUrl, relativeUrl);
                            const targetAssets = this.assetGraph.findAssets({url: targetUrl});
                            ...
                            if (targetAssets.length) {
                                ...
                                outgoingRelation.to = this.assetGraph.createAsset(outgoingRelation.to, baseUrl);
                                this.assetGraph.addAsset(outgoingRelation.to);
                            }
                        }
                    }
