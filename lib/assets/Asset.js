...
class Asset extends EventEmitter {
    ...
    get rootPath() {
        const assetGraphRoot = this.assetGraph.root;
        const url = this.url;
        if (url) {
            if (url.indexOf(assetGraphRoot) === 0) {
                return url.substr(assetGraphRoot.length - 1);
            } else {
    ...
    set rootPath(rootPath) {
        ...
        if (url) {
            if (url.indexOf(assetGraphRoot) === 0) {
                this.url = assetGraphRoot + rootPath.replace(/^\//, '');
            } else {
...
for (const urlPropertyName of ['username', 'password', 'host', 'hostname', 'pathname', 'port', 'protocol']) {
