...
class Asset extends EventEmitter {
    ...
    async load() {
        try {
            ...
            if (!Class || Class === AssetGraph.Image || Class === AssetGraph.Text) {
            ...
            if (this.url) {
                ...
                if (type && (!Class || (Class === AssetGraph.Image && AssetGraph[type].prototype.isImage) || (Class === AssetGraph.Text && AssetGraph[type].prototype.isText))) {
            ...
            if ((!Class || Class === AssetGraph.Image) && (this._rawSrc || typeof this._text === 'string') && !this.hasOwnProperty('contentType')) {
                ...
                if (detectedContentType) {
                    ...
                    if (typeFromDetectedContentType && typeFromDetectedContentType !== 'Asset') {
                        if (!Class || (Class === AssetGraph.Image && AssetGraph[typeFromDetectedContentType].prototype.isImage)) {
    ...
    externalize() {
        ...
        let url;
        do {
            url = this.assetGraph.root + baseName + (nextSuffixToTry ? '-' + nextSuffixToTry : '') + extension;
            nextSuffixToTry += 1;
        } while (this.assetGraph.findAssets({ url }).length > 0);
        this.url = url;
    }
        ...
        if (!outgoingRelation.isRelation) {
            ...
            if (outgoingRelation.to && this.assetGraph) {
                ...
                if (targetType && !outgoingRelation.to._isCompatibleWith(targetType)) {
                    this.assetGraph.warn(new Error(`${targetType} ${outgoingRelation.to.type}`));
                }
            }
        }
    ...
    _isCompatibleWith(Class) {
        ...
        return this instanceof Class || Class.prototype instanceof AssetGraph[this.type] ||
            !!(this.isImage && Class === AssetGraph.Image); // Svg is modelled as a subclass of Xml
    }
