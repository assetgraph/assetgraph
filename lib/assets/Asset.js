...
extendWithGettersAndSetters(Asset.prototype, {
    ...
    set url(url) {
        ...
        if (url && !/^[a-z\+]+:/.test(url)) {
            // Non-absolute
            var baseUrl = oldUrl || (this.assetGraph && this.baseAsset && this.baseAsset.url) || (this.assetGraph && this.assetGraph.root);
            if (baseUrl) {
                if (/^\/\//.test(url)) {
                    // Protocol-relative
                ...
                if (!outgoingRelation.assetGraph) {
                    if (outgoingRelation.to.url || typeof outgoingRelation.to === 'string') {
                        ...
                        if (/^data:/.test(relativeUrl)) {
                            ...
                            if (assetConfig) {
                                assetConfig.type = this.assetGraph.constructor.assets.lookupContentType(assetConfig.contentType);
                                outgoingRelation.to = this.assetGraph.constructor.assets.create(assetConfig);
                                this.assetGraph.addAsset(outgoingRelation.to);
                            }
                        } else {
                            var baseAssetUrl = outgoingRelation.baseAsset.url,
                                targetUrl;
