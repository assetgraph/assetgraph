...
extendWithGettersAndSetters(Asset.prototype, {
    ...
    _loadWithoutPopulating: function () {
        var that = this;
        if (that.isLoaded) {
            ...
            if (url) {
                ...
                if (protocol === 'javascript') {
                    ...
                    return that.assetGraph.teepee.request(_.defaults({ url: url, json: false }, that.assetGraph.requestOptions)).then(function (response) {
            ...
            return Promise.reject(new Error('Asset.load: No rawSrc or rawSrcProxy found, cannot load'));
        }
    },
