...
function Asset(config) {
    ...
    // Avoid being re-initialized after Asset#upgrade has run:
    if (!this._hasInitialized) {
        ...
        if (config.outgoingRelations) {
            this._outgoingRelations = config.outgoingRelations.map((outgoingRelation) => {
...
extendWithGettersAndSetters(Asset.prototype, {
     ...
     */
    async load() {
        await this._loadWithoutPopulating();
        this.emit('load', this);
        if (this.assetGraph) {
            this.populate();
        }
    ...
    async _loadWithoutPopulating() {
        if (this.isLoaded) {
            return;
        }
        if (!this.url) {
            throw new Error('Asset.load: No rawSrc or rawSrcProxy found, cannot load');
        }
        ...
        if (protocol === 'javascript') {
            this.url = undefined;
            this.upgrade(AssetGraph.JavaScript, {
                text: url.replace(/^javascript:/i, '')
            });
        } else if (protocol === 'data') {
            const resolvedDataUrl = resolveDataUrl(url);
            if (resolvedDataUrl) {
                this.url = undefined;
                extendDefined(this, resolvedDataUrl);
            } else {
                this.assetGraph.emit('warn', new errors.ParseError({
                    message: 'Cannot parse data url: ' + this.url,
                    asset: this
                }));
            }
            ...
            let rawSrc;
            try {
                rawSrc = await fs.readFileAsync(pathname);
            } catch (err) {
                if (err.code === 'EISDIR' || err.errno === constants.EISDIR || err.code === 'EINVAL' || err.errno === constants.EINVAL) {
                    rawSrc = await fs.readFileAsync(pathname.replace(/(\/)?$/, '/index.html'));
                    this._rawSrc = rawSrc;
                    this.url = url.replace(/(\/)?((?:[?#].*$)|$)/, '/index.html$2');
            ...
            const err = new Error(['No resolver found for protocol: ' + protocol, '\tIf you think this protocol should exist, please contribute it here:', '\thttps://github.com/Munter/schemes#contributing'].join('\n'));
            if (this.assetGraph) {
    ...
    // Try to upgrade from Asset to a subclass based on the provided Class or this.contentType
    upgrade(Class, constructorOptions) {
        if (!Class && this.contentType) {
            const typeFromContentType = AssetGraph.typeByContentType[this.contentType];
        ...
        if (Class) {
            ...
            Class.call(this, constructorOptions || {});
        }
    },
    ...
    get dataUrl() {
        if (this.isText) {
            const text = this.text,
                urlEncodedText = encodeURIComponent(text).replace(/%2C/g, ','),
    ...
    unload: function () {
        if (this._outgoingRelations) {
            ...
            if (this.assetGraph && this.isPopulated) {
                [].concat(this.outgoingRelations).forEach((outgoingRelation) => {
    ...
    set url(url) {
        ...
        if (url !== oldUrl) {
            ...
            if (this.assetGraph) {
                this.assetGraph.findRelations({to: this}).forEach((incomingRelation) => {
                    incomingRelation.refreshHref();
                });
                this.externalRelations.forEach(function (relation) {
    ...
    get isRedirect() {
        return typeof this.statusCode === 'number' && this.statusCode >= 301 && this.statusCode <= 303 && this.location !== undefined;
    },
    ...
    populate: function () {
        ...
        if (this.isLoaded && !this.keepUnpopulated && !this.isPopulated) {
            this.outgoingRelations.forEach((outgoingRelation) => {
                let relativeUrl;
                ...
                if (typeof relativeUrl === 'string') {
                    ...
                    if (/^data:/.test(relativeUrl)) {
                        const assetConfig = resolveDataUrl(relativeUrl);
                        if (assetConfig) {
                        ...
                        const baseUrl = outgoingRelation.baseUrl;
                        if (baseUrl) {
    ...
    replaceWith: function (newAsset) {
        ...
        this.incomingRelations.forEach((incomingRelation) => {
