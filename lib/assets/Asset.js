...
extendWithGettersAndSetters(Asset.prototype, {
        ...
        if (that.isLoaded) {
            ...
            if (url) {
                ...
                if (protocol === 'javascript') {
                    ...
                    if (resolvedDataUrl) {
                        that.url = undefined;
                        extendDefined(that, resolvedDataUrl);
                    } else {
                        that.assetGraph.emit('warn', new errors.ParseError({
                            message: 'Cannot parse data url: ' + that.url,
                            asset: that
                        }));
                    }
                    ...
                    try {
                        ...
                        if (err.code === 'EISDIR' || err.errno === constants.EISDIR || err.code === 'EINVAL' || err.errno === constants.EINVAL) {
                            ...
                            that._rawSrc = rawSrc;
                            that.url = url.replace(/(\/)?((?:[?#].*$)|$)/, '/index.html$2');
                    ...
                    const response = await that.assetGraph.teepee.request(_.defaults({ url: url, json: false }, that.assetGraph.requestOptions));
                    that.statusCode = response.statusCode;
                    that._rawSrc = response.body;
                    if (response.headers.location) {
                        that.location = response.headers.location;
                    }
                    var contentType = response.headers['content-type'];
                    if (contentType) {
                        var matchContentType = contentType.match(/^\s*([\w\-\+\.]+\/[\w\-\+\.]+)(?:\s|;|$)/i);
                        if (matchContentType) {
                            that.contentType = matchContentType[1].toLowerCase();
                            var matchCharset = contentType.match(/;\s*charset\s*=\s*(['"]|)\s*([\w\-]+)\s*\1(?:\s|;|$)/i);
                            if (matchCharset) {
                                that._encoding = matchCharset[2].toLowerCase();
                            }
                        }
                    }
                    if (response.headers.etag) {
                        that.etag = response.headers.etag;
                    }
                    if (response.headers['cache-control']) {
                        that.cacheControl = response.headers['cache-control'];
                    }
                    if (response.headers['content-security-policy']) {
                        that.contentSecurityPolicy = response.headers['content-security-policy'];
                    }
                    if (response.headers['content-security-policy-report-only']) {
                        that.contentSecurityPolicyReportOnly = response.headers['content-security-policy-report-only'];
                    }
                    ['date', 'last-modified'].forEach(function (headerName) {
                        if (response.headers[headerName]) {
                            that[headerName.replace(/-([a-z])/, function ($0, ch) {
    ...
    upgrade: function (Class, constructorOptions) {
        Object.setPrototypeOf(this, Class.prototype);
        Class.call(this, constructorOptions || {});
    },

    get isLoaded() {
        return typeof this._rawSrc !== 'undefined' || typeof this._parseTree !== 'undefined' || typeof this._text === 'string';
    },
    ...
    unload: function () {
        if (this._outgoingRelations) {
            ...
            if (this.assetGraph && this.isPopulated) {
                [].concat(this.outgoingRelations).forEach(function (outgoingRelation) {
