...
extendWithGettersAndSetters(Asset.prototype, {
     ...
     * asset.
     *
     * @return {Relation} The asset itself (chaining-friendly).
     * @api public
     */
    markDirty: function () {
        this.isDirty = true;
    ...
    get outgoingRelations() {
        if (!this.assetGraph) {
            throw new Error("Asset.outgoingRelations getter: Asset is not part of an AssetGraph");
        }
        return this.assetGraph.findRelations({from: this});
    },
    ...
    /**
     * asset.createOriginalRelations()
     * ===============================
     *
     * Traverses the parse tree of the asset and creates a relation
     * object for each outgoing relation found.
     *
     * @return {Array[Relation]} The found relations.
     * @api public
     */
    ...
    /**
     * asset.populateRelationsToExistingAssets()
     * =========================================
     *
     ...
     * helper for the `clone` method, but also used by the
     * `compressJavaScript` transform.
     *
     * @param {Asset} asset The asset.
     * @api public
     */
    populateRelationsToExistingAssets: function () {
        if (!this.assetGraph) {
            throw new Error("Asset.populateRelationsToExistingAssets: Asset is not part of an AssetGraph");
        }
        this.createOriginalRelations().forEach(function (outgoingRelation) {
            var relativeUrl;
            if (outgoingRelation.to.url || typeof outgoingRelation.to === 'string') {
                ...
                if (targetUrl in this.assetGraph.urlIndex) {
                    outgoingRelation.to = this.assetGraph.urlIndex[targetUrl];
                    this.assetGraph.addRelation(outgoingRelation);
                }
            } else {
                // Inline asset
                outgoingRelation.to = this.assetGraph.constructor.assets.create(outgoingRelation.to);
                this.assetGraph.addAsset(outgoingRelation.to);
                this.assetGraph.addRelation(outgoingRelation);
                outgoingRelation.to.populateRelationsToExistingAssets();
            }
        }, this);
    },
    ...
    replaceWith: function (newAsset) {
        ...
        this.incomingRelations.forEach(function (incomingRelation) {
            incomingRelation.updateTarget(newAsset);
        }, this);
     ...
     * @api public
     */
    clone: function (incomingRelations) {
            ...
            if (incomingRelations) {
                ...
                incomingRelations.forEach(function (incomingRelation) {
                    ...
                    if (incomingRelation.id in this.assetGraph.idIndex) {
                        incomingRelation.updateTarget(clone);
                    } else {
                        incomingRelation.to = clone;
                        this.assetGraph.addRelation(incomingRelation);
                        incomingRelation.refreshHref();
                    }
                }, this);
            }
            clone.populateRelationsToExistingAssets();
        }
        return clone;
    },
