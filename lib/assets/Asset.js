...
class Asset extends EventEmitter {
  ...
  _inferType(incomingRelation) {
    ...
    if (
      ...
      (!type ||
        (AssetGraph[typeFromExtension].prototype['is' + type] &&
      ...
      if (!this.isLoaded) {
        ...
        if (protocol === 'file') {
          ...
          if (contentType) {
            ...
            if (matchContentType) {
              const contentType = matchContentType[1].toLowerCase();
              const typeFromContentType =
                AssetGraph.typeByContentType[contentType];
              if (
                typeFromContentType &&
                !this._isCompatibleWith(typeFromContentType, contentType)
              ) {
                this._warnIncompatibleTypes([this.type, typeFromContentType]);
              }
  ...
  // contentType is optional, but can be passed when the determination of Class was based on a definitely known Content-Type
  _isCompatibleWith(Class, contentType) {
    if (typeof Class === 'undefined') {
      Class = Asset;
    } else if (typeof Class === 'string') {
      Class = this.assetGraph[Class];
    }
    if (contentType === 'text/plain' && this.type && this.type !== 'Text') {
