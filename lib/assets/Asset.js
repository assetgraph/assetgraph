 ...
 *
 * @property {Buffer} [config.rawSrc] `Buffer` object containing the raw source of the asset
 *
 * @property {String} [config.contentType] The Content-Type (MIME type) of the asset. For
 * subclasses of Asset there will be a reasonable default.
 *
 * @property {String} [config.url] The fully qualified (absolute) url of the asset.
 ...
 * @property {String} [config.baseName]
 * @property {String} [config.extension]
 * @property {String} [config.protocol]
 * @property {String} [config.username]
 * @property {String} [config.password]
 * @property {String} [config.hostname]
...
class Asset extends EventEmitter {
  ...
  _warnIncompatibleTypes(incompatibleTypes) {
    ...
    const errorString = `used as both ${[...incompatibleTypes]
      .sort()
      .join(' and ')}`;
    if (!this._issuedWarnings[errorString]) {
      this._issuedWarnings[errorString] = true;
      this.assetGraph.warn(
        new Error(`${this.urlOrDescription} ${errorString}`)
      );
    }
  }
  ...
  _inferType(incomingRelation) {
    ...
    if (this.hasOwnProperty('contentType')) {
      const typeFromContentType = AssetGraph.lookupContentType(
        this.contentType
      );
      ...
      for (const incomingRelation of incomingRelations) {
        if (incomingRelation.targetType) {
          if (!commonTargetType) {
            ...
            commonTargetType !== incomingRelation.targetType &&
            !AssetGraph[commonTargetType].prototype[
              'is' + incomingRelation.targetType
            ] &&
            !AssetGraph[incomingRelation.targetType].prototype[
              'is' + commonTargetType
            ]
          ) {
    ...
    if (
      typeFromExtension &&
      (!type || AssetGraph[typeFromExtension].prototype['is' + type])
    ) {
        ...
        if (protocol === 'file') {
          ...
          if (contentType) {
            ...
            if (matchContentType) {
              ...
              const matchCharset = contentType.match(
    ...
    if (!outgoingRelation.isRelation) {
      ...
      if (outgoingRelation.to && this.assetGraph) {
        if (/^#/.test(outgoingRelation.to.url)) {
          ...
          if (
            targetType &&
            !outgoingRelation.to._isCompatibleWith(targetType)
          ) {
            outgoingRelation.to._warnIncompatibleTypes([
              targetType,
              outgoingRelation.to.type
            ]);
          }
        }
      }
    ...
    if (incomingRelations) {
      ...
      for (const incomingRelation of incomingRelations) {
        if (!incomingRelation || !incomingRelation.isRelation) {
          throw new Error(
            'asset.clone(): Incoming relation is not a relation: ' +
              incomingRelation.toString()
          );
        }
  ...
  _isCompatibleWith(Class) {
    ...
    return (
      ...
      !this._type ||
      Class.prototype instanceof AssetGraph[this._type] ||
      !!(this.isImage && Class === AssetGraph.Image) || // Svg is modelled as a subclass of Xml, not Image
      !!(this.isImage && Class === AssetGraph.Font)
    ); // Svg can be used as a font as well
  }
