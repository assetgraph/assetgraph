...
extendWithGettersAndSetters(Asset.prototype, {
    ...
    load: function () {
        ...
        if (that.isLoaded) {
            ...
            return that.rawSrcProxy().spread(function (rawSrc, metadata) {
                ...
                if (metadata) {
                    ...
                    Object.keys(metadata).forEach(function (key) {
                        if (typeof metadata[key] !== 'undefined') {
                            // Might change contentType and encoding, and could add etag, lastModified, and date
        ...
        if (url !== oldUrl) {
            ...
            if (this.assetGraph) {
                ...
                this.externalRelations.forEach(function (relation) {
                    if (relation.to && !relation.to.isResolved) {
                        var matchFragmentInCurrentUrl = relation.to.url.match(/^([^#]*)(#.*)?$/);
                        if (matchFragmentInCurrentUrl) {
                            var urlWithoutFragmentIdentifier = matchFragmentInCurrentUrl[1];
                            var currentFragmentIdentifier = matchFragmentInCurrentUrl[2] || '';
                            if (urlWithoutFragmentIdentifier === oldUrl) {
                                relation.to.url = (url || '') + currentFragmentIdentifier;
                            }
                        }
                    }
