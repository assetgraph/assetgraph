...
class Asset extends EventEmitter {
    ...
    init(config = {}) {
        ...
        if (config.url) {
            ...
            for (const propertyName of ['baseName', 'extension', 'protocol', 'username', 'password', 'hostname', 'port']) {
                if (typeof config[propertyName] !== 'undefined' && typeof this[`_${propertyName}`] === 'undefined') {
                    this[`_${propertyName}`] = config[propertyName];
                }
            }
            if (typeof config.path !== 'undefined' && typeof this._path === 'undefined') {
                this._path = config.path;
            }
            extendDefined(this, _.omit(config, ['rawSrc', 'parseTree', 'text', 'encoding', 'sourceMap', 'lastKnownByteLength', 'url', 'fileName', 'extension', 'protocol', 'username', 'password', 'hostname', 'port', 'path']));
        }
        if (!this.id) {
            this.id = '' + _.uniqueId();
        }
    ...
    _warnIncompatibleTypes(incompatibleTypes) {
        this._issuedWarnings = this._issuedWarnings || {};
        const errorString = `used as both ${[...incompatibleTypes].sort().join(' and ')}`;
        if (!this._issuedWarnings[errorString]) {
            this._issuedWarnings[errorString] = true;
            this.assetGraph.warn(new Error(`${this.urlOrDescription} ${errorString}`));
        }
    }
    ...
    _inferType(incomingRelation) {
        ...
        if (this.hasOwnProperty('contentType')) {
            ...
            if (typeFromContentType) {
                type = typeFromContentType;
            }
        }
        if (!type || type === 'Image' || type === 'Text' || type === 'Font') {
            ...
            for (const incomingRelation of incomingRelations) {
                if (incomingRelation.targetType) {
                    ...
                    } else if (commonTargetType !== incomingRelation.targetType && !AssetGraph[commonTargetType].prototype['is' + incomingRelation.targetType]) {
                        this._warnIncompatibleTypes([commonTargetType, incomingRelation.targetType]);
            ...
            if (commonTargetType) {
                if (type && !AssetGraph[commonTargetType].prototype['is' + type]) {
                    this._warnIncompatibleTypes([type, commonTargetType]);
                } else {
                    type = commonTargetType;
                }
            }
        }
        const typeFromExtension = this.url && AssetGraph.typeByExtension[pathModule.extname(this.url.replace(/[\?\#].*$/, '')).toLowerCase()];
        if (typeFromExtension && (!type || AssetGraph[typeFromExtension].prototype['is' + type])) {
    ...
    _tryUpgrade(type, incomingRelation) {
        ...
        if (Class) {
            ...
            if (Class.prototype.type !== this.type && Class.prototype['is' + this.type]) {
                this._upgrade(Class);
            } else {
    ...
    async load() {
        try {
            ...
            if ((!type || type === 'Image') && (this._rawSrc || typeof this._text === 'string') && !this.hasOwnProperty('contentType')) {
                ...
                if (detectedContentType) {
                    ...
                    if (typeFromDetectedContentType) {
                        if (!type || (type === 'Image' && AssetGraph[typeFromDetectedContentType].prototype.isImage) || (type === 'Font' && AssetGraph[typeFromDetectedContentType].prototype.isFont)) {
    ...
    get nonInlineAncestor() {
        ...
        if (this.incomingInlineRelation && this.incomingInlineRelation.from !== this) {
            return this.incomingInlineRelation.from.nonInlineAncestor;
        } else if (this.assetGraph) {
            const incomingRelations = this.incomingRelations.filter(r => r.from !== r._to);
    ...
    set path(value) {
        ...
        if (url) {
            if (url.startsWith(this.assetGraph.root)) {
                this.url = this.assetGraph.root + value.replace(/^\//, '').replace(/\/?$/, '/') + (this.fileName || '');
            } else {
    ...
    externalize(baseUrl) {
        ...
        if (typeof this._path === 'string') {
            urlObj = new urlModule.URL(this.assetGraph.root + this._path.replace(/^\//, ''));
        } else {
            urlObj = new urlModule.URL(this.assetGraph.root);
        }
        for (const urlPropertyName of ['username', 'password', 'hostname', 'port', 'protocol']) {
        ...
        let url;
        do {
            urlObj.pathname = pathnamePrefix + baseName + (nextSuffixToTry ? '-' + nextSuffixToTry : '') + extension;
            url = urlObj.toString();
            nextSuffixToTry += 1;
            ...
            if (newUrl) {
                if (this.assetGraph._urlIndex[newUrl]) {
                    if (this.assetGraph._urlIndex[newUrl] !== this) {
                        throw new Error(`${newUrl} already exists in the graph, cannot update url`);
                    }
                } else {
            ...
            if (outgoingRelation.to && this.assetGraph) {
                if (/^#/.test(outgoingRelation.to.url)) {
                    ...
                    if (targetType && !outgoingRelation.to._isCompatibleWith(targetType)) {
                        outgoingRelation.to._warnIncompatibleTypes([targetType, outgoingRelation.to.type]);
                    }
                }
            }
        }
    ...
    get externalRelations() {
        ...
        (function gatherExternalRelations(asset) {
            if (asset.keepUnpopulated || !asset.isLoaded || seenAssets.has(asset)) {
                return;
            }
    ...
    /**
     * asset.incomingRelations (setter)
     * ================================
     *
    ...
    _updateQueryString(obj) {
        const url = this.url;
        if (url) {
            const urlObj = urlModule.parse(this.url);
            ...
            throw new Error('Cannot update the query of an inline asset');
        }
    }
    ...
    _isCompatibleWith(Class) {
        ...
        return this instanceof Class || !this._type || Class.prototype instanceof AssetGraph[this._type] ||
            !!(this.isImage && Class === AssetGraph.Image) || // Svg is modelled as a subclass of Xml, not Image
            !!(this.isImage && Class === AssetGraph.Font); // Svg can be used as a font as well
    }
    ...
    Object.defineProperty(Asset.prototype, urlPropertyName, {
        ...
        set(value) {
            ...
            if (url) {
                ...
                throw new Error(`Cannot update the ${urlPropertyName} of an inline asset`);
            }
        }
    });
}
