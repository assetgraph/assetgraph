...
extendWithGettersAndSetters(Asset.prototype, {
     ...
     */
    toString: function () {
        return '[' + this.type + '/' + this.id + (this.url ? ' /' + path.relative(this.assetGraph.root, this.url) : '') + ']';
    },

    get urlOrDescription() {
        return this.url || ('inline ' + this.type + (this.nonInlineAncestor ? ' in /' + path.relative(this.assetGraph.root, this.nonInlineAncestor.url) : ''));
    }
});
