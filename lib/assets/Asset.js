...
extendWithGettersAndSetters(Asset.prototype, {
     ...
     * incoming relations, ie. the first asset that has an
        ...
        if (this.isInline) {
            if (this.assetGraph) {
                ...
                if (incomingRelations.length > 0) {
                    return incomingRelations[0].from.nonInlineParent;
                }
            }
            return null;
        } else {
     ...
     */
    get md5Hex() {
        return crypto.createHash('md5').update(this.rawSrc).digest('hex');
    },
    ...
    set url(url) {
        if (url !== this._url) {
            ...
            if (this.assetGraph) {
                ...
                if (oldUrl) {
                    delete this.assetGraph.urlIndex[oldUrl];
                }
                if (url) {
                    this.assetGraph.urlIndex[url] = this;
                }
