...
extendWithGettersAndSetters(Text.prototype, {
        ...
        if (!this._rawSrc) {
            ...
            if ('_text' in this || this._parseTree) {
                if (/^utf-?8$/i.test(this.encoding)) {
                    ...
                    error = new Error('node-iconv not found. Cannot encode ' + this + ' as ' + this.encoding + '. ' +
                                    'Please run \'npm install iconv\' and try again');
                    error.asset = this;

                    if (this.assetGraph) {
                        this.assetGraph.emit('error', error);
                    } else {
                        throw error;
                    }
                }
    ...
    _getTextFromRawSrc: function () {
        ...
        if (/^utf-?8$/i.test(this.encoding)) {
            ...
            var error = new Error('node-iconv not found. Cannot decode ' + this + ' (encoding is ' + this.encoding + '). ' +
                            'Please run `npm install iconv` and try again');
            error.asset = this;
