            ...
            if ('_text' in this || this._parseTree) {
                if (/^utf-?8$/i.test(this.encoding)) {
                    ...
                    try {
                        ...
                        if (err.code === 'EILSEQ') {
                            this.assetgraph.emit('error', 'iconv: Illegal character sequence in ' + this.url + '\n       Conversion: utf-8 --> ' + this.encoding + '\n       Trying to transliterate, ignoring character on failure.');
                            this._rawSrc = new iconv.Iconv('utf-8', this.encoding + '//TRANSLIT//IGNORE').convert(this.text);
                        } else {
                            err.message = 'iconv: Converting ' + this.url + ' failed:\n' + err.message;
