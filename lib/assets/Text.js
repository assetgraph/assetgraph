...
extendWithGettersAndSetters(Text.prototype, {
    ...
    _getTextFromRawSrc: function () {
        if (!this.isLoaded) {
            throw new Error("assets.Text._getTextFromRawSrc(): Asset not loaded: " + this.urlOrDescription);
        }
        if (!this._rawSrc) {
            throw new Error("assets.Text._getTextFromRawSrc(): No _rawSrc property found: " + this.urlOrDescription);
        }
        if (/^utf-?8$/i.test(this.encoding)) {
