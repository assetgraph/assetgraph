...
extendWithGettersAndSetters(Html.prototype, {
    ...
    get parseTree() {
        if (!this._parseTree) {
            ...
            if (isFragment) {
                this._parseTree = document.createDocumentFragment();
                Array.prototype.slice.call(document.body.childNodes).forEach(function (node) {
                    this._parseTree.appendChild(node);
                }, this);
                // Install the properties of document on the HtmlBodyElement used as the parse tree:
                for (var propertyName in document) {
                    if (!(propertyName in this._parseTree) && propertyName !== 'head') {
                        if (typeof document[propertyName] === 'function') {
                            this._parseTree[propertyName] = document[propertyName].bind(document);
                        } else {
                            this._parseTree[propertyName] = document[propertyName];
                        }
                    }
                }
            } else {
            ...
            if (node.nodeType === node.ELEMENT_NODE) {
                ...
                if (nodeName === 'script') {
                    ...
                    if (node.hasAttribute('rel')) {
                        ...
                        if (/(?:^| )stylesheet(?:$| )/i.test(rel) && this._isRelationUrl(node.getAttribute('href'))) {
                            addOutgoingRelation(new AssetGraph.HtmlStyle({
                                from: this,
                                to: {
                ...
                } else if (/^(?:source|track)$/i.test(nodeName) && node.parentNode && /^(?:video|audio)$/i.test(node.parentNode.nodeName)) {
                    if (this._isRelationUrl(node.getAttribute('src'))) {
