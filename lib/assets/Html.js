...
extendWithGettersAndSetters(Html.prototype, {
    ...
    get internalText() {
        if (!('_text' in this)) {
            if (this._parseTree) {
                this._text = jsdom.serializeDocument(this.parseTree);
            } else {
    ...
    get parseTree() {
        if (!this._parseTree) {
            ...
            try {
                // Compensate for jsdom creating <html><head></head><body>...</body> around the document if text === '':
                if (this.isFragment) {
                    text = '<html><head></head><body>' + text + '</body></html>';
                }
                this._parseTree = jsdom.jsdom(text, {
                    features: {
        ...
        return this.isFragment ? this._parseTree && this._parseTree.body : this._parseTree;
    },
    ...
    get isFragment() {
        if (typeof this._isFragment === 'undefined' && this.isLoaded) {
            this._isFragment =  !/^\s*<!DOCTYPE|^\s*<html/i.test(this.text);
        }
