...
extendWithGettersAndSetters(Html.prototype, {
    ...
    _reformatParseTree: function (removeWhiteSpace, removeComments) {
        ...
        while (q.length) {
            var node = q.shift();
            if (node.nodeType === node.COMMENT_NODE && /^\s*\/?ko\s/.test(node.nodeValue)) {
                // Knockout.js containerless binding start or end marker. Remove superfluous whitespace if removeWhiteSpace is true:
                if (removeWhiteSpace) {
                    node.nodeValue = node.nodeValue.replace(/^\s+|\s+$/g, '');
                }
            } else if ((removeComments && node.nodeType === node.COMMENT_NODE && !/^#|^\[if|^<!\[endif\]/.test(node.nodeValue)) ||
                       (removeWhiteSpace && node.nodeType === node.TEXT_NODE && /^[\r\n\t ]*$/.test(node.nodeValue))) {

                // Non-conditional comment or pure-whitespace text node, remove:
                ...
                if (node.nodeType === node.ELEMENT_NODE) {
                    ...
                    if (hasTextChild) {
                        ...
                        var hasPreParent = false,
                            hasDataI18nParent = false,
                            n = node;
                        while (n) {
                            if (n.nodeName.toLowerCase() === 'pre') {
                                hasPreParent = true;
                            }
                            if (n.nodeType === n.ELEMENT_NODE && n.hasAttribute('data-i18n')) {
                                hasDataI18nParent = true;
                            }
                            n = n.parentNode;
                        }
                        if (!hasPreParent) {
                            ...
                            for (var i = 0 ; i < node.childNodes.length ; i += 1) {
                                if (node.childNodes[i].nodeType === node.TEXT_NODE) {
                                    var childNode = node.childNodes[i],
                                        childNodeValue = childNode.nodeValue.replace(/[\r\n\t ]+/g, ' '),
                                        previousSibling = childNode.previousSibling,
                                        nextSibling = childNode.nextSibling;
                                    if (!hasDataI18nParent) {
                                        if (!previousSibling || previousSibling.nodeType !== childNode.ELEMENT_NODE || !previousSibling.hasAttribute('data-i18n')) {
                                            childNodeValue = childNodeValue.replace(/^[\r\n\t ]+/, '');
                                        }
                                        if (!nextSibling || nextSibling.nodeType !== childNode.ELEMENT_NODE || !nextSibling.hasAttribute('data-i18n')) {
                                            childNodeValue = childNodeValue.replace(/[\r\n\t ]+$/, '');
                                        }
                                    }
