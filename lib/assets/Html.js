...
class Html extends Text {
    ...
    if (!this._parseTree) {
      ...
      try {
        const jsdom = new JSDOM(
          isFragment ? '<body>' + text + '</body>' : text,
          {
            virtualConsole,
  ...
  findOutgoingRelationsInParseTree() {
        ...
        if (matchNonInternetExplorerConditionalComment) {
          ...
          if (currentConditionalComments.length > 0) {
            ...
            const warning = new errors.SyntaxError({
              message:
                'Html: Conditional comment end marker seen without a start marker: ' +
                node.nodeValue,
              asset: this
            });
            if (this.assetGraph) {
          ...
          if (matchConditionalComment) {
            addOutgoingRelation({
              ...
              to: {
                ...
                text:
                  '<!--ASSETGRAPH DOCUMENT START MARKER-->' +
    ...
    if (currentConditionalComments.length > 0) {
      const warning = new errors.SyntaxError({
        message:
          'Html: No end marker found for conditional comment(s):\n' +
