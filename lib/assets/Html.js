...
extendWithGettersAndSetters(Html.prototype, {
    ...
    getRelations: function () {
        ...
        if (parseTree.documentElement && parseTree.documentElement.hasAttribute('manifest')) {
            _relations.push(new relations.HtmlCacheManifest({
                from: this,
            ...
            if (node.nodeType === node.ELEMENT_NODE) {
                ...
                if (nodeName === 'script') {
                    if (node.hasAttribute('src')) {
                        _relations.push(new relations.HtmlScript({
                            from: this,
                    ...
                    _relations.push(new relations.HtmlStyle({
                        from: this,
                        to: {
                            type: 'Css',
                            isResolved: true,
                    ...
                    if (node.hasAttribute('rel')) {
                        ...
                        if (rel === 'stylesheet') {
                            ...
                            _relations.push(new relations.HtmlShortcutIcon({
                                from: this,
                                to: node.getAttribute('href'),
                                node: node
                            }));
                        } else if (rel === 'alternate') {
                            _relations.push(new relations.HtmlAlternateLink({
                                from: this,
                                to: {
                    ...
                    _relations.push(new relations.HtmlImage({
                        from: this,
                        to: node.getAttribute('src'),
                        node: node
                    }));
                    ...
                    if (!/^mailto:/i.test(href) && !/^#/.test(href)) {
                        _relations.push(new relations.HtmlAnchor({
                            from: this,
                            to: href,
                            node: node
                        }));
                    ...
                    _relations.push(new relations.HtmlIFrame({
                        from: this,
                        to: node.getAttribute('src'),
                        node: node
                    }));
                } else if (nodeName === 'frame') {
                    _relations.push(new relations.HtmlFrame({
                        from: this,
                        to: node.getAttribute('src'),
                        node: node
                    }));
                } else if (nodeName === 'esi:include') {
                    _relations.push(new relations.HtmlEdgeSideInclude({
                        from: this,
                        to: node.getAttribute('src'),
                        node: node
                    }));
                    ...
                    if (node.hasAttribute('src')) {
                        _relations.push(new relations[nodeName === 'video' ? 'HtmlVideo' : 'HtmlAudio']({
                            from: this,
                            to: node.getAttribute('src'),
                            node: node
                        }));
                    ...
                    if (node.hasAttribute('poster')) {
                        _relations.push(new relations.HtmlVideoPoster({
                            from: this,
                    ...
                    _relations.push(new relations.HtmlAudio({
                        from: this,
                        to: node.getAttribute('src'),
                        node: node
                    }));
                } else if (/^(?:source|track)$/.test(nodeName) && node.parentNode && /^(?:video|audio)$/i.test(node.parentNode.nodeName)) {
                    _relations.push(new relations[node.parentNode.nodeName.toLowerCase() === 'video' ? 'HtmlVideo' : 'HtmlAudio']({
                        from: this,
                        to: node.getAttribute('src'),
                        node: node
                    }));
                } else if (nodeName === 'object' && node.hasAttribute('data')) {
                    _relations.push(new relations.HtmlObject({
                        from: this,
                    ...
                    ['archive', 'codebase'].forEach(function (attributeName) {
                        ...
                        if (node.hasAttribute(attributeName)) {
                            _relations.push(new relations.HtmlApplet({
                                from: this,
                    ...
                    _relations.push(new relations.HtmlEmbed({
                        from: this,
                        to: node.getAttribute('src'),
                        node: node
                    }));
                ...
                if (matchConditionalComment) {
                    _relations.push(new relations.HtmlConditionalComment({
                        from: this,
                        to: {
                            type: 'Html',
                            isResolved: true,
