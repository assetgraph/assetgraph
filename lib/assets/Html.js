    ...
    while (queue.length > 0) {
      ...
      if (node.nodeType === node.ELEMENT_NODE) {
        ...
        if (nodeName === 'script') {
          ...
          const srcAttributeValue = node.getAttribute('src') || node.getAttribute('data-src');
          const srcSetAttributeValue = node.getAttribute('srcset') || node.getAttribute('data-srcset');
