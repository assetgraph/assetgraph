...
function isRelationUrl(url) {
    return url && !/^javascript:|^mailto:|^\s*$|^#/.test(url);
}
            ...
            if (node.nodeType === node.ELEMENT_NODE) {
                ...
                if (nodeName === 'script') {
                    ...
                    if (node.hasAttribute('rel')) {
                        ...
                        if (/\bstylesheet(?:\/less)?\b/.test(rel) && isRelationUrl(node.getAttribute('href'))) {
                            outgoingRelations.push(new relations.HtmlStyle({
                                from: this,
                                to: {
                        ...
                        } else if (/\b(?:apple-touch-)?icon\b/.test(rel)) { // Also catches rel="shortcut icon"
                            if (isRelationUrl(node.getAttribute('href'))) {
                        ...
                        } else if (/\bapple-touch-startup-image\b/.test(rel)) {
                            if (isRelationUrl(node.getAttribute('href'))) {
                    ...
                    if (node.hasAttribute('poster') && isRelationUrl(node.getAttribute('poster'))) {
                    ...
                    if (isRelationUrl(node.getAttribute('data'))) {
                    ...
                    if (isRelationUrl(node.getAttribute('value'))) {
                        outgoingRelations.push(new relations.HtmlObject({
                            from: this,
                            to: {
                    ...
                    ['archive', 'codebase'].forEach(function (attributeName) {
                        ...
                        if (isRelationUrl(node.getAttribute(attributeName))) {
