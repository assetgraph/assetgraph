...
extendWithGettersAndSetters(Html.prototype, {
    ...
    findOutgoingRelationsInParseTree: function () {
        ...
        if (this.parseTree.documentElement && this.parseTree.documentElement.hasAttribute('manifest')) {
            addOutgoingRelation(new relations.HtmlCacheManifest({
                from: this,
                to: {
        ...
        while (queue.length) {
            ...
            if (node.nodeType === node.ELEMENT_NODE) {
                ...
                if (nodeName === 'script') {
                    ...
                    if (!type || type === 'text/javascript' || type === 'text/coffeescript') {
                        ...
                        if (src) {
                            if (this._isRelationUrl(src)) {
                                addOutgoingRelation(new relations.HtmlScript({
                                    from: this,
                                    to: {
                                        url: src
                                    },
                        ...
                        if (node.hasAttribute('data-main')) {
                            ...
                            addOutgoingRelation(new relations.HtmlRequireJsMain({
                                from: this,
                                to: {
                                    url: url
                                },
                        ...
                        addOutgoingRelation(new relations.HtmlInlineScriptTemplate({
                            from: this,
                            to: new Html({
                        ...
                        addOutgoingRelation(new relations.HtmlInlineScriptTemplate({
                            from: this,
                            to: new (require('./AngularJsTemplate'))({
                                isExternalizable: false,
                                text: text
                            }),
                            node: node
                    ...
                    addOutgoingRelation(new relations.HtmlStyle({
                        from: this,
                        to: new (require('./Css'))({
                            text: node.firstChild ? node.firstChild.nodeValue : ''
                        }),
                    ...
                    if (node.hasAttribute('rel')) {
                        ...
                        if (/(?:^| )stylesheet(?:\/less)?(?:$| )/i.test(rel) && this._isRelationUrl(node.getAttribute('href'))) {
                            ...
                            if (this._isRelationUrl(node.getAttribute('href'))) {
                                addOutgoingRelation(new relations.HtmlShortcutIcon({
                                    from: this,
                                    to: {
                                        url: node.getAttribute('href')
                                    },
                            ...
                            if (this._isRelationUrl(node.getAttribute('href'))) {
                                addOutgoingRelation(new relations.HtmlAppleTouchStartupImage({
                                    from: this,
                                    to: {
                                        url: node.getAttribute('href')
                                    },
                            ...
                            if (this._isRelationUrl(node.getAttribute('href'))) {
                                ...
                                addOutgoingRelation(new relations.HtmlAlternateLink({
                                    from: this,
                                    to: assetConfig,
                                    node: node
                                }));
                            ...
                            if (this._isRelationUrl(node.getAttribute('href'))) {
                                ...
                                addOutgoingRelation(new relations.HtmlSearchLink({
                                    from: this,
                                    to: assetConfig,
                                    node: node
                                }));
                            ...
                            if (this._isRelationUrl(node.getAttribute('href'))) {
                                ...
                                addOutgoingRelation(new relations.HtmlFluidIconLink({
                                    from: this,
                                    to: assetConfig,
                                    node: node
                                }));
                    ...
                    if (this._isRelationUrl(node.getAttribute('content'))) {
                        ...
                        addOutgoingRelation(new relations.HtmlMsApplicationTileImageMeta({
                            from: this,
                            to: assetConfig,
                            node: node
                        }));
                    ...
                    if (srcAttributeValue && this._isRelationUrl(srcAttributeValue)) {
                        addOutgoingRelation(new relations.HtmlImage({
                            from: this,
                            to: {
                    ...
                    if (srcSetAttributeValue && this._isRelationUrl(srcSetAttributeValue)) {
                        addOutgoingRelation(new relations.HtmlImageSrcSet({
                            from: this,
                    ...
                    if (this._isRelationUrl(href)) {
                        addOutgoingRelation(new relations.HtmlAnchor({
                            from: this,
                            to: {
                                url: href
                            },
                    ...
                    if (this._isRelationUrl(node.getAttribute('src'))) {
                        addOutgoingRelation(new relations.HtmlIFrame({
                            from: this,
                            to: {
                    ...
                    if (node.hasAttribute('srcdoc')) {
                        addOutgoingRelation(new relations.HtmlIFrameSrcDoc({
                            from: this,
                            to: new Html({
                    ...
                    if (this._isRelationUrl(node.getAttribute('src'))) {
                        addOutgoingRelation(new relations.HtmlFrame({
                            from: this,
                            to: {
                                url: node.getAttribute('src')
                            },
                    ...
                    if (this._isRelationUrl(node.getAttribute('src'))) {
                        addOutgoingRelation(new relations.HtmlEdgeSideInclude({
                            from: this,
                            to: {
                                url: node.getAttribute('src')
                            },
                    ...
                    if (node.hasAttribute('src')) {
                        if (this._isRelationUrl(node.getAttribute('src'))) {
                            addOutgoingRelation(new relations[nodeName === 'video' ? 'HtmlVideo' : 'HtmlAudio']({
                                from: this,
                                to: {
                                    url: node.getAttribute('src')
                                },
                    ...
                    if (node.hasAttribute('poster') && this._isRelationUrl(node.getAttribute('poster'))) {
                        addOutgoingRelation(new relations.HtmlVideoPoster({
                            from: this,
                            to: {
                    ...
                    if (this._isRelationUrl(node.getAttribute('src'))) {
                        addOutgoingRelation(new relations.HtmlAudio({
                            from: this,
                            to: {
                                url: node.getAttribute('src')
                            },
                    ...
                    if (this._isRelationUrl(node.getAttribute('src'))) {
                        addOutgoingRelation(new relations[node.parentNode.nodeName.toLowerCase() === 'video' ? 'HtmlVideo' : 'HtmlAudio']({
                            from: this,
                            to: {
                                url: node.getAttribute('src')
                            },
                    ...
                    if (this._isRelationUrl(node.getAttribute('data'))) {
                        addOutgoingRelation(new relations.HtmlObject({
                            from: this,
                            to: {
                    ...
                    ['archive', 'codebase'].forEach(function (attributeName) {
                        ...
                        if (this._isRelationUrl(node.getAttribute(attributeName))) {
                            addOutgoingRelation(new relations.HtmlApplet({
                                from: this,
                                to: {
                    ...
                    if (this._isRelationUrl(node.getAttribute('src'))) {
                        addOutgoingRelation(new relations.HtmlEmbed({
                            from: this,
                            to: {
                                url: node.getAttribute('src')
                            },
                ...
                if (node.hasAttribute('style')) {
                    addOutgoingRelation(new relations.HtmlStyleAttribute({
                        from: this,
                        to: new (require('./Css'))({
                            isExternalizable: false,
                ...
                if (node.hasAttribute('data-bind')) {
                    addOutgoingRelation(new relations.HtmlDataBindAttribute({
                        from: this,
                        to: new (require('./JavaScript'))({
                            isExternalizable: false,
                            quoteChar: "'", // Prefer single quotes for consistency with HtmlDataBindAttribute
                            text: '({' + node.getAttribute('data-bind').replace(/^\s*\{(.*)\}\s*$/, '$1') + '});'
                        }),
                        node: node
                    }));
                }
                ...
                if (matchNonInternetExplorerConditionalComment) {
                    ...
                    if (currentConditionalComments.length > 0) {
                        ...
                        var warning = new errors.SyntaxError({message: "assets.Html: Conditional comment end marker seen without a start marker: " + node.nodeValue, asset: this});
                        if (this.assetGraph) {
                    ...
                    if (matchConditionalComment) {
                        addOutgoingRelation(new relations.HtmlConditionalComment({
                            from: this,
                            to: new Html({
                        ...
                        if (matchKnockoutContainerless) {
                            addOutgoingRelation(new relations.HtmlKnockoutContainerless({
                                from: this,
                                to: new (require('./JavaScript'))({
                                    isExternalizable: false,
        ...
        if (currentConditionalComments.length > 0) {
            var warning = new errors.SyntaxError({message: "assets.Html: No end marker found for conditional comment(s):\n" + _.pluck(currentConditionalComments, 'nodeValue').join('\n  '), asset: this});
            if (this.assetGraph) {
                this.assetGraph.emit('warn', warning);
            } else {
                console.warn(this.toString() + ': ' + warning.message);
        ...
        (function processNode(node, isWithinSensitiveTag, canRemoveLeadingWhiteSpace, canRemoveTrailingWhiteSpace) {
            ...
            for (var i = 0 ; i < node.childNodes.length ; i += 1) {
                ...
                if (childNode.nodeType === childNode.ELEMENT_NODE) {
                    ...
                    } else if (removeComments && !/^ASSETGRAPH DOCUMENT (?:START|END) MARKER$|^#|^\[if|^<!\[endif\]/.test(childNode.nodeValue)) {
