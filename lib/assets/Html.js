...
class Html extends Text {
    ...
    _createSourceMapForInlineScriptOrStylesheet(element) {
        ...
        const sourceMapGenerator = new mozilla.SourceMapGenerator({file: nonInlineAncestor.url});
        ...
        while (queue.length > 0) {
            ...
            if (node.nodeType === node.ELEMENT_NODE) {
                ...
                if (nodeName === 'script') {
                    ...
                    if (!type || type === 'text/javascript') {
                        ...
                        if (href) {
                            ...
                            addOutgoingRelation({
                                ...
                                to: {
                                    type: 'JavaScript',
                                    sourceMap: this.nonInlineAncestor && this._createSourceMapForInlineScriptOrStylesheet(node),
                    ...
                    addOutgoingRelation({
                        type: node.parentNode.nodeName.toLowerCase() === 'video' ? 'HtmlVideo' : 'HtmlAudio',
                        href: node.getAttribute('src'),
                        node
                    });
