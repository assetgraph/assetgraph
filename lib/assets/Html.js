...
class Html extends Text {
  ...
  get baseUrl() {
    ...
    // Memoize the href of <base> (null if none)
    if (typeof this._baseElementHref === 'undefined') {
      const baseElement = this.parseTree.querySelector('base');
      this._baseElementHref = (baseElement && baseElement.href) || null;
    }
    if (this._baseElementHref) {
      baseUrl = this.assetGraph.resolveUrl(baseUrl, this._baseElementHref);
    }
    return baseUrl;
  }
        ...
        for (const propertyName in document) {
          if (!(propertyName in document.body) && propertyName !== 'head') {
            if (typeof document[propertyName] === 'function') {
              document.body[propertyName] =
                document[propertyName].bind(document);
            } else {
      ...
      if (node.nodeType === node.ELEMENT_NODE) {
        ...
        if (nodeName === 'script') {
            ...
            if (href) {
              ...
              addOutgoingRelation({
                ...
                to: {
                  type: 'JavaScript',
                  sourceMap:
                    this._createSourceMapForInlineScriptOrStylesheet(node),
          ...
          if (node.hasAttribute('rel') && node.hasAttribute('href')) {
            ...
            if (/(?:^| )stylesheet(?:$| )/i.test(rel)) {
              ...
              /(?:^| )(?:apple-touch-icon(?:-precomposed)?|(?:mask-)?icon)(?:$| )/i.test(
                rel
              )
            ) {
        ...
        for (const attributeName of ['data-bind', 'params']) {
          if (node.hasAttribute(attributeName)) {
            ...
            try {
              parseTree = parseJavaScript(javaScriptObjectLiteral, {
                sourceType: 'module',
              });
            } catch (e) {}
        ...
        if (matchNonInternetExplorerConditionalComment) {
          ...
          if (matchConditionalComment) {
            ...
            const matchKnockoutContainerless =
              node.nodeValue.match(/^\s*ko\s+([\s\S]+)$/);
