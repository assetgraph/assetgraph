                    ...
                    if (!type || type === 'text/javascript' || type === 'text/coffeescript') {
                        ...
                        if (node.hasAttribute('data-main')) {
                            ...
                            if (this.assetGraph && this.assetGraph.requireJsConfig) {
                                var requireJsConfig = this.assetGraph.requireJsConfig;

                                if (requireJsConfig.baseUrl) {
                                    // baseUrl has already been configured. data-main must take this into account.
                                    url = this.assetGraph.resolveUrl(requireJsConfig.baseUrl, url);
                                } else {
                                    // baseUrl is undefined. data-main is an implicit baseUrl setter
                                    requireJsConfig.baseUrl = this.assetGraph.resolveUrl(this.nonInlineAncestor.url, url).replace(/[^\/]*$/, '');
                                }
                            }
