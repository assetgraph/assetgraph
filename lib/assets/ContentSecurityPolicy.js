...
class ContentSecurityPolicy extends Text {
    ...
    allows(directive, urlOrToken, protectedResourceUrl) {
        ...
        if (sourceExpressions.length === 0) {
            ...
            return sourceExpressions.some(
                sourceExpression => matchSourceExpression(urlOrToken, sourceExpression, protectedResourceUrl || this.nonInlineAncestor.url)
            );
        }
    }
}
...
ContentSecurityPolicy.directiveFallsBackToDefaultSrc = directive => !!fallsBackToDefaultSrcByDirective[directive];
