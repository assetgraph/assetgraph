...
i18nTools.extractAllKeysForLocaleFromHTMLAsset = function (assetGraph, localeId, htmlAsset, cb) {
    i18nTools.getBootstrappedContext(assetGraph, htmlAsset, error.passToFunction(cb, function (context) {
        var allKeysForLocale = {},
            i18nKeysObj = context.one && context.one.i18nKeys;
        if (i18nKeysObj) {
            ...
            _.keys(i18nKeysObj).forEach(function (key) {
                ...
                for (var i = 0 ; i < prioritizedLocaleIds.length ; i += 1) {
                    if (prioritizedLocaleIds[i] in i18nKeysObj[key]) {
                        allKeysForLocale[key] = i18nKeysObj[key][prioritizedLocaleIds[i]];
                        found = true;
                        break;
                    }
                }
