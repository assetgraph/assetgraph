    ...
    fs.mkdir(dir, permissions, function (err) {
        ...
        if (err && err.errno === constants.ENOENT) {
            ...
            if (parentDir !== '/' && parentDir !== '') {
                fsTools.mkpath(parentDir, permissions, error.passToFunction(cb, function () {
                    fs.mkdir(dir, permissions, function (err) {
                        if (!err || err.errno === constants.EEXIST) {
                            // Success!
                            return cb();
...
fsTools.mkpathAndWriteFile = function mkpathAndWriteFile(fileName, contents, encoding, cb) {
    fs.writeFile(fileName, contents, encoding, function (err) {
        if (err && err.errno === constants.ENOENT) {
            fsTools.mkpath(Path.dirname(fileName), error.passToFunction(cb, function () {
