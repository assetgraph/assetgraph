...
function extractQuotes(quotes, token) {
  ...
  quotes.replace(/"((?:[^"\\]|\\.)*)"|'((?:[^'\\]|\\.)*)'/g, ($0, doubleQuotedString, singleQuotedString) => {
    if (
  ...
  tokens.forEach(token => {
    if (token.type === 'string') {
      ...
      if (counterRendererByListStyleType[token.value]) {
        ...
        var counterStyles = [];
        Object.keys(hypotheticalCounterStyleByName).forEach(counterStyleName => {
          counterStyles.push({
...
function extractTextFromContentPropertyValue(
  ...
  var usesQuotes = tokens.some(token => token.type === 'open-quote' || token.type === 'close-quote');
  function expandCounterStyles(quotes) {
    ...
    if (referencedCounterStyleNames.length > 0) {
      ...
      expandPermutations(
      ...
      ).forEach(hypotheticalCounterStyleByName => {
        ...
        if (content) {
          var combinedPredicates = combinePredicates(
            Object.keys(hypotheticalCounterStyleByName).map(counterStyleName => hypotheticalCounterStyleByName[counterStyleName]
              .predicates)
          );
  ...
  if (usesQuotes) {
    var result = [];
    hypotheticalQuotesValues.forEach(hypotheticalQuotesValue => {
      var expandedContentValues = expandCounterStyles(
        hypotheticalQuotesValue.value
      );
      expandedContentValues.forEach(value => {
