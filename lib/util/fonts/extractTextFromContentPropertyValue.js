  ...
  tokens.forEach(function(token) {
    if (token.type === 'string') {
      ...
      if (counterRendererByListStyleType[token.value]) {
        ...
        var counterStyles = [];
        Object.keys(hypotheticalCounterStyleByName).forEach(function(
          counterStyleName
        ) {
          counterStyles.push({
...
function extractTextFromContentPropertyValue(
  ...
  function expandCounterStyles(quotes) {
    ...
    if (referencedCounterStyleNames.length > 0) {
      ...
      expandPermutations(
      ...
      ).forEach(function(hypotheticalCounterStyleByName) {
        ...
        if (content) {
          var combinedPredicates = combinePredicates(
            Object.keys(hypotheticalCounterStyleByName).map(function(
              counterStyleName
            ) {
              return hypotheticalCounterStyleByName[counterStyleName]
                .predicates;
            })
          );
