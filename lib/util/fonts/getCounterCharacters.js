...
function getCounterCharacters(counterStyle, counterStyles, counterValue) {
    if (Array.isArray(counterValue)) {
        return counterValue.map(function (counterValue) {
        ...
        if (counterRendererByListStyleType[fallback]) {
            ...
            counterStyles.forEach(function (counterStyle) {
                if (counterStyle.name === fallback) {
                    text += getCounterCharacters(counterStyle, counterStyles, counterValue);
                }
            });
            return text;
        }
    ...
    if (counterStyle.props.range && counterStyle.props.range !== 'auto' && !isWithinRange(counterValue, counterStyle.props.range)) {
    ...
    if (system === 'cyclic') {
        if (counterValue >= 1) {
            text += textBySymbolNumber[(counterValue - 1) % textBySymbolNumber.length];
        } else {
            text += renderFallback();
        }
    } else if (system === 'symbolic') {
        if (counterValue >= 1) {
            var numRepetitions = 1 + Math.floor((counterValue - 1) / textBySymbolNumber.length);
            text += textBySymbolNumber[(counterValue - 1) % textBySymbolNumber.length].repeat(numRepetitions);
        } else {
            text += renderFallback();
        }
        ...
        if (remainder >= 1) {
            var rendered = '';
            while (remainder > 0) {
                rendered = textBySymbolNumber[(remainder - 1) % textBySymbolNumber.length] + rendered;
        ...
        var tokens = [];
        (counterStyle.props['additive-symbols'] || '').replace(/(\d+)\s+(?:(url\(\s*(?:'(?:[^']|\\')*'|"(?:[^"]|\\")*"|(?:[^'"\\]|\\.)*?\s*)\))|"((?:[^"\\]|\\.)*)"|'((?:[^'\\]|\\.)*)'|([^'",]+))/g, function ($0, value, url, doubleQuotedString, singleQuotedString, other) {
            var text = '';
            if (typeof doubleQuotedString === 'string') {
    ...
    if (typeof padValue === 'string') {
        padValue.replace(/\d+ (?:"((?:[^"\\]|\\.)*)"|'((?:[^'\\]|\\.)*)'|(url\(\s*(?:'(?:[^']|\\')*'|"(?:[^"]|\\")*"|(?:[^'"\\]|\\.)*?\s*)\)))/g, function ($0, doubleQuotedString, singleQuotedString) {
            if (typeof doubleQuotedString === 'string') {
