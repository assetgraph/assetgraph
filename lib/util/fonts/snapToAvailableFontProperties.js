...
function snapToAvailableFontProperties(fontFaceDeclarations, propsToSnap) {
    ...
    fontFaceDeclarations = fontFaceDeclarations.map(fontFaceDeclaration => ({ ...INITIAL_VALUES, ...fontFaceDeclaration }));
    ...
    const familyMatches = fontFaceDeclarations.filter(fontFaceDeclaration => fontFaceDeclaration['font-family'] === fontFamilies[0]);
    ...
    const stretchStartIndex = fontStretchValues.indexOf(propsToSnap['font-stretch']);
    ...
    const styleMatches = styleLookupOrder[propsToSnap['font-style']]
        .map(style => stretchMatches.filter(stretchMatch => stretchMatch['font-style'] === style))
        .find(list => list.length > 0);


    // Find the best font-weight
    const desiredWeight = propsToSnap['font-weight'];
    const availableFontWeights = styleMatches.map(m => m['font-weight']).sort(ascending);
    let resolvedWeight;
    ...
    if (typeof desiredWeight === 'string') {
        ...
        const startWeight = resolveFontWeight(operations.shift(), availableFontWeights);
    ...
    return styleMatches.find(styleMatch => styleMatch['font-weight'] === resolvedWeight);
}
