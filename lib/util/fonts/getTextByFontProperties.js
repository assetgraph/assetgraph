...
function getTextByFontProperties(htmlAsset, memoizedGetCssRulesByProperty) {
    ...
    function adjustPossibleCountersAndListItemNumbers(computedStyle) {
        ...
        if (numHypotheticalListItems > 0) {
            if (numHypotheticalListItems === computedStyle.display.length) {
            ...
            if (node.tagName === 'INPUT' && visualValueInputTypes.indexOf(node.type || 'text') !== -1) {
                ...
                ['before', 'after'].forEach(function (pseudoElementName) {
                    ...
                    if (computedStyle.content.length > 0) {
                        styledTexts.push(adjustPossibleCountersAndListItemNumbers(computedStyle));
                    }
                });
                ...
                for (var i = 0 ; i < node.childNodes.length ; i += 1) {
                    ...
                    if (child.nodeType === 1 && excludedNodes.indexOf(child.tagName) === -1) {
                        traversePreOrder(child, idArray.concat(i));
                    }
                }
