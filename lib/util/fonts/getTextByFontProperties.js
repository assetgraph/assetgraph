...
function getFontRulesWithDefaultStylesheetApplied(htmlAsset, memoizedGetCssRulesByProperty) {
    var fontPropRules = [{ text: defaultStylesheet, predicates: {} }].concat(gatherStylesheetsWithIncomingMedia(htmlAsset.assetGraph, htmlAsset))
    ...
    var getComputedStyle = memoizeSync(function (node, idArray, truePredicates, falsePredicates) {
        ...
        function traceProp(prop, startIndex, truePredicates, falsePredicates) {
            ...
            for (var i = startIndex; i < localFontPropRules[prop].length; i += 1) {
                ...
                if (!INHERITED[prop] && !hasPseudoElement && Object.keys(truePredicates).some(function (predicate) { return /^pseudoElement:/.test(predicate); })) {
                    continue;
                }
...
function getTextByFontProperties(htmlAsset, memoizedGetCssRulesByProperty) {
    ...
    function adjustPossibleCountersAndListItemNumbers(computedStyle, isWithinConditionalComment) {
        ...
        if (numHypotheticalListItems > 0) {
            if (numHypotheticalListItems === computedStyle.display.length && !isWithinConditionalComment) {
            ...
            if (node.tagName === 'INPUT' && visualValueInputTypes.indexOf(node.type || 'text') !== -1) {
                ...
                ['before', 'after'].forEach(function (pseudoElementName) {
                    ...
                    if (computedStyle.content.length > 0) {
                        styledTexts.push(adjustPossibleCountersAndListItemNumbers(computedStyle, conditionalCommentStack.length > 0));
                    }
                });
