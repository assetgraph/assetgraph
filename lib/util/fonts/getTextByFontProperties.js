...
function getMemoizedElementStyleResolver(fontPropRules, memoizedGetCssRulesByProperty) {
    ...
    var getComputedStyle = memoizeSync(function (node, idArray, propsToTrace, truePredicates, falsePredicates) {
        ...
        if (!node.tagName) {
            propsToTrace.forEach(function (prop) {
        ...
        function traceProp(prop, startIndex, truePredicates, falsePredicates) {
            ...
            for (var i = startIndex; i < localFontPropRules[prop].length; i += 1) {
                ...
                if (isStyleAttribute || node.matches(strippedSelector)) {
                    ...
                    if (declaration.value === 'inherit' || declaration.value === 'unset') {
                        hypotheticalValues = getComputedStyle(node.parentNode, idArray.slice(0, -1), [prop], truePredicates, falsePredicates)[prop];
                    } else {
                        var value;
                        if (declaration.value === 'initial') {
                            ...
                            if (declaration.value === 'lighter' || declaration.value === 'bolder') {
                                var inheritedWeight = getComputedStyle(node.parentNode, idArray.slice(0, -1), [prop], truePredicates, falsePredicates)[prop][0].value;
            ...
            if (nonInheritingTags.indexOf(node.tagName) === -1) {
                return getComputedStyle(node.parentNode, idArray.slice(0, -1), [prop], truePredicates, falsePredicates)[prop];
            } else {
        ...
        argumentsStringifier: function (args) {
            return (
                args[1].join(',') + '\x1e' +
                args[2].join(',') + '\x1e' +
                (args[3] ? Object.keys(args[3]).join('\x1d') : '') + '\x1e' +
                (args[4] ? Object.keys(args[4]).join('\x1d') : '')
            );
        }
    });
...
function getTextByFontProperties(htmlAsset, memoizedGetCssRulesByProperty) {
    ...
    textNodes.forEach(function (textNodeObj) {
        styledTexts.push({
            ...
            props: getComputedStyle(textNodeObj.node.parentNode, textNodeObj.parentId, FONT_PROPS)
        });
    });
    ...
    nonTextnodes.forEach(function (nodeObj) {
        if (nodeObj.text) {
            styledTexts.push({
                text: nodeObj.text,
                props: getComputedStyle(nodeObj.node, nodeObj.id, FONT_PROPS)
            });
        }
        ['before', 'after'].forEach(function (pseudoElement) {
            var truePredicates = {};
            truePredicates['pseudoElement:' + pseudoElement] = true;
            var pseudoElementProps = getComputedStyle(nodeObj.node, nodeObj.id, FONT_PROPS_AND_CONTENT, truePredicates);
