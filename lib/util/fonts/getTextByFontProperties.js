...
var FONT_PROPS = ['font-family', 'font-style', 'font-weight'];
...
function createPredicatePermutations(predicatesToVary, predicates, i) {
  ...
  if (i < predicatesToVary.length) {
    var permutations = [];
    createPredicatePermutations(predicatesToVary, predicates, i + 1).forEach(
      permutation => {
...
function getFontRulesWithDefaultStylesheetApplied(
  ...
  var fontPropRules = defaultStylesheets
    .concat(gatherStylesheetsWithPredicates(htmlAsset.assetGraph, htmlAsset))
    .map(stylesheetAndIncomingMedia => memoizedGetCssRulesByProperty(
    ALL_PROPS_TO_TRACE,
  ...
  Object.keys(fontPropRules).forEach(prop => {
...
function getMemoizedElementStyleResolver(
  ...
  var getComputedStyle = memoizeSync(
    (node, idArray, pseudoElementName, parentTrace, predicates) => {
      ...
      if (!node.tagName) {
        ALL_PROPS_TO_TRACE.forEach(prop => {
          result[prop] = [
            { value: INITIAL_VALUES[prop], predicates: predicates }
          ];
        });
        return result;
      }
      ...
      if (node.getAttribute('style')) {
        var attributeStyles = memoizedGetCssRulesByProperty(
          ALL_PROPS_TO_TRACE,
        ...
        Object.keys(attributeStyles).forEach(prop => {
          if (attributeStyles[prop].length > 0) {
            var concatRules = attributeStyles[prop].concat(
      ...
      function traceProp(prop, startIndex, predicates) {
        ...
        for (var i = startIndex; i < localFontPropRules[prop].length; i += 1) {
          ...
          if (
            Object.keys(declaration.predicates).some(predicate => typeof predicates[predicate] === 'boolean' &&
          ...
          if (!isStyleAttribute) {
            var matchPseudoElement = strippedSelector.match(
          ...
          if (
            isStyleAttribute ||
            node.matches(strippedSelector.replace(/ i\]/, ']'))
          ) {
            // nwmatcher doesn't support the i flag in attribute selectors
            if (hasPseudoElement) {
            ...
            if (predicatesToVary.length > 0) {
              ...
              createPredicatePermutations(
                predicatesToVary,
                declaration.predicates
              ).forEach(predicatePermutation => {
                var predicatePermutationKeys = Object.keys(
                  predicatePermutation
                );
                if (predicatePermutationKeys.length === 0) {
                  return;
                }
                var predicatesForThisPermutation = combinePredicates([
                  predicates,
                  predicatePermutation
                ]);
                var predicatesOtherwise = combinePredicates([
                  predicates,
                  _.mapValues(predicatePermutation, value => !value)
                ]);
                if (
                  predicatesForThisPermutation &&
                  Object.keys(declaration.predicates).every(predicate => declaration.predicates[predicate] ===
...
function expandAnimations(computedStyle, keyframesDefinitions) {
  if (computedStyle['animation-name'].length > 0) {
    ...
    computedStyle['animation-name'].forEach(animationNameValue => {
    ...
    if (animatedPropertyNames.length > 0) {
      ...
      expandPermutations(computedStyle, animatedPropertyNames).forEach(permutation => {
        if (permutation['animation-name'].value !== 'none') {
          keyframesDefinitions.forEach(keyframesDefinition => {
            if (
              ...
              Object.keys(permutation).forEach(prop => {
              ...
              Object.keys(seenValuesByProp).forEach(prop => {
                var values = seenValuesByProp[prop];
                if (prop === 'font-weight') {
                  // https://drafts.csswg.org/css-transitions/#animtype-font-weight
                  var sortedValues = values
                    .map(value => cssFontWeightNames[value] || parseInt(value, 10))
                    .sort();
                  values = [];
                  for (
                    var fontWeight = sortedValues[0];
      ...
      Object.keys(extraValuesByProp).forEach(prop => {
        Array.prototype.push.apply(
          computedStyle[prop],
          extraValuesByProp[prop]
        );
...
function expandTransitions(computedStyle) {
  var fontWeightTransitions = computedStyle['transition-property'].filter(
    hypotheticalValue => /\b(?:font-weight|all)\b/.test(hypotheticalValue.value)
  );
  if (fontWeightTransitions.length > 0) {
    var hypotheticalFontWeightValuesInPseudoClassStates = computedStyle[
      'font-weight'
    ].filter(hypotheticalValue => Object.keys(hypotheticalValue.predicates).some(predicate => hypotheticalValue.predicates[predicate] &&
    /^selectorWithPseudoClasses:/.test(predicate)));
    if (hypotheticalFontWeightValuesInPseudoClassStates.length > 0) {
      var hypotheticalNonZeroTransitionDurations = computedStyle[
        'transition-duration'
      ].filter(hypotheticalValue => !/^\s*0s\s*(,\s*0s\s*)*$/.test(hypotheticalValue.value));
      if (hypotheticalNonZeroTransitionDurations.length > 0) {
        var extraHypotheticalFontWeightValues = [];
        hypotheticalNonZeroTransitionDurations.forEach(transitionDuration => {
          fontWeightTransitions.forEach(fontWeightTransition => {
            hypotheticalFontWeightValuesInPseudoClassStates.forEach(hypotheticalFontWeightValueInPseudoClassStates => {
              computedStyle['font-weight'].forEach(hypotheticalFontWeightValue => {
                var fontWeight1 =
                ...
                for (
                  var fontWeight = Math.min(fontWeight1, fontWeight2) + 100;
        ...
        if (extraHypotheticalFontWeightValues.length > 0) {
          ...
          computedStyle['font-weight'] = computedStyle['font-weight'].concat(
      ...
      for (var j = 0; j < computedStyle['list-style-type'].length; j += 1) {
        ...
        if (predicates) {
          if (/^['"]/.test(listStyleType)) {
            ...
            var found = false;
            counterStyles.forEach(counterStyle => {
...
function getTextByFontProperties(htmlAsset, memoizedGetCssRulesByProperty) {
  ...
  var fontPropRules = getFontRulesWithDefaultStylesheetApplied(
    htmlAsset,
    memoizedGetCssRulesByProperty
  );
  var getComputedStyle = getMemoizedElementStyleResolver(
    fontPropRules,
    memoizedGetCssRulesByProperty
  );

  var hypotheticalCounterStylesByName = {};
  fontPropRules.counterStyles.forEach(counterStyle => {
  ...
  function adjustPossibleCountersAndListItemNumbers(
    ...
    computedStyle.display.forEach(hypotheticalDisplayValue => {
      if (/\blist-item\b/.test(hypotheticalDisplayValue.value)) {
        numHypotheticalListItems += 1;
      }
    });
    var nextPossibleCounterValuesByName = {};
    ['counter-reset', 'counter-set'].forEach(propertyName => {
      var values = _.uniq(
        computedStyle[propertyName].map(hypotheticalCounterResetValue => hypotheticalCounterResetValue.value)
      );
      values.forEach(value => {
        ...
        Object.keys(valueByCounterName).forEach(counterName => {
          (possibleCounterValuesByName[counterName] || [0]).forEach(possibleCounterValue => {
        ...
        Object.keys(possibleCounterValuesByName).forEach(counterName => {
          if (!valueByCounterName[counterName]) {
            nextPossibleCounterValuesByName[counterName] = [].concat(
    ...
    var counterIncrementValues = _.uniq(
      computedStyle['counter-increment'].map(hypotheticalCounterIncrementValue => hypotheticalCounterIncrementValue.value)
    );

    var counterIncrementByName = {};
    counterIncrementValues.forEach(counterIncrementValue => {
    ...
    Object.keys(counterIncrementByName).forEach(counterName => {
      (possibleCounterValuesByName[counterName] || [0]).forEach(possibleCounterValue => {
        (nextPossibleCounterValuesByName[counterName] =
          nextPossibleCounterValuesByName[counterName] || []).push(
          counterIncrementByName[counterName] + possibleCounterValue
        );
      });
    });
    Object.keys(possibleCounterValuesByName).forEach(counterName => {
  ...
  function traceBeforeOrAfterPseudoElement(pseudoElementName, node, idArray) {
    ...
    if (computedStyle) {
      ...
      computedStyle.content.forEach(hypotheticalContent => {
        var hypotheticalValues = extractTextFromContentPropertyValue(
        ...
        hypotheticalValues.forEach(hypotheticalValue => {
      ...
      var styledText = adjustPossibleCountersAndListItemNumbers(
        computedStyle,
        conditionalCommentStack.length > 0 || noscriptStack.length > 0
      );
      styledText.text = styledText.text.filter(hypotheticalText => hypotheticalText.value.length > 0);
  ...
  function expandFirstLineAndFirstLetter(groupedStyledTexts, node, idArray) {
    ['first-line', 'first-letter'].forEach(pseudoElementName => {
      ...
      groupedStyledTexts.some(styledTextsInSection => {
        ...
        for (var i = 0; i < styledTextsInSection.length; i += 1) {
          ...
          var thisExhaustive = Object.keys(styledTextInSection).every(prop =>
          ...
          var pseudoElementStyle = getComputedStyle(
            node,
            idArray.slice(0, -1),
            pseudoElementName,
            styledTextInSection
          ...
          if (pseudoElementStyle) {
            styledTextInSection.text.forEach(hypotheticalValue => {
              var matchContent;
      ...
      Array.prototype.unshift.apply(
        groupedStyledTexts[0],
        additionalStyledTexts
      );
    });
  }
  ...
  (function traversePreOrder(node, idArray) {
    ...
    if (node.nodeType === node.TEXT_NODE) {
      ...
      excludedNodes.indexOf(node.tagName) === -1
    ) {
      if (!idArray) {
        idArray = [0];
      }
      ...
      if (node.tagName === 'NOSCRIPT') {
        ...
        node.tagName === 'INPUT' &&
        visualValueInputTypes.indexOf(node.type || 'text') !== -1
      ) {
        ...
        if (node.constructor.name === 'HTMLBRElement') {
          ...
          var beforeStyledTexts = traceBeforeOrAfterPseudoElement(
            'before',
            node,
            idArray
          );
          var afterStyledTexts = traceBeforeOrAfterPseudoElement(
            'after',
            node,
            idArray
          );

          var childTextNodeValues = _.flatten(
            [].slice
          ...
          if (childTextNodeValues.length > 0) {
            computedStyle['white-space'].forEach(hypotheticalValue => {
              var normalizedText = normalizeTextNodeValues(
                childTextNodeValues,
          ...
          Array.prototype.push.apply(
            styledTexts,
            _.flattenDeep(groupedStyledTexts)
          );
  ...
  var multipliedStyledTexts = _.flatten(
    styledTexts.map(styledText => expandPermutations(styledText)
        .filter(function removeImpossibleCombinations(
          hypotheticalValueByProp
        ) {
          return ALL_PROPS_TO_TRACE_AND_TEXT.every(prop => Object.keys(
            hypotheticalValueByProp[prop].predicates
          ).every(predicate => {
            var predicateValue =
              hypotheticalValueByProp[prop].predicates[predicate];
            return predicateValue === false ||
            ALL_PROPS_TO_TRACE_AND_TEXT.every(otherProp => hypotheticalValueByProp[otherProp].predicates[
              predicate
            ] !== false);
          }));
        })
        ...
        .filter(function filterAndDeduplicate(textWithProps) {
          ...
          var permutationKey = '';
          ['font-weight', 'font-style', 'font-family', 'text'].forEach(
            prop => {
