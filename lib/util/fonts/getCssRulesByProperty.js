  ...
  (function visit(node) {
    ...
    if (node.type === 'decl' && node.parent.selector) {
      if (properties.indexOf(node.prop) !== -1) {
        ...
        properties.indexOf('list-style-type') !== -1
      ) {
        // Shorthand
        var listStyleType;
        node.value.replace(
          /"((?:[^"]|\\.)*")|'((?:[^']|\\.)*)'|([^'"]+)/,
          function($0, doubleQuotedString, singleQuotedString, other) {
            if (typeof doubleQuotedString === 'string') {
              ...
              other
                .trim()
                .split(' ')
                .forEach(function(otherFragment) {
        ...
        node.prop === 'animation' &&
        properties.indexOf('animation-name') !== -1
      ) {
        // Shorthand
        ...
        specificity
          .calculate(node.parent.selector)
          .forEach(function(specificityObject) {
            Object.keys(fontProperties).forEach(function(prop) {
