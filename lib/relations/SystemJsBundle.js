...
extendWithGettersAndSetters(SystemJsBundle.prototype, {
    ...
    set href(href) {
        this.node.value = this.node.value.replace(/([@#]\s*SystemJsBundle=)[^\s]*/, '$1' + href);
    },
    ...
    detach: function () {
        this.node.value = this.node.value.replace(/#\s*SystemJsBundle=([^\s]*)/, '');
        if (/^\s*$/.test(this.node.value)) {
            this.parentNode[this.commentPropertyName].splice(this.parentNode[this.commentPropertyName].indexOf(this.node), 1);
        }
        this.node = null;
