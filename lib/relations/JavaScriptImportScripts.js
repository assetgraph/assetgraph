...
class JavaScriptImportScripts extends Relation {
  static get selectors() {
    return {
      'CallExpression:matches([callee=(Identifier[name=importScripts])], [callee=(MemberExpression[computed=false][object=(Identifier[name=self])][property=(Identifier[name=importScripts])])])'(
        node,
        stack
      ) {
        const parentNode = stack[0];
        const parentParentNode = stack[1];
        console.log({ parentNode, parentParentNode });
