...
extendWithGettersAndSetters(CssUrlTokenRelation.prototype, {
    ...
    get href() {
        ...
        return this.findUrlsInPropertyValue(this.cssRule.style.getPropertyValue(this.propertyName))[this.tokenNumber];
    },

    set href(href) {
        var cssUrlToken = this.createUrlToken(href),
            tokenNumber = 0;
        this.cssRule.style.setProperty(this.propertyName, this.cssRule.style.getPropertyValue(this.propertyName).replace(this.tokenRegExp, function ($0) {
            tokenNumber += 1;
    ...
    detach: function () {
        ...
        if (matchToken) {
            if (value === matchToken[0]) {
                ...
                var tokenNumber = 0;
                this.cssRule.style.setProperty(this.propertyName, value.replace(this.tokenRegExp, function ($0) {
                    tokenNumber += 1;
                    if (tokenNumber - 1 === this.tokenNumber) {
                        return '';
                    } else {
