...
class JavaScriptImport extends Relation {
    ...
    attach(position, adjacentRelation) {
        ...
        if (position === 'last') {
            ...
            if (otherOutgoingJavaScriptImports.length > 0) {
                position = 'after';
                adjacentRelation = otherOutgoingJavaScriptImports[otherOutgoingJavaScriptImports.length - 1];
            } else {
                position = 'first';
            }
        }
        ...
        if (position === 'before' || position === 'after') {
            ...
            if (i === -1) {
                throw new Error('JavaScriptImport#attach: adjacentRelation.node not found in adjacentRelation.from.parseTree.body');
            }
            this.from.parseTree.body.splice(i + (position === 'after' ? 1 : 0), 0, this.node);
        }
    ...
    detach() {
        ...
        if (i === -1) {
            throw new Error('relations.JavaScriptImport#detach: this.node not found in module array of this.from.parseTree.body.');
        }
