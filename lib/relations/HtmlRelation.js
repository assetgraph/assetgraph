...
class HtmlRelation extends Relation {
    ...
    attach(position, adjacentNodeOrRelation) {
        ...
        if (this.node === document.documentElement) {
            ...
            if (this.from.isFragment && (position === 'first' || position === 'last')) {
            ...
            if (position.startsWith('firstIn') || position.startsWith('lastIn')) {
                ...
                const headOrBody = !isBeforeOrAfter && position.endsWith('Body') ? 'body' : 'head';
                const firstOrLast = !isBeforeOrAfter && position.startsWith('first') ? 'first' : 'last';
                ...
                if (!headOrBodyNode) {
                    ...
                    if (!document.documentElement) {
                        const err = new Error(`HtmlRelation.attach: Could not attach asset ${this.to.toString()} to <${headOrBody}>. Missing <html> and <${headOrBody}>`);
                        err.asset = this.from;
                        throw err;
                    }
                    headOrBodyNode = document.createElement(headOrBody);
                    if (headOrBody === 'head') {
                        document.documentElement.insertBefore(headOrBodyNode, document.documentElement.firstChild);
                    } else {
                ...
                if (headOrBody === 'head') {
                    existingRelationsInSameSection = this.from.outgoingRelations.filter(
                        relation => relation !== this && relation.node.parentNode === document.head
                    );
                    if (existingRelationsInSameSection.length === 0) {
                        neighbouringRelationInOtherSection = this.from.outgoingRelations.find(
                            relation => relation !== this && (relation.node.matches ? relation.node.matches('body *') : relation.node.parentNode.matches('body *'))
                        );
                    }
                    ...
                    neighbouringRelationInOtherSection = headRelations[headRelations.length - 1];
                }
                ...
                if (firstOrLast === 'last') {
                    ...
                    if (existingRelationsInSameSection.length > 0) {
                        position = 'after';
                        adjacentRelation = existingRelationsInSameSection[existingRelationsInSameSection.length - 1];
                    } else if (neighbouringRelationInOtherSection) {
                        position = headOrBody === 'head' ? 'before' : 'after';
                        adjacentRelation = neighbouringRelationInOtherSection;
                    } else {
                ...
                if (adjacentNodeOrRelation.isRelation) {
                    ...
                    if (position === 'before') {
                        ...
                        adjacentNode.parentNode.insertBefore(this.node, adjacentNode.nextSibling);
                    }
                } else {
                    adjacentNode = adjacentNodeOrRelation;
                    if (position === 'before') {
                        const firstRelationAfter = this.from.outgoingRelations.find(
                            relation => relation !== this && (adjacentNode === relation.node || adjacentNode.compareDocumentPosition(relation.node) & relation.node.DOCUMENT_POSITION_FOLLOWING)
                        );
                    ...
                    if (position === 'after') {
                        const relationsBeforeOrAtAdjacentNode = this.from.outgoingRelations.filter(
                            relation => relation !== this && (adjacentNode === relation.node || adjacentNode.compareDocumentPosition(relation.node) & relation.node.DOCUMENT_POSITION_PRECEDING)
                        );
                        if (relationsBeforeOrAtAdjacentNode.length > 0) {
                            adjacentRelation = relationsBeforeOrAtAdjacentNode[relationsBeforeOrAtAdjacentNode.length - 1];
                        } else {
                            position = 'first';
                        }
                        adjacentNode.parentNode.insertBefore(this.node, adjacentNode.nextSibling);
                ...
                if (lastExistingRelation) {
                    lastExistingRelation.node.parentNode.insertBefore(this.node, lastExistingRelation.node.nextSibling);
                ...
                if (this.node.compareDocumentPosition) {
                    ...
                    const relationsBefore = this.from.outgoingRelations.filter(
                        relation => relation !== this && (this.node === relation.node || this.node.compareDocumentPosition(relation.node) & relation.node.DOCUMENT_POSITION_PRECEDING)
                    );
                ...
                if (firstExistingRelation) {
                    firstExistingRelation.node.parentNode.insertBefore(this.node, firstExistingRelation.node);
                } else if (document.head && this.preferredPosition === 'firstInHead') {
                    document.head.insertBefore(this.node, document.head.firstChild);
                } else if (document.head && this.preferredPosition === 'lastInHead') {
                    document.head.appendChild(this.node);
                    ...
                    document.documentElement.insertBefore(this.node, document.documentElement.firstChild);
                }
                if (this.node.compareDocumentPosition) {
                    // not supported by xmldom
                    adjacentRelation = this.from.outgoingRelations.find(
                        relation => relation !== this && (this.node === relation.node || this.node.compareDocumentPosition(relation.node) & relation.node.DOCUMENT_POSITION_FOLLOWING)
                    );
                    if (adjacentRelation) {
                        position = 'before';
                    } else {
