...
extendWithGettersAndSetters(JavaScriptAmdRequire.prototype, {
    ...
    set href(href) {
        var existingHref = this.href,
            matchExistingPrefix = existingHref.match(/^.*!/);
    ...
    refreshHref: function () {
        ...
        if (that.to && that.to.url) {
            var hrefType = that.hrefType,
                href;
            if (hrefType === 'moduleName') {
                var baseUrl = (that.assetGraph && that.assetGraph.requireJsConfig && that.assetGraph.requireJsConfig.baseUrl) || that.baseAsset.nonInlineAncestor.url;
