...
extendWithGettersAndSetters(StaticUrlMapEntry.prototype, {
    ...
    attach: function (asset, position, adjacentRelation) {
        ...
        if (adjacentRelation && !this.wildCardValues) {
            ...
            if (matchWildCardValues) {
                this.wildCardValues = Array.prototype.slice.call(matchWildCardValues, 1);
            } else {
    ...
    detach: function () {
        if (this.wildCardValues) {
            ...
            this.wildCardValues.forEach(function (wildCardValue, i) {
                if (i === this.from.wildCardValueAsts.length - 1) {
                    if (wildCardValue in cursor) {
                        ...
                        throw new Error("relation.StaticUrlMapEntry.detach: The relation's set of wild card values aren't mapped.");
                    }
                } else {
                    if (!(wildCardValue in cursor)) {
                        throw new Error("relation.StaticUrlMapEntry.detach: The relation's set of wild card values aren't mapped.");
            ...
            throw new Error("relation.StaticUrlMapEntry.detach: Cannot detach relation without wildCardValues property or asterisks in its href");
        }
