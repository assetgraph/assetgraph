...
/** @typedef {import('../util/getHrefType').HrefType} HrefType */

/** @typedef {'first' | 'last' | 'before' | 'after'} RelationAttachmentPosition */

/**
 * @typedef {object} RelationConfig
 * @prop {string} type
 * @prop {Asset} [from]
 * @prop {string | Asset | AssetConfig} [to]
 * @prop {string} [href]
 * @prop {HrefType} [hrefType]
...
class Relation {
   ...
   *
   * @constructor
   * @param {RelationConfig} config The relation configuration
   */
  constructor(config) {
    const { type, from, to, href, hrefType, ...remainingConfig } = config;
    ...
    Object.assign(this, remainingConfig);
   ...
   */
  get to() {
    return /** @type{Asset} */ (this._to);
  }

  /** @param {string | Asset | AssetConfig} to */
  set to(to) {
  ...
  refreshHref() {
    ...
    if (hrefType === 'inline') {
      ...
      if (hrefType === 'rootRelative' && !canonical) {
        href = assetGraph.buildRootRelativeUrl(targetUrl, this.baseUrl);
      } else if (hrefType === 'relative' && !canonical) {
        href = urlTools.buildRelativeUrl(this.baseUrl, targetUrl);
        if (this.nonBareRelative && !/^\.\.\/|^\/|^[a-z0-9+]+:/.test(href)) {
          href = `./${href}`;
        }
   ...
   *
   * @returns {this} The relation itself (chaining-friendly).
   */
  inline() {
    if (
      this.to.isAsset &&
   ...
   * @return {this} The relation itself (chaining-friendly).
   */
  ...
  /**
   * @param {RelationAttachmentPosition} position
   * @param {Relation} adjacentRelation
   */
 ...
 * @type {string}
 */
Relation.prototype.targetType = undefined;

/**
 * @type {string}
 */
Relation.prototype.href;

/**
 * @type {boolean}
 */
Relation.prototype.nonBareRelative = false;
