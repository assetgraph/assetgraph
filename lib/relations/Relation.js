...
Relation.prototype = {
    ...
    attach: function (asset, position, adjacentRelation) {
        this.from = asset;
        this.from.markDirty();
        if (this.from.assetGraph && this.to && this.to.isAsset) {
    ...
    _registerBaseAssetPath: function () {
        ...
        this.assetGraph._baseAssetPathForRelation[this.id] = baseAssetPath;
    ...
    _unregisterBaseAssetPath: function () {
        ...
        var baseAssetPath = this.assetGraph._baseAssetPathForRelation[this.id];
        if (baseAssetPath) { // FIXME: Should always be there!
            if (baseAssetPath.length === 0) {
                ...
                baseAssetPath.forEach(function (obj) {
                    if (this.assetGraph._objInBaseAssetPaths[obj.id]) { // FIXME: Should always be there!
                        this.assetGraph._objInBaseAssetPaths[obj.id].splice(this.assetGraph._objInBaseAssetPaths[obj.id].indexOf(this), 1);
                    }
                }, this);
            }
            delete this.assetGraph._baseAssetPathForRelation[this.id];
        }
    },
