...
Relation.prototype = {
    ...
    refreshHref: function () {
        ...
        if (this.to && this.to.url) {
            ...
            if (hrefType === 'rootRelative') {
                ...
                href = urlTools.buildRelativeUrl(this.baseUrl, this.to.url);
            } else if (hrefType === 'protocolRelative') {
                href = urlTools.buildProtocolRelativeUrl(this.baseUrl, this.to.url);
            } else {
                // Absolute
                href = this.to.url;
            }
    ...
    get baseUrl() {
        ...
        if (baseAsset) {
            ...
            baseUrl = this.from.nonInlineAncestor.url;
        }
        return baseUrl;
    },
    ...
    resolve: function (cb) {
        ...
        if (baseUrl) {
            this.from.assetGraph.resolveAssetConfig(this.to, baseUrl, passError(cb, function (resolvedAssetConfigs) {
                ...
                if (resolvedAssetConfigs.length === 0) {
                    ...
                    resolvedAssetConfigs.forEach(function (resolvedAssetConfig) {
                        var multipliedRelation = new this.constructor({to: resolvedAssetConfig, hrefType: this.hrefType});
            ...
            setImmediate(function () {
                cb(new Error('No base url found for relation (' + that.hrefType + ') ' + that.toString()));
            });
        }
    },
