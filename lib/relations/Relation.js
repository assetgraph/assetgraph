...
Relation.prototype = {
    ...
    refreshHref: function () {
        ...
        if (this.to.url) {
            var url;
            if (this.hrefType === 'relative') {
                url = urlTools.buildRelativeUrl(this.baseAsset.url, this.to.url);
            } else {
                url = this.to.url;
            }
            // Don't do anything for relation types that are "always inline",
            // such as HtmlConditionalComment and JavaScriptOneGetStaticUrl.
            if ('href' in this) {
                if (this.href !== url) {
                    this.href = url;
                    this.from.markDirty();
     ...
     * =================================
     *
     * Either 'absolute' or 'relative' (the default). Controls whether
