...
Relation.prototype = {
    ...
    refreshHref: function () {
        ...
        var targetUrl;
        if (this.to) {
            if (this.to.isResolved) {
                targetUrl = this.to.url;
            } else {
                // Unresolved relation -- strip off the fragment identifier
                // so we don't double it.
                // Fortunately, all of this goes away in AssetGraph 4
                targetUrl = this.to.url.replace(/#.*$/, '');
            }
        }
        if (targetUrl) {
