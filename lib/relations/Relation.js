...
class Relation {
    ...
    /**
     * relation.fragment (getter/setter)
     * =================================
     *
    ...
    refreshHref() {
        ...
        if (hrefType === 'inline') {
            ...
            if (hrefType === 'rootRelative' && !canonical) {
                href = urlTools.buildRootRelativeUrl(this.baseUrl, targetUrl, assetGraph.root);
     ...
     * Decides what "degree" of relative url refreshHref tries to issue, except for
    ...
    set hrefType(hrefType) {
        if (hrefType === 'inline') {
            this.inline();
        } else if (hrefType !== this._hrefType) {
     ...
     */
    inline() {
        if (this.to.incomingRelations.filter(r => r._to !== r.from).length > 1) {
