...
class HtmlPreloadLink extends HtmlResourceHint {
    ...
    attach(asset, position, adjacentRelation) {
        ...
        if (position === 'before' || position === 'after') {
            ...
            let head = this.from.parseTree.head;
            if (!head) {
                head = this.from.parseTree.createElement('head');
                this.from.parseTree.documentElement.insertBefore(head);
            }
            if (position === 'first') {
                this.from.parseTree.head.insertBefore(this.node);
            } else {
                // position === 'last'
