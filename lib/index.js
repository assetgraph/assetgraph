    ...
    resolveAssetConfig: function (assetConfig, fromUrl) {
        ...
        if (url) {
            ...
            if (protocol === 'javascript') {
                ...
                assetConfig.rawSrcProxy = function () {
                    return that.teepee.request(_.defaults({ url: url }, that.requestOptions)).then(function (response) {
            ...
            if (metadata && metadata.contentType) {
                ...
                return new Promise(function (resolve, reject) {
                    fileProcess.stdout.on('data', function (chunk) {
                        ...
                        resolve(foundType(AssetGraph.lookupContentType(fileOutput.match(/^([^\n]*)/)[1])));
                    });
                });
            }
        })
