...
//        asset.populate();
    ...
    _traverseAsync: function (startAssetOrRelation, relationQueryObj, preOrderLambda, postOrderLambda, cb) {
        preOrderLambda = preOrderLambda || function (asset, incomingRelation, cb) {
            setImmediate(cb);
        };
        postOrderLambda = postOrderLambda || function (asset, incomingRelation, cb) {
            setImmediate(cb);
        };
        var that = this,
            relationQueryMatcher = relationQueryObj && this.query.createValueMatcher(relationQueryObj),
            startAsset,
            startRelation;
        if (startAssetOrRelation.isRelation) {
            startRelation = startAssetOrRelation;
        ...
        var seenAssets = {},
            assetStack = [];
        (function traverse(asset, incomingRelation, cb) {
            if (seenAssets[asset.id]) {
                return setImmediate(cb);
            }
            preOrderLambda(asset, incomingRelation, function (err) {
                if (err) {
                    return cb(err);
                }
                seenAssets[asset.id] = true;
                assetStack.push(asset);
                var outgoingRelations = that.findRelations({from: asset}, true);

                async.eachLimit(outgoingRelations, 1, function foo(outgoingRelation, cb) {
                    if (!outgoingRelation.to.isAsset) {
                        return that.resolveAssetConfig(outgoingRelation.to, outgoingRelation.baseAsset.nonInlineAncestor.url, function (err, resolvedAssetConfig) {
                            outgoingRelation.to = that.createAsset(resolvedAssetConfig);
                            that.addAsset(outgoingRelation.to);
                            foo(outgoingRelation, cb);
                        });
                    }

                    if (!relationQueryMatcher || relationQueryMatcher(outgoingRelation)) {
                        traverse(outgoingRelation.to, outgoingRelation, cb);
                    } else {
                        setImmediate(cb);
                    }
                }, passError(cb, function () {
                    postOrderLambda(assetStack.pop(), incomingRelation, cb);
                }));
            });
        }(startAsset, startRelation, cb));
    },

    eachAssetPreOrderAsync: function (startAssetOrRelation, relationQueryObj, lambda, cb) {
        this._traverseAsync(startAssetOrRelation, relationQueryObj, lambda, null, cb);
    },

    eachAssetPostOrderAsync: function (startAssetOrRelation, relationQueryObj, lambda, cb) {
        this._traverseAsync(startAssetOrRelation, relationQueryObj, null, lambda, cb);
    },

    populateAsset: function (asset, incomingRelation, context) {
        var that = this;
        if ((asset.type === 'JavaScript' && asset.fileName === 'require.js') || asset.type === 'RequireJsConfig') {
            context = context.cloneAndExpand(asset);
        }
        asset.outgoingRelations.forEach(function (outgoingRelation) {
            if (outgoingRelation.assetGraph) {
                if (outgoingRelation.hrefType === 'moduleName') {
                    var url = context.vmContext.require.toUrl(outgoingRelation.href),
                        hasThisContext = false,
                        resolvesToDifferentUrls = false;
                    for (var i = 0 ; i < outgoingRelation.contexts.length ; i += 1) {
                        if (outgoingRelation.contexts[i] === context) {
                            hasThisContext = true;
                        } else if (outgoingRelation.contexts[i].vmContext.require && outgoingRelation.contexts[i].vmContext.require.toUrl(outgoingRelation.href) !== url) {
                            resolvesToDifferentUrls = true;
                        }
                    }
                    if (!hasThisContext) {
                        outgoingRelation.contexts.push(context);
                    }
                    if (resolvesToDifferentUrls) {
                        throw new Error('The module name ' + outgoingRelation.href + ' resolves to different urls in different contexts: ' + outgoingRelation.contexts.map(function (context) {
                            return context.vmContext.require.toUrl(outgoingRelation.href);
                        }).sort().join(' '));
                    }
                }
            } else {
                outgoingRelation.contexts = [context];
                if (!outgoingRelation.to && (outgoingRelation.type === 'JavaScriptAmdDefine' || outgoingRelation.type === 'JavaScriptAmdRequire' || outgoingRelation.type === 'JavaScriptShimRequire')) {

                    var href = outgoingRelation.href;
                    if (outgoingRelation.hrefType === 'moduleName') {
                        href += '.js';
                    }
                    outgoingRelation.to = {
                        url: context.vmContext.require.toUrl(href)
                    };
                }
                ...
                if (outgoingRelation.to.url || typeof outgoingRelation.to === 'string') {
                    ...
                    var relativeUrl;
                    if (outgoingRelation.hrefType === 'moduleName') {
                        relativeUrl = context.vmContext.require.toUrl(outgoingRelation.href);
                    } else {
                        relativeUrl = outgoingRelation.to.url || outgoingRelation.to;
                    }
                    if (/^data:/.test(relativeUrl)) {
                        var assetConfig = resolveDataUrl(relativeUrl);
                        if (assetConfig) {
                            assetConfig.type = that.lookupContentType(assetConfig.contentType);
                            outgoingRelation.to = that.createAsset(assetConfig);
                            that.addAsset(outgoingRelation.to);
                        }
                    } else {
                        ...
                        if (baseAsset) {
                            var baseAssetUrl = baseAsset.nonInlineAncestor.url,
                                targetUrl = that.resolveUrl(baseAssetUrl, relativeUrl),
                                targetAssets = that.findAssets({url: targetUrl});
                    ...
                    that.addRelation(outgoingRelation);
                } else {
                    // Inline asset
                    that.addRelation(outgoingRelation);
    ...
    _browserPopulate: function (initialAssetsQueryObj, relationsQueryObj, cb) {
        var that = this,
            initialAssets = initialAssetsQueryObj.isAsset ? [initialAssetsQueryObj] : this.findAssets(initialAssetsQueryObj);

        async.eachLimit(initialAssets, 1, function (initialAsset, cb) {
            var context = new Context(initialAsset, that);
            that.eachAssetPreOrderAsync(initialAsset, relationsQueryObj, function preOrderCb(asset, incomingRelation, cb) {
                if (!asset.isLoaded) {
                    return asset.load(function (err) {
                        if (err) {
                            ...
                            preOrderCb(asset, incomingRelation, cb);
                        }
                    });
                }
                ...
                try {
                    context = that.populateAsset(asset, incomingRelation, context);
                } catch (err) {
                    return cb(err);
                }
                cb();
