...
var util = require('util'),
    ...
    knownAndUnsupportedProtocols = ['mailto', 'tel', 'sms', 'fax', 'android-app', 'chrome-extension', 'about', 'magnet'].reduce(function (obj, protocol) {
        obj[protocol] = true;
        return obj;
    }, {});
                ...
                if (resolver) {
                    ...
                    return setImmediate(function () {
                        if (!knownAndUnsupportedProtocols[protocol]) {
                           that.emit('warn', new Error('No resolver found for protocol: ' + protocol));
                       }
                       cb(null, assetConfig);
                    });
                }
