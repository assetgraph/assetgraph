...
SenchaJSBuilder.prototype = {
    fixupAssetConfig: function (assetConfig, cb) {
        if (/\.css$/.test(assetConfig.url) && /^Ext JS Library [23].\d.\d/.test(this.body.licenseText)) {
            ...
            fs.readFile(fileUtils.fileUrlToFsPath(assetConfig.url), 'utf8', error.passToFunction(cb, function (src) {
                assetConfig.type = 'CSS';
                assetConfig.originalSrc = src.replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/url\s*\(\s*/g, function () {
