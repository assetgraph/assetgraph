        ...
        seq.ap(fileNames)
            ...
            .seq(function () {
                cb(null, {
                    ...
                    url: urlTools.resolveUrl(that.url, pkg.target)
                });
            })
            ['catch'](cb);
    },
    ...
    resolvePkg: function (pkg, cb) {
        var that = this,
            ...
            fileNames = (pkg.files || []).map(function (fileDef) {
                return urlTools.resolveUrl(that.url, fileDef.path + fileDef.name);
            });
    ...
    resolve: function (labelRelativePath, cb) {
        if (labelRelativePath in this.pkgIndex) {
            ...
            this.fixupAssetConfig({
                url: urlTools.resolveUrl(this.url, labelRelativePath)
            }, cb);
        }
    }
};
