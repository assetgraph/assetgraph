    ...
    return function http(assetConfig, fromUrl, cb) {
        ...
        assetConfig.rawSrcProxy = function (cb) {
            var matchUrl = url.match(/^([^#]*)((?:#.*)?)$/),
                ...
                req;
            req = request({
                // Hack: Allows you to control the request headers by setting 'headers' of the returned function
                headers: http.headers || {},
                jar: false, // Prevent request from overwriting http.headers.Cookie with an empty string
                ...
                if (response.statusCode >= 400) {
                    ...
                    if ('content-type' in response.headers) {
                        var matchContentType = response.headers['content-type'].match(/^\s*([\w\-\+\.]+\/[\w\-\+\.]+)\s*(?:;\s*charset=([\'\"]|)\s*([\w\-]+)\s*\2\s*)?$/i);
                        if (matchContentType) {
