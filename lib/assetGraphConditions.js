...
assetGraphConditions.parse = node => {
    const assetGraphConditionsAttributeValue = node && node.getAttribute('data-assetgraph-conditions');
    if (assetGraphConditionsAttributeValue) {
        const obj = {};
        for (const keyAndValue of objectLiteralParse(assetGraphConditionsAttributeValue)) {
...
assetGraphConditions.stringify = obj => {
    return Object.keys(obj).map(
        key => (/[^a-z]/.test(key) ? singleQuoteString(key) : key) + ': ' + (Array.isArray(obj[key]) ? '[' + obj[key].map(singleQuoteString).join(', ') + ']' : singleQuoteString(obj[key]))
    ).join(', ');
};
