...
var ipv4DigitRegExp = /([0-9]|1?[0-9][0-9]|2[0-4][0-9]|25[0-5])/;
...
function origin(url) {
    ...
    var matchOrigin = url.match(/^([a-z][a-z0-9+.-]+):\/\/(\*|(?:\*\.)?[a-z0-9-]+(?:\.[a-z0-9-]+)*)(?::(\d+|\*))?(\/.*)?$/i);
    if (matchOrigin) {
        var scheme = matchOrigin[1];
        var host = matchOrigin[2];
        var port = matchOrigin[3] || defaultPortByScheme[scheme];
...
module.exports = function matchSourceExpression(url, sourceExpression, protectedResourceUrl) {
    var protectedResourceUrlObj = urlModule.parse(protectedResourceUrl);
    ...
    if (sourceExpression === '*' && urlObj.protocol !== 'data:' && urlObj.protocol !== 'blob:' && urlObj.protocol !== 'filesystem:') {
        return true;
    }
    ...
    var matchHostSource = sourceExpression.match(/^(?:([a-z][a-z0-9+.-]+):\/\/)?(\*|(?:\*\.)?[a-z0-9-]+(?:\.[a-z0-9-]+)*)(?::(\d+|\*))?(\/.*)?$/i);
    if (matchHostSource) {
        var sourceExpressionScheme = matchHostSource[1];
        var sourceExpressionHostPart = matchHostSource[2];
        var sourceExpressionPortPart = matchHostSource[3];
        var sourceExpressionPathPart = matchHostSource[4];
        ...
        var urlScheme = urlObj.protocol.replace(/:$/, '');
        var urlHost = urlObj.hostname;
        var urlPort = urlObj.port === null ? defaultPortByScheme[urlScheme] : parseInt(urlObj.port, 10);

        // Let url-path-list be the path of url.
        var urlPathList = slashSplit(urlObj.pathname);
        ...
        if (sourceExpressionScheme && sourceExpressionScheme.toLowerCase() !== urlScheme.toLowerCase()) {
            return false;
        }
        ...
        if (!sourceExpressionScheme) {
            ...
            if (protectedResourceUrlObj.protocol.toLowerCase() === 'http:' && !/^https?/i.test(urlScheme)) {
                return false;
            }
            ...
            if (protectedResourceUrlObj.protocol.toLowerCase() !== 'http:' && urlScheme !== protectedResourceUrlObj.protocol.replace(/:$/, '').toLowerCase()) {
        ...
        if (sourceExpressionHostPart.charAt(0) === '*' && urlHost.toLowerCase().lastIndexOf(sourceExpressionHostPart.substr(1).toLowerCase()) !== urlHost.length - sourceExpressionHostPart.length + 1) {
            return false;
        }
        ...
        if (sourceExpressionHostPart.charAt(0) !== '*' && urlHost.toLowerCase() !== sourceExpressionHostPart.toLowerCase()) {
            return false;
        }
        ...
        if (ipv4RegExp.test(sourceExpressionHostPart) && sourceExpressionHostPart !== '127.0.0.1') {
            return false;
        }
        ...
        if (!sourceExpressionPortPart && urlPort !== defaultPortByScheme[urlScheme]) {
            return false;
        }
        ...
        if (sourceExpressionPathPart) {
            ...
            var exactMatch = !/\/$/.test(sourceExpressionPathPart);

            // Let source-expression-path-list be the result of splitting path-part on the U+002F SOLIDUS character (/).
            var sourceExpressionPathList = slashSplit(sourceExpressionPathPart);
            ...
            for (var i = 0 ; i < sourceExpressionPathList.length ; i += 1) {
                // Percent decode entry.
                // (intentionally not using decodeURIComponent so we can support urls with non-UTF8 octets)
                var decodedSourceExpressionPathFragment = unescape(sourceExpressionPathList[i]);
                ...
                if (unescape(urlPathList.shift()) !== unescape(decodedSourceExpressionPathFragment)) {
                    return false;
                }
            }
    ...
    if (sourceExpression.toLowerCase() === '\'self\'') {
        ...
        var urlOrigin = origin(url);
        var protectedResourceOrigin = origin(protectedResourceUrl);
        if (urlOrigin && protectedResourceOrigin && urlOrigin === protectedResourceOrigin) {
            return true;
        }
    }
