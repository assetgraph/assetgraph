...
const ipv4RegExp = new RegExp(
  `^${ipv4DigitRegExp.source}\\.${ipv4DigitRegExp.source}\\.${ipv4DigitRegExp.source}\\.${ipv4DigitRegExp.source}$`
);
...
module.exports = function matchSourceExpression(
  ...
  const protectedResourceUrlObj = new urlModule.URL(protectedResourceUrl);
  ...
  if (matchHostSource) {
    ...
    // If url’s host is absent, return does not match.
    if (urlObj.hostname === '') {
      return false;
    }
