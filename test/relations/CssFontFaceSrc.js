...
describe('relations/CssFontFaceSrc', function () {
    it('should handle a simple test case', function () {
        return new AssetGraph({root: __dirname + '/../../testdata/relations/CssFontFaceSrc/simple/'})
    ...
    it('should handle a test case with multiple src properties in one rule and multiple urls in one value', function () {
        return new AssetGraph({root: __dirname + '/../../testdata/relations/CssFontFaceSrc/multipleSrc/'})
            .loadAssets('index.css')
            .populate()
            .queue(function (assetGraph) {
                expect(assetGraph, 'to contain asset', 'Css');
