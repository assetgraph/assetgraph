...
describe('relations/CssBehavior', function() {
  it('should handle a simple test case', async function() {
    ...
    expect(
      assetGraph.findRelations({ type: 'CssBehavior' })[0].href,
      'to equal',
      '/htc/theBehavior.htc'
    );
    expect(
    ...
    expect(
      assetGraph.findRelations({ type: 'HtmlScript' })[0].href,
      'to equal',
      '../js/theScript.js'
    );
    expect(
      assetGraph.findAssets({ type: 'Htc' })[0].text,
      'to match',
      /src=['"]\.\.\/js\/theScript\.js/
    );
  });
