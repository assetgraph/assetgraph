...
describe('AssetGraph.findAssets', function() {
  it('should handle a test case with 6 assets', async function() {
    ...
    expect(
      assetGraph,
      'to contain assets',
      { foo: { $or: ['quux', val => typeof val === 'undefined'] } },
      2
    );
    ...
    expect(assetGraph, 'to contain no assets', {
      type: 'Html',
      incomingRelations: { $elemMatch: { type: 'HtmlAnchor' } }
    });

    expect(assetGraph, 'to contain no assets', {
      outgoing: { type: 'HtmlAnchor' }
    });
    ...
    expect(assetGraph, 'to contain asset', {
      outgoingRelations: { $elemMatch: { type: 'HtmlAnchor' } }
    });
  });
});
