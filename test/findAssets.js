...
describe('AssetGraph.findAssets', function () {
    it('should handle a test case with 6 assets', async function () {
        ...
        await assetGraph.loadAssets(
            ...
            new AssetGraph.Css({text: 'body { color: #ddd; }', foo: 'baz'}),
            new AssetGraph.Css({text: 'body { color: #eee; }'}),
        ...
        assetGraph.findAssets({text: 'a'})[0].addRelation({
            type: 'HtmlAnchor',
            to: assetGraph.findAssets({text: 'b'})[0],
            node: false
        }, 'last');
        assetGraph.findAssets({text: 'a'})[0].addRelation({ // Identical to the first
            type: 'HtmlAnchor',
            to: assetGraph.findAssets({text: 'b'})[0],
            node: false
        }, 'last');
