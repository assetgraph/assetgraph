...
describe('transforms/inlineCssImagesWithLegacyFallback', function() {
  it('should handle a test case with a single Html asset', async function() {
    ...
    expect(
      assetGraph.findAssets({ fileName: 'smallImagesWithInlineFalse.css' })[0]
        .text,
      'not to match',
      /inline=false/
    );
    ...
    expect(
      ...
      { type: 'CssImage', to: { isInline: true } },
      3
    );

    expect(
      assetGraph.findAssets({ type: 'Html' })[0].text.match(/<link[^>]*>/g),
      'to have length',
      7
    );
    ...
    expect(
      text.match(/<!--\[if lt IE 8\]><link[^>]*><!\[endif\]-->/g),
      'to have length',
      1
    );
    ...
    expect(
      text.match(/<!--\[if lt IE 9\]><link[^>]*><!\[endif\]-->/g),
      'to have length',
      1
    );
  });
  ...
  it('should handle a test case with multiple Html asset that point at the same Css', async function() {
    ...
    expect(
      assetGraph
        .findAssets({ type: 'Html', fileName: '1.html' })[0]
        .text.match(/<link[^>]*>/g),
      'to have length',
      2
    );
    ...
    expect(
      assetGraph
        .findAssets({ type: 'Html', fileName: '2.html' })[0]
        .text.match(/<link[^>]*>/g),
      'to have length',
      2
    );
  ...
  it('should handle a test case with a root-relative HtmlStyle relation', async function() {
    ...
    expect(
      assetGraph,
      'to contain relations',
      { type: 'HtmlStyle', hrefType: 'rootRelative' },
      2
    );
  });
