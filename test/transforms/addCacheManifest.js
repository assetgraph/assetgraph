...
describe('transforms/addCacheManifest', function () {
    it('should handle a single page with an existing cache manifest', async function () {
        ...
        const outgoingRelations = assetGraph.findRelations({from: assetGraph.findAssets({type: 'CacheManifest'})[0]});
    ...
    it('should add cache manifest to multiple pages', async function () {
        ...
        const cacheManifest = assetGraph.findAssets({type: 'CacheManifest', incoming: {from: {url: /\/index\.html$/}}})[0];
        ...
        const otherCacheManifest = assetGraph.findAssets({type: 'CacheManifest', incoming: {from: {url: /\/otherpage\.html$/}}})[0];
