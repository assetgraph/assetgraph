...
describe('transforms/bundleSystemJs', function() {
  ...
  it('should handle a multi-page test case with one System.import call per page importing the same thing', async function() {
    ...
    expect(
      assetGraph,
      'to contain relations',
      { type: 'HtmlScript', from: { fileName: 'page1.html' } },
      3
    );
    expect(
      assetGraph,
      'to contain relations',
      { type: 'HtmlScript', from: { fileName: 'page2.html' } },
      3
    );
  ...
  it('should add a SystemJsBundle relation to a non-Css entry in the asset list', async function() {
    ...
    expect(
      assetGraph.findAssets({ fileName: 'common-bundle.js' })[0].text,
      'to contain',
      '//# SystemJsBundle=/foo.html'
    );
  });
  ...
  it('should add a SystemJsBundle relation to a conditional non-Css entry in the asset list', async function() {
    ...
    expect(
      assetGraph.findAssets({ fileName: 'bundle-main-sunny.js' })[0].text,
      'to contain',
      '//# SystemJsBundle=/foo-sunny.html'
    ).and('not to contain', 'rainy');
    expect(
      assetGraph.findAssets({ fileName: 'bundle-main-rainy.js' })[0].text,
      'to contain',
      '//# SystemJsBundle=/foo-rainy.html'
  ...
  it('should error out if two pages include the same System.config assets in different orders', async function() {
    await expect(
      ...
      'to be rejected with',
      new Error(
        'bundleSystemJs transform: System.config calls come in conflicting order across pages'
      )
    );
  });
  ...
  it('should error out if two pages include conflicting System.js configs', async function() {
    await expect(
      new AssetGraph({
        ...
        .bundleSystemJs(),
      'to be rejected with',
      new Error('bundleSystemJs transform: Configs conflict')
    );
  });
  ...
  it('should handle the use of system.js in a web worker', async function() {
    ...
    expect(
      ...
      'to contain',
      "importScripts('system.js', 'config.js', '/common-bundle.js')"
    );
  });
  ...
  it('should handle the use of system.js in a service worker registered via <link rel="serviceworker" href=...>', async function() {
    ...
    expect(assetGraph, 'to contain relations', 'JavaScriptImportScripts', 3);
    expect(
      assetGraph.findAssets({ fileName: 'sw.js' })[0].text,
      'to contain',
      "importScripts('system.js', 'config.js', '/common-bundle.js')"
    );
  });
  ...
  it('should error out if the same manual bundle is defined multiple times', async function() {
    return expect(
      ...
      'to be rejected with',
      new Error(
        'bundleSystemJs transform: Conflicting definitions of the manual bundle foo'
      )
    );
  });
  ...
  describe('with a data-systemjs-polyfill attribute', function() {
    it('should remove the data-systemjs-polyfill attribute', async function() {
      ...
      expect(
        assetGraph,
        'to contain relations',
        { type: 'HtmlScript', to: { isInline: true } },
        2
      );
      expect(
        assetGraph.findAssets({ type: 'Html' })[0].text,
        'not to contain',
        'data-systemjs-polyfill'
      );
    });
    ...
    it('should polyfill system.js when instructed to', async function() {
      ...
      await assetGraph.bundleSystemJs({ polyfill: true }).populate();
  ...
  it('should pick up assets referenced via an asset plugin', async function() {
    ...
    assetGraph.findAssets({ fileName: 'test-foo.txt' })[0].fileName =
      'somethingElse.txt';
    expect(
      assetGraph.findAssets({ fileName: 'bundle-main-foo.js' })[0].text,
      'to contain',
      "'/somethingElse.txt'.toString('url')"
  ...
  it('should pick up assets referenced via an asset plugin using a wildcard', async function() {
    ...
    expect(
      ...
      .and(
        'to contain',
        "System.registerDynamic('test-*.txt!systemjs-asset-plugin/asset-plugin.js'"
      );
  });
  ...
  describe('with a buildConfig property in a System.config({...})', function() {
    ...
    it('should remove the buildConfig and testConfig properties after building', async function() {
      ...
      expect(
        assetGraph.findAssets({ fileName: 'build-config.js' })[0].text,
        'not to contain',
        'buildConfig'
  ...
  describe('with conditionals', async function() {
    ...
    describe('with unspecified conditionals', async function() {
      it('should create separate bundles per variant', async function() {
        ...
        expect(
          assetGraph.findAssets({ fileName: 'common-bundle.js' })[0].text,
          'to contain',
          "System.registerDynamic('lang.js"
        );
      });
      ...
      it('should create separate stylesheets per variant', async function() {
        ...
        expect(
          ...
          'to contain',
          '<link rel="stylesheet" href="/styles.en_us.css" data-assetgraph-conditions="\'lang.js|default\': \'en_us\'">'
        );
      });
    ...
    it('should support a combination of paths aliases and conditions', async function() {
      ...
      expect(
        assetGraph.findAssets({ fileName: 'common-bundle.js' })[0].text,
        'to contain',
        "System.registerDynamic('environment'"
      ).and(
        'to contain',
        "System.registerDynamic('main.js', ['virtual-#{environment|default}.configjson', 'virtual-#{locale|default}.i18n', 'actual-#{locale|default}.js']"
      );
    });
  });
});
